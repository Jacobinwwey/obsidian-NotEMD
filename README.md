...
- **Experimental Diagram Pipeline**:
    ...
    - Multi-panel previews preserve every Mermaid fence and every persisted Drawnix source visual. The top-level SVG export asks for the source folder or a custom Vault-relative folder, then writes one deterministic SVG per panel (for example `Architecture_preview_mermaid-2.svg`) and continues after an individual panel failure; each panel header also exposes SVG/PNG/PDF export for one diagram or image at a time.
    - PNG/PDF raster export uses a canvas-safe intermediate SVG: Mermaid raster rendering disables HTML labels, foreignObject labels are converted to SVG text, and external image references are removed while data-backed companions remain exportable. Original SVG export remains vector-preserving.
