...
- **Experimental Diagram Pipeline**:
    ...
    - Multi-panel previews preserve every Mermaid fence and every persisted Drawnix source visual. The top-level SVG, PNG, and PDF exports all ask for the source folder or a custom Vault-relative folder, then write one deterministic image per panel (for example `Architecture_preview_mermaid-2.svg`) in source order and continue after an individual panel failure; each panel header also exposes SVG/PNG/PDF export for one diagram or image at a time.
    - Drawnix Mermaid visuals are embedded as sanitized SVG plus source text in the `.drawnix` metadata, so Mermaid-only diagrams do not create a separate `.assets` tree; regeneration removes only stale plugin-owned Mermaid companions with a valid manifest, while external binary or unresolved visuals retain companions when they cannot be embedded safely. Drawnix relation labels use node-aware orthogonal routing, measured wrapping, and a final label layer so long explanations are not hidden by node rectangles; native arrow text and SVG previews share the same wrapped lines. The title/summary header is a protected route region and is rendered last with an opaque guard band, so perimeter dashed routes cannot obscure it.
    - PNG/PDF raster export uses a canvas-safe intermediate SVG: Mermaid raster rendering disables HTML labels, foreignObject labels are converted to SVG text, external image references are removed while data-backed companions remain exportable, and multi-panel styles/definitions remain attached to each nested SVG canvas. Original SVG export remains vector-preserving.
...
| Drawnix | `_diagram.drawnix` with inline Mermaid source visuals; SVG/media companions only for the primary preview or unresolved/non-inline media | SVG companion | Yes | Yes | Yes | Yes | Single document root by default, node-aware labelled arrows, obstacle-aware cross-root routing, self-contained Mermaid previews, and no Drawnix or Plait runtime bundled. |
...
-   **Max Tokens**: Global maximum tokens the LLM should generate per response chunk. Affects cost and detail. If the active provider has a **Provider output token override** set in advanced settings, that provider-specific cap takes precedence. When this field is still on the default auto-managed baseline, switching the provider model can also update it to that model's known output-token ceiling. (Default: 8192)
-   **Chunk Word Count**: Maximum words per chunk sent to the LLM. Affects the number of API calls for large files. Recommended default: one third of **Max Tokens**, rounded up. If you have not customized it yet, changing **Max Tokens** or switching to a different known-cap model auto-fills the recommended chunk size for you. (Default: 3000)
