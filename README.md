...
- **Experimental Diagram Pipeline**:
    - The [Diagram Gallery](./docs/diagram-gallery.md) lists the ten executable semantic types, their default/compatible render targets, and production SVG/PNG previews. Reference-only layouts from `diagram-design` are labeled explicitly and are not runtime support.
...
| Drawnix | `_diagram.drawnix` with inline Mermaid/source visuals by default; optional `.assets` companions when complete Mermaid export is enabled | SVG companion | Yes | Yes | Yes | Yes | Self-contained previews, legacy `.assets` compatibility, node-aware labelled arrows, obstacle-aware cross-root routing, and no Drawnix or Plait runtime bundled. |
