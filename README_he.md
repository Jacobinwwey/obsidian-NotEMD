...
- כל פרויקט שומר על הרישיון והתנאים שלו; Notemd זמין תחת [רישיון MIT](./LICENSE).
...
*Notemd v1.9.6 - שפרו את גרף הידע שלכם ב-Obsidian בעזרת AI.*


<!-- repo-chronicle:start -->
## כרוניקת הפיתוח

הכרוניקה הרבעונית הזו שומרת על הסגנון החזותי המקורי של [repo-saga](https://github.com/teee32/repo-saga); כעת repo-saga עצמו מייצר את החלוקה לרבעונים.

![Notemd Development Chronicle](./docs/repo-saga/notemd-development-history.he.svg)

_רוענן לאחרונה עבור תג השחרור `1.9.6` בתאריך 2026-08-09. תאריך הקומיט האחרון: 2026-08-09._
<!-- repo-chronicle:end -->
...
## גילוי הגדרות, היסטוריית דיאגרמות, CircuitikZ ותיקיות בצורה בטוחה

עמוד ההגדרות Notemd מציע חיפוש מעורף המבין את השדות, ניווט בין קטגוריות רבות, כלי עבודה מתקפלים לגילוי הגדרות, ורשימת מועדפים לכל הגדרה שנשמרת עבור Vault הנוכחי. לחצו על **★ Favorites** כדי לפתוח רשימה ייעודית; כל רשום מעביר אתכם להגדרה המתאימה וניתן למחוק אותו מבלי לעזוב את עמוד ההגדרות.

- ההיסטורייה של הדיאגרמות נשמרת בהיקף Vault, ממוינת לפי חדשות, ניתנת לחיפוש ומחולקת לעמודות של 20 רשומות. מחיקת רשום היסטוריה אינה ממחקת את הקבצים שנוצרו.
- ייצוא תצוגת מבחן של הדיאגרמה באמצעות PPI מוגדר באופן אוטומטי ל‑300, אך ניתן להשתמש בערכים של 72–600. הוא שולט רק ברסטריזציה של PNG; SVG ו‑PDF נשארים בבסיס וקטורי.
- ניתן גם לייצא את כל הוויזואלים של Mermaid, ובאופציונלי לכתוב גם את מקור Mermaid, את SVG ואת הקבצים המלווים; Mermaid עדיין זמין בתצוגות Drawnix גם כאשר הוא מושבת.
- סוג הדיאגרמה המועדף ופורמט המקור המועדף הם בחירות נפרדות.
- משתמשי מחשב יכולים לפתוח את סביבת ההרכבה המובנית של CircuitikZ, להשתמש שוב ב‑Tectonic/pdflatex של המערכת, לבחור מקומפיילר מותאם אישית, או להתקין באופן מפורש את סביבת הריצה של Tectonic שנמצאת בניהול. תצוגות מבחן, SVG, PNG וייצואים של PDF אינם דורשים LaTeX.
- בחירה מתקדמת של קבצי בצורה מאפשרת שימוש בפרופילי בחירה שנשמרו ובתצוגות של חוקים.
- ניתן ליצור תיקיית יעד לבצורה שחסרה לאחר אישור, עם אפשרות לזכור את היצירה האוטומטית עבור תיקיות חסרות בעתיד.
- תיקייה קיימת שאינה ריקה דורשת אישור אחד לפני עיבוד כל הבצורה, ולא אישור אחד לכל קובץ שנוצר.
- מצב מפתחים מגלה אבחונים של הספק ושליטות מתקדמות לפתרון בעיות.
