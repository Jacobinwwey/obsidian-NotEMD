...
- התיעוד הרשמי נשמר באנגלית ובסינית פשוטה, עם תמיכה מלאה ביותר מ-30 שפות.
...
- **Disable auto translation**: כאשר אפשרות זו מופעלת, משימות שאינן `Translate` אינן כופות עוד פלט לשפה מסוימת, וכך נשמר הקשר השפה המקורית. משימת "Translate" המפורשת עדיין תבצע תרגום בהתאם להגדרה.
...
### תצורת ספק LLM
1.  **Active Provider**: בחרו את ספק ה-LLM שבו תרצו להשתמש מתוך הרשימה הנפתחת.
2.  **Provider Settings**: הגדירו את ההגדרות הספציפיות עבור הספק שנבחר:
...
3.  **Test Connection**: השתמשו בכפתור "Test Connection" עבור הספק הפעיל כדי לאמת את ההגדרות. ספקי OpenAI-compatible משתמשים כעת בבדיקות מודעות-ספק: endpoints כגון `Qwen`, `Qwen Code`, `Doubao`, `Moonshot`, `GLM`, `Z AI`, `MiniMax`, `Huawei Cloud MaaS`, `Baidu Qianfan`, `SiliconFlow`, `Groq`, `Together`, `Fireworks`, `LMStudio` ו-`OpenAI Compatible` בודקים `chat/completions` ישירות, בעוד ספקים בעלי `/models` אמין עדיין יכולים להתחיל ברשימת מודלים. אם הבדיקה הראשונה נכשלת עקב ניתוק רשת זמני כגון `ERR_CONNECTION_CLOSED`, Notemd עובר אוטומטית לרצף ה-retry היציב במקום להיכשל מיד.
4.  **Manage Provider Configurations**: השתמשו בכפתורי "Export Providers" ו-"Import Providers" כדי לשמור/לטעון את הגדרות הספקים שלכם אל/מ־`notemd-providers.json` בתוך תיקיית התצורה של התוסף. זה מאפשר גיבוי ושיתוף קלים.
5.  **Preset Coverage**: בנוסף לספקים המקוריים, Notemd כולל כעת presets עבור `Qwen`, `Qwen Code`, `Doubao`, `Moonshot`, `GLM`, `Z AI`, `MiniMax`, `Huawei Cloud MaaS`, `Baidu Qianfan`, `SiliconFlow`, `xAI`, `Groq`, `Together`, `Fireworks`, `Requesty`, וכן יעד כללי `OpenAI Compatible` עבור LiteLLM, vLLM, Perplexity, Vercel AI Gateway או proxies מותאמים.
...
-   **Use Different Providers for Tasks**:
    *   **Disabled (Default)**: משתמש בספק "Active Provider" יחיד לכל המשימות.
    *   **Enabled**: מאפשר לבחור ספק שונה *וגם* להחליף אופציונלית את שם המודל עבור כל משימה ("Add Links", "Research & Summarize", "Generate from Title", "Translate", "Extract Concepts"). אם שדה ה-model override נשאר ריק, ייעשה שימוש במודל ברירת המחדל של הספק שנבחר עבור המשימה.
-   **Select different languages for different tasks**:
    *   **Disabled (Default)**: משתמש בשפה יחידה של "Output language" לכל המשימות.
    *   **Enabled**: מאפשר לבחור שפה שונה לכל משימה ("Add Links", "Research & Summarize", "Generate from Title", "Summarise as Mermaid diagram", "Extract Concepts").
...
### ארכיטקטורת שפה (UI Locale לעומת Task Output Language)

-   **UI Locale** שולט רק בטקסט הממשק של התוסף (תוויות Settings, כפתורי Sidebar, notices ו-dialogs). מצב ברירת המחדל `auto` עוקב אחר שפת ה-UI הנוכחית של Obsidian.
-   **Task Output Language** שולט בפלט משימות שנוצר על ידי המודל (קישורים, סיכומים, יצירת כותרות, סיכום Mermaid, חילוץ מושגים, שפת יעד לתרגום).
-   **Per-task language mode** מאפשר לכל משימה לפתור את שפת הפלט שלה מתוך שכבת מדיניות מאוחדת במקום overrides מפוזרים בכל מודול.
-   **Disable auto translation** משאיר משימות שאינן `Translate` בהקשר שפת המקור, בעוד שמשימות `Translate` מפורשות עדיין אוכפות את שפת היעד שהוגדרה.
...
-   **Enable Stable API Calls (Retry Logic)**:
    *   **Disabled (Default)**: כשל של קריאת API אחת יעצור את המשימה הנוכחית.
    *   **Enabled**: מנסה שוב אוטומטית קריאות LLM API שנכשלו (שימושי לבעיות רשת זמניות או rate limits).
...
-   **API Error Debugging Mode**:
    *   **Disabled (Default)**: משתמש בדיווח שגיאות סטנדרטי ותמציתי.
    *   **Enabled**: מפעיל רישום שגיאות מפורט (בדומה ל-verbose output של DeepSeek) עבור כל הספקים והמשימות (כולל Translate, Search ו-Connection Tests). זה כולל HTTP status codes, raw response text, request transport timelines, request URLs מנוקים ו-headers, משכי ניסיונות, response headers, partial response bodies, parsed partial stream output ועקבות מחסנית — נתונים חיוניים לפתרון תקלות חיבור API ואיפוסי upstream gateway.
-   **Developer Mode**:
    *   **Disabled (Default)**: מסתיר את כל פקדי האבחון המיועדים למפתחים ממשתמשים רגילים.
    *   **Enabled**: מציג פאנל diagnostics ייעודי למפתחים ב-Settings.
...
-   **Customize Processed File Save Path**:
    *   **Disabled (Default)**: קבצים מעובדים (למשל `YourNote_processed.md`) נשמרים ב-*אותה תיקייה* כמו ההערה המקורית.
    *   **Enabled**: מאפשר להגדיר מיקום שמירה מותאם.
...
-   **Use Custom Output Filename for 'Add Links'**:
    *   **Disabled (Default)**: קבצים מעובדים שנוצרים על ידי פקודת 'Add Links' משתמשים ב-suffix ברירת המחדל `_processed.md` (למשל `YourNote_processed.md`).
    *   **Enabled**: מאפשר להגדיר שם פלט מותאם באמצעות ההגדרה שמתחת.
...
-   **Remove Code Fences on Add Links**:
    *   **Disabled (Default)**: code fences **(\`\\\`\`)** נשמרים בתוכן בעת הוספת קישורים, ואילו **(\`\\\`markdown)** יוסר אוטומטית.
    *   **Enabled**: מסיר code fences מהתוכן לפני הוספת קישורים.
...
-   **Customize Concept Note Path**:
    *   **Disabled (Default)**: יצירה אוטומטית של הערות עבור `[[linked concepts]]` מושבתת.
    *   **Enabled**: מאפשר לציין תיקייה שבה ייווצרו concept notes חדשים.
...
-   **Generate Concept Log File**:
    *   **Disabled (Default)**: לא נוצר קובץ יומן.
    *   **Enabled**: יוצר קובץ יומן שמפרט concept notes חדשים לאחר העיבוד. הפורמט הוא:
        ```
...
-   **Customize Log File Save Path**: (גלוי רק כאשר "Generate Concept Log File" מופעל)
    *   **Disabled (Default)**: קובץ היומן נשמר ב-**Concept Note Folder Path** (אם הוגדר) או בשורש ה-vault אחרת.
    *   **Enabled**: מאפשר להגדיר תיקייה מותאמת לשמירת היומן.
...
-   **Customize Log File Name**: (גלוי רק כאשר "Generate Concept Log File" מופעל)
    *   **Disabled (Default)**: שם קובץ היומן הוא `Generate.log`.
    *   **Enabled**: מאפשר להגדיר שם קובץ מותאם.
...
-   **Enable Batch Parallelism**:
    *   **Disabled (Default)**: משימות עיבוד באצווה (כגון "Process Folder" או "Batch Generate from Titles") מעבדות קבצים אחד-אחד (serially).
    *   **Enabled**: מאפשר לתוסף לעבד מספר קבצים במקביל, דבר שיכול להאיץ משמעותית עבודות אצווה גדולות.
...
-   **Enable Duplicate Detection**: מפעיל/מכבה את הבדיקה הבסיסית של מילים כפולות בתוכן המעובד (התוצאות נרשמות לקונסול). (ברירת מחדל: Enabled)
...
-   **Customise Translation File Save Path**:
    *   **Disabled (Default)**: קבצים מתורגמים נשמרים ב-*אותה תיקייה* כמו ההערה המקורית.
    *   **Enabled**: מאפשר להגדיר *נתיב יחסי* בתוך ה-vault (למשל `Translations`) שבו יישמרו קבצים מתורגמים. תיקיות ייווצרו אם אינן קיימות.
-   **Use custom suffix for translated files**:
    *   **Disabled (Default)**: קבצים מתורגמים משתמשים ב-suffix ברירת המחדל `_translated.md` (למשל `YourNote_translated.md`).
    *   **Enabled**: מאפשר להגדיר suffix מותאם.
...
-   **Enable Research in "Generate from Title"**:
    *   **Disabled (Default)**: "Generate from Title" משתמש רק בכותרת כקלט.
    *   **Enabled**: מבצע מחקר אינטרנט באמצעות **Web Research Provider** שהוגדר ומוסיף את הממצאים כהקשר ל-LLM בעת יצירה המבוססת על כותרת.
-   **Auto-run Mermaid Syntax Fix after Generation**:
    *   **Enabled (Default)**: מריץ אוטומטית שלב תיקון תחביר Mermaid לאחר זרימות עבודה הקשורות ל-Mermaid, כמו Process, Generate from Title, Batch Generate from Titles, Research & Summarize, Summarise as Mermaid ו-Translate.
    *   **Disabled**: משאיר את פלט ה-Mermaid שנוצר ללא שינוי, אלא אם כן מריצים ידנית `Batch Mermaid Fix` או מוסיפים אותו לזרימת עבודה מותאמת.
...
-   **Use Custom Output Folder for 'Generate from Title'**:
    *   **Disabled (Default)**: קבצים שנוצרו בהצלחה מועברים לתת-תיקייה בשם `[OriginalFolderName]_complete` יחסית לתיקיית האב של התיקייה המקורית (או `Vault_complete` אם התיקייה המקורית הייתה השורש).
    *   **Enabled**: מאפשר להגדיר שם מותאם לתת-התיקייה שאליה יועברו הקבצים המושלמים.
...
-   **Enable Custom Prompts for Specific Tasks**:
    *   **Disabled (Default)**: התוסף משתמש ב-prompts המובנים שלו לכל הפעולות.
    *   **Enabled**: מפעיל את היכולת להגדיר prompts מותאמים למשימות המפורטות להלן. זהו מתג העל של התכונה.
...
-   **Use Custom Prompt for [Task Name]**: (גלוי רק כאשר האפשרות לעיל מופעלת)
    ...
    *   **Enabled**: המשימה תשתמש בטקסט שתספקו בשדה "Custom Prompt" המתאים בהמשך.
...
-   **Enable Focused Learning Domain**:
    *   **Disabled (Default)**: prompts שנשלחים ל-LLM משתמשים בהוראות כלליות סטנדרטיות.
    *   **Enabled**: מאפשר להגדיר תחום או תחומים לימודיים לשיפור ההבנה ההקשרית של ה-LLM.
...
-   השתמשו באזור **Quick Workflows** בראש סרגל הצד כדי להפעיל כפתורים מותאמים מרובי-שלבים.
...
-   **בעיות חיבור של LM Studio / Ollama**:
    *   **Test Connection Fails**: ודאו שהשרת המקומי (LM Studio או Ollama) פועל ושהמודל הנכון נטען / זמין.
...
*Notemd v1.8.0 - שפרו את גרף הידע שלכם ב-Obsidian בעזרת AI.*
