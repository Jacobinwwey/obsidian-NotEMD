...
## גילוי הגדרות, היסטוריית תרשימים ותיקיות בצורה בטוחה

דף ההגדרות של Notemd מציע חיפוש עמום, ניווט בין קטגוריות רבות, ואפשרות לשמור העדפות לכל הגדרה ספציפית עבור ה‑Vault הנוכחי.

- ההיסטורייה של התרשימים נשמרת ברמת ה‑Vault, ממוינת לפי תאריך – החדשים ביותר קודם, ניתן לחפש בה ולעבור בין עמודות של 20 רשומות. מחיקת רשומת היסטוריה אינה ממחקת את הקבצים שנוצרו.
- יצוא תצוגת מבחן של התרשים מאפשר שליטה ברמת הדיוק PPI של ה‑PNG וה‑PDF; ה‑SVG נשאר בבסיס וקטורי.
