...
**Created:** 2026-06-12
**Updated:** 2026-06-22
**Status:** Phase 1-4 shipped. Phase 5 source-side language ownership, zh-CN critical-path promotion, provider-page quality gate, and measurement evidence log are now the active completion slice. Live Search Console and AI visibility checks remain external post-deploy work.
...
4. **Use `llms.txt` as the compact answer-engine map.** It should point to canonical docs, state partial zh-CN coverage, and reject generated exports or stale issue text as primary sources.
5. **Prefer fewer provider pages with real operational content.** Thin pages dilute answer quality. Provider docs need setup, endpoint/auth, model discovery, troubleshooting, and use boundaries.
6. **Gate the built output, not only the source.** Docusaurus locale behavior mutates the final route graph. `npm run audit:build` must pass before Pages upload.
7. **Separate local proof from external proof.** Local build/audit can prove route truth; Search Console indexing and AI visibility require live post-deploy measurement.
