...
Current shipped scope is fifteen semantic types, eight render targets, and three export formats. Radar is a dedicated Vega-Lite intent with a bounded `radarSpec`; org chart is a dedicated bounded ownership intent with Mermaid and HTML targets. Neither is a `dataChart` chart-type alias. Timeline, swimlane, and quadrant are shipped Mermaid-only types; no editable HTML/SVG, Draw.io, Drawnix, or external-consumer capability is claimed for them. The settings page and generation workbench now consume one capability gallery: shipped types show production-renderer fixtures, mapped types also show pinned `diagram-design` reference screenshots, and reference-only layouts show static previews without entering the generation selector. `scripts/generate-diagram-gallery.js` continues to produce deterministic SVG/PNG assets and a hashed manifest for the bilingual docs gallery. Remaining reference layouts from `ref/diagram-design` stay `reference-only/planned` until a renderer, fixture, preview, persistence mapping, docs row, and automated gate exist.
...
The capability manifest is a separate three-axis projection: `src/diagram/diagramCapabilityManifest.ts` joins semantic type, default/compatible targets, fixture ownership, and pinned reference-preview descriptors, while the target descriptor owns artifact mechanics. `src/ui/diagramCapabilityGallery.ts` is the read-model consumer for both Settings and the generation workbench; reference-only rows are deliberately preview-only. `scripts/diagram-gallery-browser-entry.ts` imports the executable fixture catalog and production renderers; `scripts/generate-diagram-gallery.js` renders accessible SVG, rasterizes PNG at a fixed card size, writes `docs/assets/diagrams/manifest.json`, and fails closed on stale or invalid assets. This keeps selector previews, reference previews, docs previews, and runtime fixtures on one evidence path without turning reference screenshots into generation contracts.
