...
flowchart LR
    ...
    subgraph Target["Output Targets"]
        ...
        VEGA["Vega-Lite<br/>(dataChart, radar)"]
        HTML["HTML Fallback"]
...
| `radar` | vega-lite | VegaLiteRenderer | modal/iframe (sandboxed) | source `.json` / Vault `.md`, SVG, PNG, PDF; HTML table fallback |
...
Relation-label wrapping and measured width/height now come from `drawnixRelationLabelLayout.ts`; lane reservation and native label metadata therefore cannot silently diverge on line breaks or line height. Collision policy remains in the projection because it depends on protected header and occupied-label state, while routing remains responsible only for obstacle-safe ingress.
...
Current shipped scope is thirty-three executable catalog types, eight render targets, and three export formats. Radar and the explicit bar/line/scatter variants use the Vega-Lite target; topology, lane-grid, access-matrix, schedule, ordered-stack, set-overlap, ranked-segments, cycle, nested, and tree payload families use the deterministic editable HTML/SVG target with HTML fallback. Mermaid-only timeline, swimlane, and quadrant behavior remains unchanged. The settings page and generation workbench expose all catalog IDs, including independent quantitative variants, and one dynamic preview panel. The stable `preferredDiagramTypeId` setting projects to the legacy semantic intent for backward compatibility; explicit variants carry through operation planning and force their declared native target. After selection, the panel calls the production renderer fixture; there is no reference-image column or static reference screenshot in the plugin UI. `scripts/generate-diagram-gallery.js` now produces thirty-three deterministic SVG/PNG assets and a hashed manifest for the bilingual docs gallery. Five reference grammars remain roadmap-only because their exact layout contract is not yet claimed.

The delivered ordering was correctness foundation, catalog/contract generation, deterministic preview assets, then selector/docs integration. Mermaid normalization, legacy repair staging, family gating, fence ownership, validation-runtime initialization, shared Drawnix geometry, and Circuitikz template convergence are now landed. Remaining work is deliberately narrower: external Draw.io and real Drawnix application evidence, plus an explicitly authorized Circuitikz repair command if product scope later requires one. The standalone Plait consumer gate is available as `npm run diagram:consumer:drawnix`. See [the current progress audit](./brainstorms/2026-08-16-mainline-diagram-architecture-progress-and-next-direction.md), [the capability catalog](./maintainer/diagram-capability-catalog.md), [the gallery](./diagram-gallery.md), and [the forward architecture plan](./superpowers/plans/2026-08-16-diagram-capability-catalog-and-forward-architecture.en.md).
...
The quadrant repair is intentionally grammar-scoped. A provider can copy the opening coordinate bracket into a point label (`"Docs gallery[": [0.32, 0.68]`); `repairQuadrantPointLabels()` removes that bracket only when the complete line matches the quadrant point grammar. Flowchart quote/shape stages are skipped for a detected `quadrantChart`, and titles, axes, and quadrant names continue to use the ordinary text sanitizer so intentional punctuation is preserved.
...
The capability manifest is a separate three-axis projection: `src/diagram/diagramCapabilityManifest.ts` joins semantic type, default/compatible targets, fixture ownership, and reference-only roadmap metadata, while the target descriptor owns artifact mechanics. `src/ui/diagramTypePreviewPanel.ts` is the shared UI read-model boundary for Settings and the generation workbench; it never resolves reference assets. `scripts/diagram-gallery-browser-entry.ts` imports the executable fixture catalog and production renderers; `scripts/generate-diagram-gallery.js` renders accessible SVG, rasterizes PNG at a fixed card size, writes `docs/assets/diagrams/manifest.json`, and fails closed on stale or invalid assets. This keeps selector previews and docs previews on the production-fixture evidence path; development-only taxonomy files never become generation contracts.
...
The final presentation boundary is shared by selector previews, the bundled render host, and the production gallery. After an SVG is mounted, `src/rendering/preview/svgSafety.ts` measures visible text with browser geometry, rejects text/text collisions, and rejects later-painted shapes that cover a label. This is deliberately a post-mount gate: serialization-time checks cannot see font fallback, CSS scaling, or DOM z-order. Native family renderers must therefore paint surfaces first and labels/connectors second; lane-grid, nested, and semantic-figure adapters encode this ordering and route labels against occupied geometry. `scripts/diagram-gallery-browser-entry.ts` runs the same presentation diagnostics before accepting any of the 33 generated fixtures, so gallery-green is evidence for the production mounted path rather than a separate serializer-only claim. A failed presentation gate keeps the panel in an error state and never advertises `ready`.

The lane-grid layout follows the same boundary: summary text is measured and wrapped before the lane header is placed, the header stays below the protected summary region, cells are painted before connectors and labels, and cross-lane routes reserve occupied cells plus lane-label regions. This prevents the earlier fixed-header and straight-line assumptions from producing text/background mismatches in dense real-vault diagrams.
