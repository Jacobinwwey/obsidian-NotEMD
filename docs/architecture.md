...
| `drawnix` | `.drawnix` JSON subset with inline Mermaid source visuals; SVG/media companions only for the primary preview or unresolved/non-inline media | no Drawnix or Plait runtime in the plugin; regeneration removes only manifest-backed plugin-owned Mermaid companion scopes, while node-aware relation routing, sparse-grid fallback, and layered label boxes keep labels out of node rectangles; explicit Drawnix failures do not masquerade as Mermaid payloads |
