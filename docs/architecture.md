...
Current shipped scope is fifteen semantic types, eight render targets, and three export formats. Radar is a dedicated Vega-Lite intent with a bounded `radarSpec`; org chart is a dedicated bounded ownership intent with Mermaid and HTML targets. Neither is a `dataChart` chart-type alias. Timeline, swimlane, and quadrant are shipped Mermaid-only types; no editable HTML/SVG, Draw.io, Drawnix, or external-consumer capability is claimed for them. The settings page and generation workbench expose the same executable type selector and one dynamic preview panel. After selection, the panel calls the production renderer fixture; there is no reference-image column or static reference screenshot in the plugin UI. `scripts/generate-diagram-gallery.js` continues to produce deterministic SVG/PNG assets and a hashed manifest for the bilingual docs gallery. Remaining reference layouts from `ref/diagram-design` stay `reference-only/planned` until a renderer, fixture, preview, persistence mapping, docs row, and automated gate exist.
