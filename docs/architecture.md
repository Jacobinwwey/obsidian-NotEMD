...
- `src/main.ts` now mainly retains command registration and remaining direct execution surfaces; the registry already covers diagram/provider/config-profile plus process/generate/research/translation/extraction/utility/selection/export batches, so the next real gap is the remaining `src/fileUtils.ts` tail (`mermaid.batch-fix` plus `concept.dedupe`) plus the remaining direct-read/sidebar surfaces rather than the already-landed process/generate flows
...
- The ordered convergence path is now explicit: finish the remaining `src/fileUtils.ts` tail first, then finish the remaining direct-read/sidebar surfaces, and only then reopen packaging/semantic-verification convergence work
