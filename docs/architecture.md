...
- The next real gap is no longer the public command entrypoints themselves: typed contracts already exist for `diagram.preview` and `provider.connection.test`, and the substantive save/artifact execution path now lives in `src/operations/diagramCommandExecution.ts`. The remaining pressure point is whether the internal save/artifact branches beneath `diagram.generate` deserve further typed boundaries beyond the now-landed `outputPath` / `previewOpened` result fields.
