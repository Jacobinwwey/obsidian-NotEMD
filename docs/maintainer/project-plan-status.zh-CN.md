...
[1.9.8 发布／文档／可发现性方案](../plans/2026-09-13-001-feat-1-9-8-release-docs-geo-plan.zh-CN.md)以 `291ea45` 为调研基线，完整覆盖 1.9.7 tag 之后的 14 个提交。调研发现发布预检缺口、入门指南默认值错误、旧翻译指令和开发者／Agent 入口不足。V0–V6 分别定义发布完整性、源文档纠正、受众指导、Codex 直接本地化、GEO／治理、候选验证和发布。实施已启动：V0 发布防护和本地验证已完成（282 个套件、2633 项通过、一项平台条件跳过），Linux 候选验证仍待执行；V1 源文档纠正进行中，1.9.8 尚未发布；上面的 19 份正式计划清单继续作为历史清单，已完成 U1–U8 不重新扩张为无边界重构。
