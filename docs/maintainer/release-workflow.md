...
Release trigger/tag-guard parsing now tolerates mixed tag quote styles in workflow YAML (`'*.*.*'`, `"*.*.*"`, or unquoted), supports inline push syntax such as `push: { tags: ["*.*.*"] }` plus top-level inline `on` object forms such as `on: { push: { tags: ["*.*.*"] }, workflow_dispatch: {} }`, detects `workflow_dispatch` in `on` sequence forms (`- workflow_dispatch` and `- workflow_dispatch: {}`) and inline event arrays (`on: [push, workflow_dispatch]`), parses `on` sequence push-mapping forms (`- push:` with nested `tags`) for tag-trigger truth, and accepts quoted YAML keys (for example `'push':`, `"tags":`, `'workflow_dispatch':`) while scoping event-key detection to top-level `on` mappings so nested non-event keys (for example `workflow_call.inputs.workflow_dispatch`) do not produce false positives, still scoping tag trigger detection to `on.push.tags` (ignoring unrelated `tags:` keys elsewhere), and explicitly detecting `v*.*.*` wildcard patterns as contract violations.
...
The release helper itself now enforces this ownership guardrail as executable contract truth: if required release assets omit `main.js`, it fails fast and blocks `outfile -> outdir` migration promotion until a replacement ownership contract is explicitly delivered with tests/workflow/docs.
...
- The follow-up chronicle job runs `node scripts/repo-saga/update-quarterly-saga.mjs --tag "$TAG_NAME"` on `main`, then delegates commit/push handling to `node scripts/release/commit-chronicle-refresh.js "$TAG_NAME"`.
- That checked-in helper treats tracked and untracked chronicle artifacts as change candidates, stages only the expected chronicle paths, and exits cleanly when nothing remains to commit.
- The same helper now hardens the `main` push path: after a failed push it fetches `origin/main`, treats “remote already contains the chronicle commit” as success, rebases onto the refreshed remote tip when needed, and retries with backoff before failing the job.
