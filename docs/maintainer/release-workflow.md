...
Release trigger/tag-guard parsing now tolerates mixed tag quote styles in workflow YAML (`'*.*.*'`, `"*.*.*"`, or unquoted) and explicitly detects `v*.*.*` wildcard patterns as contract violations.
