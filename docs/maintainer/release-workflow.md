...
```
The helper reads packaging entry/output facts from `esbuild.config.mjs`; keep that file as the packaging truth source when evaluating renderer-boundary claims.
