...
Release trigger/tag-guard parsing now tolerates mixed tag quote styles in workflow YAML (`'*.*.*'`, `"*.*.*"`, or unquoted), scopes tag trigger detection to `on.push.tags` (ignoring unrelated `tags:` keys elsewhere), and explicitly detects `v*.*.*` wildcard patterns as contract violations.
