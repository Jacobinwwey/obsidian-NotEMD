...
packaging-contract 区块现在还会记录 release-helper 运行时归属 guard（`validateRequiredReleaseAssets`）是否被成功验证；若 guard 检查不完整，应将迁移提升声明视为阻断状态，直到 helper/运行时契约检查恢复。
运行时 guard 检测现在优先使用结构化契约（`RELEASE_ASSET_OWNERSHIP_GUARD_CODE` / `isReleaseAssetOwnershipGuardError`），仅将旧报错文案匹配作为兼容兜底，避免“只改文案”导致 guard 核验静默失效。
