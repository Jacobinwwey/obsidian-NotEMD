...
## Active Release Program: 1.9.8

The [1.9.8 release/documentation/discoverability plan](../plans/2026-09-13-001-feat-1-9-8-release-docs-geo-plan.en.md) uses `291ea45` as its research baseline and accounts for all 14 commits since the 1.9.7 tag. It identifies release preflight gaps, incorrect onboarding defaults, legacy translation instructions and missing developer/Agent entry points. V0–V6 define release integrity, source correction, audience guidance, direct Codex localization, GEO/governance, candidate verification and publication. Execution is active: V0 publication safeguards and local verification are implemented (282 suites, 2633 passing tests, one platform-specific skip); Linux candidate verification is still pending. V1 source correction is underway. 1.9.8 is not yet published. The historical 19-plan inventory above remains a historical inventory, and completed U1–U8 work is not reopened as an unbounded refactor.
