...
- `content.split-note-by-chapters` results now also expose `requestedSplitHeadingLevel`, `chapterNotePaths`, `managedArtifactPaths`, and `removedStalePaths` so automation can reason about the managed artifact set without re-deriving it from filenames
