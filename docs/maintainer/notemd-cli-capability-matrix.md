...
- `content.split-note-by-chapters` results now also expose `requestedSplitHeadingLevel`, `chapterNotePaths`, `managedArtifactPaths`, `removedStalePaths`, and stable `nestedHeadings[].blockId` values so automation can reason about the managed artifact set and repeated-heading-safe TOC targets without re-deriving them from filenames or ambiguous heading text
