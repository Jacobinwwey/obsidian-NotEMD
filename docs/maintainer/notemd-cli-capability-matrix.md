...
|---|---|---|---|
| P0 | Packaging / semantic-verification convergence | Public provider-test and diagram command wrappers now delegate through host adapters, typed contracts already exist for `provider.connection.test` plus `diagram.preview`, and `diagram.generate` now has both a host-neutral generation core and explicit typed follow-through beneath it. The next highest-leverage work is therefore packaging isolation, semantic verification, and any later contract promotion decisions rather than another immediate diagram operation split | `docs/maintainer/*`, `src/operations/diagramGenerateOperation.ts`, `src/operations/diagramCommandHostAdapter.ts`, `src/operations/diagramCommandExecution.ts` |
