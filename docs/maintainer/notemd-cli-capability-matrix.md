...
|---|---|---|---|
| P0 | Diagram/provider command-core convergence | Public provider-test and diagram command wrappers now delegate through host adapters, but `src/main.ts` still owns deeper `executeSaveMermaidDiagramCommand` / `executeArtifactDiagramCommand` helpers and the typed-contract decision for `diagram.preview` plus provider connection-test automation exposure | `src/operations/diagramCommandHostAdapter.ts`, `src/operations/providerConnectionTestCommandHostAdapter.ts`, `src/main.ts` diagram helpers |
