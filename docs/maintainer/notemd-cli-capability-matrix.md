...
- `translate.*`, `formula.*`, and `content.extract-original-text` now form the first proven write-heavy contract-enrichment pattern: utility cores return structured results, host adapters own localized success notices, and the registry exports the richer schemas directly.
- The next contract-deepening order is now explicit: `src/fileUtils.ts` write-heavy flows first, the remaining direct-read/sidebar surfaces second, and packaging/semantic-verification convergence last.
...
|---|---|---|---|
| P0 | `src/fileUtils.ts` write-heavy flow tightening | Largest remaining side-effect core; needs explicit created/overwritten/moved/error semantics for processed files, generated content, Mermaid repair, and duplicate cleanup | `src/fileUtils.ts`, `src/extractOriginalText.ts` |
