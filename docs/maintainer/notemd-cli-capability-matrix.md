...
|---|---|---|---|
| P0 | Diagram/provider command-core convergence | Public provider-test and diagram command wrappers now delegate through host adapters, and typed contracts already exist for `provider.connection.test` plus `diagram.preview`; the remaining gap is the deeper `executeSaveMermaidDiagramCommand` / `executeArtifactDiagramCommand` core and any richer save/artifact branch contract depth below those wrappers | `src/operations/diagramCommandHostAdapter.ts`, `src/operations/providerConnectionTestCommandHostAdapter.ts`, `src/main.ts` diagram helpers |
