...
- `src/diagram/adapters/circuitikz/circuitikzRepairBrief.ts` and `--repair-brief-output` produce schema `notemd.circuitikz.repair-brief.v1`, a topology-preserving repair handoff that carries the source `CircuitSpec`, topology signature, compile/render diagnostics, allowed changes, prohibited topology changes, and next verification steps.
