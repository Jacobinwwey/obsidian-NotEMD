...
- 现在也已有专用的离线 retrieval-quality fixture：`npm run verify:local-kb-fixtures`。它直接复用线上 MiniSearch runtime path 跑一组小型维护者夹具，而不是再造一条只给评测用的分叉 retriever；当前 Stage-C 收口还把 task-scoped 的 batch-title / research inspect case、exact-file-vs-folder retrieval 边界，包含重复/空白 override path、非 Markdown 干扰文件、无关文件夹与空 searchable section 候选的 noisy mixed-corpus scope，以及 real-note-style 的 chapter-split showcase retrieval 一并纳入，而不再只覆盖单个 diagram 示例。
