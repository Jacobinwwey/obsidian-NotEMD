...
release 触发与 tag 防护解析现在可容忍 workflow YAML 中 tags 列表的混合引号风格（`'*.*.*'`、`"*.*.*"` 或不加引号），支持 `push: { tags: ["*.*.*"] }` 这类内联写法，也支持 `on: { push: { tags: ["*.*.*"] }, workflow_dispatch: {} }` 这类顶层内联 `on` 对象写法，并且只把 `on.push.tags` 作为有效 tag 触发来源（忽略其他位置的 `tags:` 字段），同时会显式识别 `v*.*.*` 通配模式为契约违规。
