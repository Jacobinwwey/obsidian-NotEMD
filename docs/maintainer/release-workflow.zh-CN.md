...
```
该 helper 会从 `esbuild.config.mjs` 提取当前打包入口/输出事实，从 `scripts/release/publish-github-release.js` 提取 release 打包契约事实，并从 `.github/workflows/release.yml` 提取 release 触发与 tag 防护契约事实；评估 renderer 边界声明时，应以这些文件作为打包真值源。
