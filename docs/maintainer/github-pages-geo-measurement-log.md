...
| Evidence | Current state | Owner |
|---|---|---|
| Language policy | The previous partial zh-CN fallback boundary is superseded by full docs route publication for every README/UI locale declared in `website/src/lib/publishedLocales.mjs`; `en` and `zh-CN` are verified/indexable, while the other route-complete locales are explicitly machine-translated, reachable for review, `noindex,follow`, and omitted from sitemaps | `website/src/lib/publishedLocales.mjs`, `website/src/lib/localePublication.mjs`, `website/docusaurus.config.js`, `website/README.md` |
...
| Build audit | `npm run audit:build` must verify localized source coverage, localized build output, verified sitemap entries, `llms.txt` entry points, and the intentional `noindex,follow` signal on machine-translated docs | `website/scripts/audit-build.cjs` |
| Answer-engine map | `llms.txt` now states that English remains canonical, English and Simplified Chinese are verified/indexable, and other complete route locales remain machine-translated for review | `website/static/llms.txt` |
...
## 2026-08-10 Localization Pipeline Hardening

| Evidence | Result |
|---|---|
| LM Studio endpoint | `http://100.80.17.113:301/v1/chat/completions`, model `hy-mt2-7b`, verified with successful structured translation requests |
| Batch safety | FAQ, UI, and homepage translations were executed in bounded batches (at most eight locales; estimated summed context below 30,000 tokens, under the model's 32k window) |
| Translation integrity | Stable request identity, protected technical tokens, exact JSON/array shape, heading/code-fence checks, retry-on-truncation, and atomic writes passed |
| Incremental update | Homepage `languageBoundary` was retranslated through `home-boundary` without retranslating the full homepage object; no partial catalog was written after the intentionally timed-out full-page attempt |
| Build | Docusaurus static output generated for English plus all 33 localized route locales with `--no-minify`; NUL cleanup removed 1,628 generated bytes from 405 HTML files |
| Audit | `website/scripts/audit-build.cjs` passed after fixing CRLF-safe YAML FAQ parsing and limiting sitemap requirements to indexable locales |
| Indexability policy | `en` and `zh-CN` are verified/indexable; the other 32 locale routes are reachable, labeled machine-translated, `noindex,follow`, and absent from sitemap output |
| External measurement | Search Console and AI visibility remain post-deploy checks; this local run does not claim remote indexing |
