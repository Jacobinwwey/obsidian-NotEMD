...
Its packaging-contract section tracks release-asset, release-tag, publish-mode, and release-notes contract truth from `scripts/release/publish-github-release.js`, and release-trigger/tag-guard/workflow-branch/chronicle-target contract truth from `.github/workflows/release.yml`, so Stage-B contract definition stays aligned with release enforcement.
