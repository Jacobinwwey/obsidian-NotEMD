...
Its packaging-contract section tracks release-asset, release-tag, publish-mode, and release-notes contract truth from `scripts/release/publish-github-release.js`, and release-trigger/tag-guard/workflow-branch/chronicle-target contract truth from `.github/workflows/release.yml`, so Stage-B contract definition stays aligned with release enforcement. The release workflow tag-trigger glob and disallowed `v`-prefixed globs are owned by `RELEASE_WORKFLOW_TAG_TRIGGER_GLOB` and `RELEASE_WORKFLOW_DISALLOWED_TAG_TRIGGER_GLOBS` in `scripts/lib/packaging-contract.js`; the YAML still carries the bootstrap literal because GitHub Actions parses tag triggers before repository JavaScript can be checked out, and repo-side tests lock that literal against the shared contract.
