...
The packaging-contract section now also records whether the release-helper runtime ownership guard (`validateRequiredReleaseAssets`) is verifiably active; if guard inspection is incomplete, treat migration promotion claims as blocked until helper/runtime contract checks are restored.
Runtime guard detection now prefers structured guard contracts (`RELEASE_ASSET_OWNERSHIP_GUARD_CODE` / `isReleaseAssetOwnershipGuardError`) and only uses legacy error-text matching as a compatibility fallback, so wording-only refactors do not silently weaken guard verification.
For implementation-readiness planning, any `outfile -> outdir` migration candidate now also requires an explicit `audit:render-host` contract delta definition before build-topology edits are treated as promotable.
