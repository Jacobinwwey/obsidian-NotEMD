...
Its packaging boundary line is derived from current `entryPoints` / `outfile` values in `esbuild.config.mjs`; if parsing fails, the helper emits explicit placeholder wording so boundary drift is visible.
