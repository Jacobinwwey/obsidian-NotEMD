...
The result is recorded as `compileExecution.renderSmoke`. Missing or empty artifacts add `render-artifact-missing` or `render-artifact-empty`; SVG structure failures add diagnostics such as `render-svg-invalid`, `render-svg-dimension-missing`, `render-svg-no-visible-elements`, `render-svg-text-missing`, `render-svg-text-path-only`, `render-svg-out-of-bounds`, `render-svg-text-overlap`, or `render-svg-label-overlap`; PNG screenshot failures add diagnostics such as `render-png-blank`, `render-png-content-clipped`, or `render-png-foreground-dense`.

The SVG bounded-canvas, path-only label classification, text-overlap, and label-vs-drawing overlap checks are intentionally conservative. They parse common SVG coordinates in `path`, `line`, `rect`, `circle`, `ellipse`, and positioned `text` elements, then compose common group and element transforms before checking boxes. Label-vs-drawing overlap uses a small stroke tolerance around drawing boxes so thin wires can still be treated as potential label-legibility failures. Path-only label classification detects reusable glyph paths, not the text value encoded by those glyph shapes. These checks catch obvious fixture failures before screenshot review, but they do not replace OCR, precise pixel-level overlap detection, or final image-based visual inspection.
...
- bounded SVG viewBox, obvious text-overlap, and label-vs-drawing overlap checks, including transform-aware geometry for common SVG transforms;
