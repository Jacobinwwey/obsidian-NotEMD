...
- bounded SVG viewBox, arc path endpoint geometry for A/a arc endpoints, Bezier path endpoint geometry for C/S/Q/T curve endpoints, obvious text-overlap, positioned `tspan` label overlap, path-only glyph label overlap, and label-vs-drawing overlap checks, including transform-aware geometry for common SVG transforms;
