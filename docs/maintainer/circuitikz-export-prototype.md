...
The SVG bounded-canvas and text-overlap checks are intentionally conservative. They parse common SVG coordinates in `path`, `line`, `rect`, `circle`, `ellipse`, and positioned `text` elements, then compose common group and element transforms before checking boxes. They catch obvious fixture failures before screenshot review, but they do not replace OCR, path-only label inspection, pixel-level overlap detection, or final image-based visual inspection.
