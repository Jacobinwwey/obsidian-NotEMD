...
2. richer custom/component-heavy Slidev layouts beyond the current supported structural set still remain conservative/manual-review paths, especially when no stable owner surface exists, when table/fence/directive syntax is mixed into a component surface, when component/prose/component ordering is unstable, or when content cannot be safely paginated. Stage 12 proves only clear-boundary mixed component/prose separation into presentation surfaces; it is not proof that arbitrary Vue component trees can be transformed safely;
