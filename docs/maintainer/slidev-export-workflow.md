...
7. component-heavy custom slot layouts can now fall back to local `<Transform :scale=\"...\">` wrapping for the overflowing slot zone when structural splitting is unavailable; when several component-heavy zones exist, the patcher now uses rendered text hints to attribute overflow to the right slot and avoids retargeting already-nonoverflowing sibling zones;
