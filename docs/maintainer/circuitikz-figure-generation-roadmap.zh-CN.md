...
- 六个 golden renderer 现在共享一个 standalone-document wrapper 和一个 component-label lookup helper；这去除了重复的 preamble/label plumbing，同时保持精确 fixture 输出、拓扑、voltage convention 和 layout-hint 行为不变。
