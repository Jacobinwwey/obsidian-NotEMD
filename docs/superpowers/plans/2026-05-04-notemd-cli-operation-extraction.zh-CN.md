...
- note-processing host adaptation 也已抽离一批：`src/operations/noteProcessingCommandHostAdapter.ts` 现在承接 `generate-from-title` 与 `research-and-summarize` 的 busy-guard、reporter 生命周期、notice/error-log 编排，`src/main.ts` 不再内联保留这两条命令包装。
...
- 实施状态：
  ...
  - 剩余缺口：继续收缩 `processWithNotemdCommand`、`processFolderWithNotemdCommand` 及其他批处理宿主副作用，且不能把 Obsidian 耦合重新塞回 operation 层
...
- 第 1-4 项已在主线上部分落地。
- 下一步最稳妥的推进点仍是 MT2，但范围已经继续缩小：provider diagnostic、config/profile、共享 provider test，以及 `generate-from-title` / `research-and-summarize` host adaptation 都已落地，下一批应继续把 `processWithNotemdCommand`、`processFolderWithNotemdCommand` 与其他批处理宿主副作用从 `src/main.ts` 剥离出去。
