...
- The next defensible move is still MT2, but it is now narrower: provider-diagnostic and config/profile host adaptation are already landed, so the next batch should peel broader non-CLI interactive and batch host effects away from `src/main.ts`.
