...
- [ ] **Step 1: Write failing tests** for two catalog rows sharing `dataChart`, explicit variant lookup, default lookup, and an ambiguity error from the legacy lookup.
- [ ] **Step 2: Run the focused tests** with `rtk npm.cmd test -- --runInBand src/tests/diagramTypeCatalog.test.ts src/tests/diagramCapabilityManifest.test.ts`; expect failures because intent lookup is currently one-to-one.
- [ ] **Step 3: Add `variant?: string` and `payloadKind` to the type definition**, replace the intent map with `Map<DiagramIntent, DiagramTypeDefinition[]>`, and keep all existing IDs unchanged.
- [ ] **Step 4: Add the explicit lookup functions** and make the old lookup throw `AMBIGUOUS_DIAGRAM_INTENT` when multiple variants exist.
- [ ] **Step 5: Extend manifest rows** with variant/layout/payload metadata while retaining `schemaVersion: 1` reader compatibility through an additive field policy.
- [ ] **Step 6: Run the focused tests and `rtk npm.cmd run build`**; expected result is PASS with the existing catalog count unchanged.
- [ ] **Step 7: Commit** with `git add src/diagram src/tests && git commit -m "refactor(diagrams): make catalog lookup variant aware"`.
...
- [ ] **Step 1: Add failing normalization tests** covering old `dataSeries`, `layoutHints.chartType`, timeline, swimlane, quadrant, radar, and circuit payloads.
- [ ] **Step 2: Run those tests** with `rtk npm.cmd test -- --runInBand src/tests/diagramSpecResponseParser.test.ts src/tests/diagramSpecValidation.test.ts`; expect missing canonical payload failures.
- [ ] **Step 3: Define discriminated payload interfaces** in `src/diagram/payloads/types.ts`; keep the first implementation limited to `legacy`, `quantitative`, `topology`, `lane-grid`, and `access-matrix` contracts needed by the staged roadmap.
- [ ] **Step 4: Add `schemaVersion` and optional `payload`** while preserving the legacy projection for current callers.
- [ ] **Step 5: Implement one parser boundary** that maps legacy fields to canonical payloads, normalizes numeric aliases, preserves evidence, and rejects unknown schema versions.
- [ ] **Step 6: Make `validateDiagramSpec()` validate canonical payloads first** and legacy projections only after normalization. Do not scatter null checks into renderers.
- [ ] **Step 7: Run focused tests, full build, and full Jest**; expected result is PASS with byte-identical Mermaid artifacts for existing fixtures.
- [ ] **Step 8: Commit** with `git add src/diagram src/tests && git commit -m "feat(diagrams): add versioned canonical payload boundary"`.
...
- [ ] **Step 7: Commit** with `git add src/diagram src/tests && git commit -m "refactor(diagrams): drive generation prompts from profiles"`.
...
- [ ] **Step 1: Write failing matrix tests** for target descriptors, default presentation, missing preview mode, duplicate `(target, payloadKind)` ownership, and explicit incompatible-target rejection.
- [ ] **Step 2: Run the focused tests** with `rtk npm.cmd test -- --runInBand src/tests/renderTargetCatalog.test.ts src/tests/rendererRegistry.test.ts`; expect failures because descriptor admission is incomplete.
- [ ] **Step 3: Add descriptors** for all current targets without changing their public IDs; include MIME, extension, preview kind, export formats, and fallback policy.
- [ ] **Step 4: Add registry admission** that rejects duplicate target ownership and a renderer that advertises unsupported preview behavior.
- [ ] **Step 5: Thread presentation defaults through `RenderOptions`** without changing existing artifact cache identity unless a presentation value is explicitly supplied.
- [ ] **Step 6: Run the focused tests, `rtk npm.cmd run audit:render-host`, and build**.
- [ ] **Step 7: Commit** with `git add src/diagram src/rendering src/tests && git commit -m "feat(rendering): admit targets and presentation contracts"`.
...
- [ ] **Step 1: Add failing payload tests** for zone bounds, stable node IDs, edge references, focal cap, and orthogonal routing constraints.
- [ ] **Step 2: Add the production fixture for each of the three IDs** using the same catalog-owned fixture contract; no reference screenshots are copied.
- [ ] **Step 3: Implement deterministic topology geometry** with zone layout, text wrapping, orthogonal connectors, stable SVG IDs, and light/dark token resolution.
- [ ] **Step 4: Register the native renderer and compatible targets**; do not add Mermaid, Drawio, Drawnix, or Circuitikz compatibility.
- [ ] **Step 5: Add preview/gallery tests** and update both support matrices.
- [ ] **Step 6: Run `rtk npm.cmd run diagram:gallery` followed by `rtk npm.cmd run diagram:gallery:check`, build, full Jest, docs build, and i18n audit.
- [ ] **Step 7: Append a bilingual progress entry** recording the three IDs, target claims, fixture hashes, and unavailable external consumers.
- [ ] **Step 8: Commit** with `git add src docs scripts && git commit -m "feat(diagrams): add topology layout capabilities"`.
...
- [ ] **Step 1: Write failing tests** for lane/step/cell bounds, empty-cell omission, focal handoff uniqueness, matrix role/component limits, closed permission levels, and focal-cell uniqueness.
- [ ] **Step 2: Implement validators before renderers** so renderers can trust the invariants.
- [ ] **Step 3: Implement fixed geometry** based on the reference formulas, including data-type chips and permission cell categories without accepting arbitrary renderer styles from the model.
- [ ] **Step 4: Add fixtures, production previews, gallery assets, bilingual docs, and manifest rows.**
- [ ] **Step 5: Run focused tests, gallery check, docs build, full build, full Jest, and `git diff --check`.**
- [ ] **Step 6: Append progress in both languages and commit** with `feat(diagrams): add data-flow and access-matrix layouts`.
...
- [ ] **Step 1: Add failing migration tests** proving old `data-chart` and `layoutHints.chartType` remain readable.
- [ ] **Step 2: Add chart caps and honest-data validation**; reject missing numeric data rather than inventing it.
- [ ] **Step 3: Route explicit chart variants to the existing Vega-Lite adapter** and keep `data-chart` as auto/legacy.
- [ ] **Step 4: Implement deterministic Gantt geometry** with phases, task bars, milestones, and no dependency arrows in v1.
- [ ] **Step 5: Add fixtures, previews, gallery assets, bilingual docs, and support matrix rows.
- [ ] **Step 6: Run full regression and commit** with `feat(diagrams): add quantitative variants and gantt`.
...
- [ ] **Step 1: Admit only 2–4 IDs per batch** and reject layouts whose geometry requires an unbounded graph algorithm.
- [ ] **Step 2: Implement each family with explicit budgets**: stack 4–6 layers, Venn 2–3 sets, ranked segments 4–6, loop 5–8 stations plus one hub.
- [ ] **Step 3: Keep circular loop arcs as a documented type-specific exception**; all other non-axis connectors remain orthogonal.
- [ ] **Step 4: Add production fixtures, preview/gallery evidence, bilingual docs, and progress entries.
- [ ] **Step 5: Run all gates and commit each independently** so a failed family does not block already shipped families.
...
- [ ] **Step 1: Update progress after every batch**, separating shipped, partial, reference-only, and unavailable consumer evidence.
- [ ] **Step 2: Run `rtk npm.cmd run diagram:gallery:check`, `rtk npm.cmd run docs:build`, `rtk npm.cmd run build`, `rtk npm.cmd test -- --runInBand`, `rtk npm.cmd run audit:i18n-ui`, and `rtk git diff --check`.
- [ ] **Step 3: Confirm generated assets are current and `main.js` remains ignored/generated.
- [ ] **Step 4: Run `rtk proxy git status --porcelain=v1 -b`; expected output is only `## main...origin/main`.
- [ ] **Step 5: Commit documentation/release evidence** and push to `origin/main` only after all gates pass.
