...
上面的“8 种图表意图已通过实时 API 验证”应理解为**一次本地历史验证证据**，而不是当前仓库内可持续执行的受控门槛。相关 live test 文件已在 `92d3ad3` 移出主线，因为它们依赖本地 vault 路径、真实密钥与非确定性网络调用。

同样需要明确：

- 远端 `main` 当前没有普通 push/PR CI；`.github/workflows/release.yml` 只在数字 tag push 或 `workflow_dispatch` 时运行
- `commits/main/status` 当前会返回 `pending` 且没有任何 status，同时 `main` 没有 branch protection，也没有普通分支级 required checks；但同一个 `main@09ef239` commit 已挂上成功的 release 路径 `check_suite` / `check_run`，这里真正的 CI 信号仍应看 Actions runs 与 checks
- 成功的 `1.8.3` 修复 run 曾提示 GitHub 官方的 Node 20 JavaScript-action 弃用告警，而当前 `1.8.4` 的成功 run 已证明 `actions/checkout@v6` 与 `actions/setup-node@v6` 的加固路径可用
- 运行时仍支持 8 种图表意图，但 UI 首选图表选择器当前只暴露子集；`mindmap` 与 `canvasMap` 不是当前默认暴露项
- `preview-experimental-diagram` 已针对保存后的 `vega-lite` fenced artifact 改为本地预览，因此命令表面统一仍然是“部分完成”，不是“已彻底收口”
- Drawnix 应作为数据边界 / 转换边界参考，而不是下一步要整体嵌入的宿主；详见 `docs/brainstorms/2026-05-03-drawnix-feasibility-and-integration-direction.zh-CN.md`。完整 Drawnix 宿主嵌入不属于当前路线图批次。

## 2026-05-08 后续硬化更新

- semantic verification helper 现在会从 `esbuild.config.mjs` 自动提取打包入口/输出事实；若解析失败，会输出显式占位提示。
- 回归测试已锁定 helper 输出与当前构建配置形态的一致性，避免 packaging-boundary 真值静默漂移。
- `repo-saga` 编年史刷新 fallback 现在会在执行失败时逐候选重试 package-manager 运行器（`pnpm`、`corepack pnpm`、`bun x pnpm`），以降低 CI 脆弱性。
- semantic helper 解析已进一步针对常见配置漂移加固：支持反引号字面量、显式跟踪输出目标状态（`outfile` / `outdir` / `unknown` / `ambiguous`），并优先在 `esbuild.context({...})` 选项块内解析以避免同名 decoy 字段碰撞。
- helper 还新增了 Stage B 的 packaging 契约真值检查：release 必需资产从 `scripts/release/publish-github-release.js` 同步提取，双语 release notes 文件约束也进入生成清单，同时数字 tag 与 create/upload 模式也成为显式契约检查项，并从 `.github/workflows/release.yml` 校验 tag-only workflow 触发防护。
- helper 还新增了 Stage B 契约提升边界真值检查：从 `src/operations/registry.ts` 提取 workflow/settings/export 邻近操作约束，并转化为显式清单行。
- 下一条活跃工作线保持不变：先持续维护这套真值对齐，再在“真实 heavy-runtime packaging isolation”与“后续 contract-promotion 切片”之间做下一阶段取舍。

深度对比与下一步落地说明：
- `docs/brainstorms/2026-05-08-packaging-semantic-convergence-progress-and-next-steps.zh-CN.md`

## 2026-06-09 Current-Main 真值同步

这份路线图现在也需要把 current-main 的 docs-sync 基线显式写出来：`7af2f9b`（`docs(progress): sync current-main helper proof truth`），并且该基线开始前仓库处于 clean 的 `main...origin/main` 状态。

这条校正的真实含义是：

- 任务 0 仍然是首要架构瓶颈。当前 build/release/audit/docs 真值仍然只承认单入口 `main.js` + inline `srcdoc`，并且 `createRenderHostBundleBuildOptions()` 继续保持在 production `esbuild.config.mjs` 之外的 candidate-only 状态。
- 任务 2 已不再是“spec-first pipeline 是否存在”的问题，而是 legacy command ID、workflow/sidebar 文案以及 experimental-era naming 的兼容收口问题。
- 任务 3 已不再是“Mermaid subtype 支持是否存在”的问题，而是带真实 Obsidian 会话与已保存产物检查门槛的 `src/mermaidProcessor.ts` 降责问题。
- 已检入的 release / chronicle / repo-saga helper 入口现在都具备 process-level 回归证明，因此任务 0 的 packaging 证据已经是 repo-owned 真值，而不再只是维护者经验。

## 2026-07-04 参考项目集成现实校正

Cloudy 与 Drawnix 的参考项目分析已记录在 `docs/brainstorms/2026-07-04-diagram-reference-integration-and-figure-generation-plan.zh-CN.md`。

本地已检查基线：

- `cloudy-liu/cloudy-tech-diagrams-skill`：`main@719a5be`。
- `plait-board/drawnix`：`develop@9939f45`。
- Notemd 架构基线：`main@562074f`。

这条校正的含义：

- Cloudy 强化的是带语义 Draw.io export annotations 的可编辑 HTML/SVG figure target，而不是再增加一种 Mermaid-only prompt style。
- Drawnix 仍然只是未来 `.drawnix` data/export target 候选，不应被解读为需要把完整 React/Plait 白板宿主嵌入 Obsidian 插件。
...
## 2026-08-15 Mermaid 规范化合并方案（历史基线）

全仓库 code-slop 审计发现一处有用户可见行为影响的发散与若干收敛项。合并方案记录在 `docs/plans/2026-08-15-mermaid-normalization-consolidation.zh-CN.md`（英文：`...en.md`）。

以下条目是 2026-08-17 diagram-level 收敛之前记录的历史审计基线。它们用于保留决策依据，不代表当前实现状态。
...
- **已交付的平台底座**：任务 1、4、5、任务 6 的大部分，以及任务 7 中与 release/export 相关的底座，都已经真实存在于 `main`。
- **仍然活跃的成熟度工作**：任务 0 的剩余打包边界、任务 2 的命令表面收口、以及任务 3 的 legacy 边界收缩，仍然是当前工程主议程。
- **更远期的未来选项**：任务 8 以及任何 Drawnix 邻近的 board export 工作，都属于后续选择，而不是当前欠账。

这层解读很重要，因为它改变了这份文档的战略含义：产品现在不需要再发起一轮“新图表平台建设”，而需要一轮“平台稳定化与边界加固”。
...
更早的 DeepSeek 实时链路验证现在应被视为历史本地证据，而不是仓库内仍在维护的测试表面。相关 live test 文件已不在 `main` 上，当前文档不应再暗示 `src/tests/liveChainTest.test.ts` 仍是可持续执行门。
...
| 任务 | 状态 | 当前现实 |
|---|---|---|
| 任务 0 | 已交付，但限制已明确 | `src/rendering/webview/*` 与 `src/rendering/host/iframeRenderHost.ts` 已落地，继续采用内联 `srcdoc` 方案；`scripts/audit-render-host-bundle.js`、release workflow、`scripts/lib/packaging-contract.js` 与单测已把“render host 必须由 `main.js` 自包含携带”的 smoke gate 固化下来。`createRenderHostBundleBuildOptions()` 继续保持在 production `esbuild.config.mjs` 之外的 candidate-only 状态，因此真正的 heavier-runtime isolation 仍未完成。 |
...
| 任务 7 | 已交付，但限制已明确 | 主题、locale、SVG/PNG/source export 和文档矩阵已对齐当前代码；HTML 目标仍只承诺 iframe fallback preview 与 raw source save。release workflow 也已补上 `actions/checkout@v6` 与 `actions/setup-node@v6` 的 action pin，避免继续背着旧版 Node 20 JavaScript-action 弃用告警；同时，当前 release / chronicle / repo-saga helper 入口也已经具备当前 release-contract 轨道所需的 process-level 回归证明。 |
...
剩余限制依然存在：`esbuild.config.mjs` 仍是单入口，当前 smoke gate 证明的是“现有 `srcdoc` host 继续由 `main.js` 自包含携带且不退化”，不是“已经具备真正独立 heavier runtime bundle 作为单独发布资产的打包、安装与交付策略”。
...
- 当前路线图事实也需要说清：今天的打包仍是单入口。被真实强制约束的边界是“render host 自包含于 `main.js`”，而不是“已经交付独立 render-host 资产包”。
...
1. 先回到任务 0 的剩余 heavier-runtime packaging boundary，持续对齐当前 source/build/release/audit 真值，再考虑任何更宽的能力叙述
2. 完成任务 2 剩余部分：diagram command architecture 收口
3. 完成任务 3 剩余部分：`mermaidProcessor.ts` 降责与 legacy fixer sunset boundary
4. 继续维护任务 7：把 support matrix、release contracts 与已检入 helper-entrypoint proof 绑定得更硬
5. 把任务 0 的 smoke gate 与 candidate-only build guard 一起当成回归边界；后续若引入独立 host 资产必须先升级 release/install 设计
