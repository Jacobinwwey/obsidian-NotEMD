...
- `commits/main/status` 当前会返回 `pending` 且没有任何 status，但 `main` 同时没有 check suite、没有 check run、也没有 branch protection；这里真正的 CI 信号应看 Actions runs
- 最近一次成功的 `1.8.3` release run 仍提示 GitHub 官方的 Node 20 JavaScript-action 弃用告警，因此本次已把 `actions/checkout` 与 `actions/setup-node` 升级到 `v6`
