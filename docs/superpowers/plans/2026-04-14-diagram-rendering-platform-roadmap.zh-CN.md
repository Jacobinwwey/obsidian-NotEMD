...
| 任务 7 | 已交付，但限制已明确 | 主题、locale、SVG/PNG/source export 和文档矩阵已对齐当前代码；HTML 目标仍只承诺 iframe fallback preview 与 raw source save。release workflow 也已补上 `actions/checkout@v6` 与 `actions/setup-node@v6` 的 action pin，避免继续背着旧版 Node 20 JavaScript-action 弃用告警。 |
