...
- Stage B release-trigger 解析现在也支持 `on` 序列内联对象事件映射写法（例如 `- { push: { tags: [...] }, workflow_dispatch: {} }`），并继续忽略这类内联对象中的嵌套非事件键。
...
- Stage B implementation-readiness 检查现在也编码了显式 multi-entry 候选契约真值：helper 输出新增 `Implementation Readiness Contract` 区块并锚定当前 `esbuild.config.mjs` 单入口事实，使 multi-entry/dedicated-asset 方向在 Stage-C runtime-boundary 交付被证明前保持“实现前契约”语义。
...
2. sidebar progress 现在包含类型化 API 测活指示器，底层事件契约为 `request-start`、`response-headers`、`response-chunk`、`request-complete`、`request-error`
...
7. sidebar 现在也已经提供首个 inline active/recent request timeline，并继续复用同一份 record store，用户无需离开面板即可看到近期重试与终局转换
8. 本切片的具体深度对比进度工件现已检入：
...
3. 下一阶段更深的工作应基于这次已落地的 export/report + inline drill-down 基线继续扩展 per-request 可观测性深度，而不是继续在全局 UI 状态上堆条件分支，因为 `requestId`、“请求被接收 / 正文接收”的状态区分、基础结构化 liveness trace、第一段导出工作面，以及第一段 inline timeline 都已经落地
