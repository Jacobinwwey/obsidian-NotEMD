...
- Stage B 契约定义推进也已具备可执行形态：semantic helper 模板新增 `Packaging Contract` 区块，从 `scripts/release/publish-github-release.js` 同步 release 必需资产，并在维护者核验中显式保留双语 release notes 文件契约，同时记录数字 tag 与 create/upload 模式契约真值。
