...
**Files:** `src/ui/diagramTypePreviewPanel.ts`, new `scripts/generate-diagram-gallery.js`, new `scripts/lib/diagram-gallery-runtime.js`, `docs/assets/diagrams/` (generated), `docs/maintainer/diagram-capability-catalog.*`.
...
The in-app selector uses a single selection-driven preview panel shared by Settings and the Notemd Workbench. Selecting an executable type calls `renderDiagramExampleThumbnail()` and updates the fixed-size panel with production SVG, loading, unavailable, or error state. `ref/diagram-design` remains development-only taxonomy; no original screenshot is bundled or displayed.
...
Generate the support matrix from the manifest. Separate “shipped”, “partial”, and “reference-only/planned” rows. Link every shipped type to its preview image and fixture ID. The runtime manifest carries only executable capabilities plus reference-only roadmap metadata; the build verifies production fixture coverage and never reads the reference checkout. Do not claim Draw.io, Drawnix, or Circuitikz external interoperability without the corresponding consumer gate.
...
Radar is admitted only through a dedicated `radarSpec` with bounded axes/series, deterministic Cartesian projection into a layered Vega-Lite document, a stable fixture, HTML fallback, bilingual gallery rows, and focused parser/renderer/generation tests. A `dataChart` `chartType: radar` alias remains invalid. Future candidates must repeat the same schema, fixture, persistence, docs, and consumer gates.

#### Phase 5 update (2026-08-18)
...
- **Radar representation:** a dedicated intent and `radarSpec` preserve axis identity and scale invariants. Reusing `dataSeries` plus a `chartType` alias would make incomplete axis coverage indistinguishable from a Cartesian chart. Vega-Lite `area` marks were rejected after browser rendering showed Cartesian baseline spikes; closed lines and points are the robust adapter boundary.
