...
|---|---|---|
| Task 0 | Partial | `src/rendering/webview/*` 与 `src/rendering/host/iframeRenderHost.ts` 已落地，但仍采用内联 `srcdoc` 方案；`esbuild.config.mjs` 仍是单入口，尚无 dedicated packaging smoke test。 |
...
当前已经通过 `src/rendering/webview/contract.ts`、`src/rendering/webview/page.ts`、`src/rendering/webview/renderFrame.ts` 和 `src/rendering/host/iframeRenderHost.ts` 建立了内联 `srcdoc` preview host。也就是说，“渲染页面如何被插件携带”这个问题已通过主 bundle 内嵌 HTML 的方式部分解决，而不是通过额外静态资产复制解决。

剩余缺口不在“有没有 webview 页面”，而在“有没有真实的 build/runtime isolation 验证”。`esbuild.config.mjs` 仍是单入口，当前也没有 `buildRenderAssets` 类 smoke test 去证明后续重型 runtime 可以安全演进。

- [x] 明确渲染资产目录约定，避免后续把 HTML、JS、CSS 散落到插件根目录。
- [x] 为生产构建增加静态渲染资源复制或生成步骤，保证 `main.js` 之外的资源可随插件一起发布。
...
- 完成 Task 0 中真正缺失的 build/runtime isolation 与 smoke verification
...
1. 完成 Task 0 剩余部分：build/runtime isolation 与 render-host packaging smoke gate
2. 完成 Task 2 剩余部分：diagram command architecture 收口
3. 完成 Task 3 剩余部分：`mermaidProcessor.ts` 降责与 legacy fixer sunset boundary
4. 完成 Task 6 剩余部分：决定 Vega-Lite heavier runtime 的真正宿主边界
5. 继续维护 Task 7：把 support matrix 与 release/docs contract 绑定得更硬
