...
原有批次已经完成，但审计发现 UI 仍把 catalog 行折叠回 semantic intent。本跟进补齐该契约缺口：

- Settings 与 Workbench 现在暴露全部 33 个 catalog ID；`bar-chart`、`line-chart`、`scatter-plot` 可独立选择并预览。
- 以 `preferredDiagramTypeId` 持久化稳定 catalog 身份；`preferredDiagramIntent` 作为对旧 settings 的 additive legacy projection 保留。
- operation planning 将 variant 传入 prompt profile、planner target 解析和 generation；显式定量 variant 强制 Vega-Lite/best-fit，不会静默降级为 Mermaid。
- `diagram.generate` maintainer 边界接受 `requestedTypeId`，registry 与 help metadata 使用同一 operation contract。
- 过期 catalog ID 回退到 legacy semantic intent；不兼容的持久化 type/target 组合在生成前清为 Auto。

证据：variant/UI/operation/CLI 聚焦测试通过；本跟进之后仍需重新运行下方全量回归与发布门禁。
...
- [x] **步骤 1：写失败测试**，覆盖两个目录行共享 `dataChart`、显式 variant 查询、默认查询和 legacy 查询的歧义错误。
- [x] **步骤 2：运行聚焦测试**：`rtk npm.cmd test -- --runInBand src/tests/diagramTypeCatalog.test.ts src/tests/diagramCapabilityManifest.test.ts`；预期因 intent 一对一假设失败。
- [x] **步骤 3：在类型定义中增加 `variant?: string` 与 `payloadKind`**，把 intent map 改为 `Map<DiagramIntent, DiagramTypeDefinition[]>`，保持现有 ID 不变。
- [x] **步骤 4：增加显式查询函数**，当一个 intent 有多个 variant 时让旧查询抛出 `AMBIGUOUS_DIAGRAM_INTENT`。
- [x] **步骤 5：扩展 manifest 行**，增加 variant/layout/payload 元数据，并以 additive field policy 保留 `schemaVersion: 1` 读取兼容。
- [x] **步骤 6：运行聚焦测试和 `rtk npm.cmd run build`**；预期现有目录数量不变且通过。
- [x] **步骤 7：提交**：`git add src/diagram src/tests && git commit -m "refactor(diagrams): make catalog lookup variant aware"`。
...
- [x] **步骤 1：增加失败归一化测试**，覆盖旧 `dataSeries`、`layoutHints.chartType`、timeline、swimlane、quadrant、radar、circuit。
- [x] **步骤 2：运行测试**：`rtk npm.cmd test -- --runInBand src/tests/diagramSpecResponseParser.test.ts src/tests/diagramSpecValidation.test.ts`；预期 canonical payload 缺失。
- [x] **步骤 3：在 `src/diagram/payloads/types.ts` 定义判别式 payload**；第一步只实现本路线需要的 `legacy`、`quantitative`、`topology`、`lane-grid`、`access-matrix`。
- [x] **步骤 4：增加 `schemaVersion` 和可选 `payload`**，保留现有调用方所需的 legacy projection。
- [x] **步骤 5：实现一个 parser 边界**，把旧字段映射为 canonical payload，归一化数值别名，保留 evidence，未知 schema version 直接拒绝。
- [x] **步骤 6：让 `validateDiagramSpec()` 先校验 canonical payload**，再校验归一化后的 legacy projection；renderer 内不散落 null check。
- [x] **步骤 7：运行聚焦测试、build 和全量 Jest**；现有 fixture 的 Mermaid artifact 必须字节稳定。
- [x] **步骤 8：提交**：`git add src/diagram src/tests && git commit -m "feat(diagrams): add versioned canonical payload boundary"`。
...
- [x] **步骤 1：写失败测试**，断言每个 executable catalog 行拥有 profile，profile 声明 required fields/hard limits，prompt 含 source delimiter 且不要求输出 renderer 语法。
- [x] **步骤 2：运行聚焦测试**：`rtk npm.cmd test -- --runInBand src/tests/diagramPrompt.test.ts src/tests/diagramPlanner.test.ts`；预期 profile 查询失败。
- [x] **步骤 3：创建纯数据 profile catalog**，覆盖全部 33 个目录行及 payload family；每个 profile 声明 version、payloadKind、字段、限制、语义规则、target 规则和非法示例。
- [x] **步骤 4：重构 `buildDiagramSpecPrompt()`**，组合通用契约、选定 profile、target/presentation 规则，并保留 Circuitikz/Drawnix 安全约束。
- [x] **步骤 5：增加 source-note delimiter 与反编造规则**，不改变现有语言行为。
- [x] **步骤 6：运行聚焦测试并检查生成 prompt** 的 Mermaid/Drawnix/Circuitikz 回归文本，再执行 `rtk npm.cmd run build`。
- [x] **步骤 7：提交**：`git add src/diagram src/tests && git commit -m "refactor(diagrams): drive generation prompts from profiles"`。
...
- [x] **步骤 1：写失败矩阵测试**，覆盖 target descriptor、默认 presentation、缺 preview mode、重复 `(target, payloadKind)` 所有权和显式不兼容 target 拒绝。
- [x] **步骤 2：运行聚焦测试**：`rtk npm.cmd test -- --runInBand src/tests/renderTargetCatalog.test.ts src/tests/rendererRegistry.test.ts`；预期 descriptor admission 失败。
- [x] **步骤 3：增加所有现有 target descriptor**，不改公共 ID；包含 MIME、扩展名、preview kind、导出格式和 fallback policy。
- [x] **步骤 4：增加 registry admission**，拒绝重复 target 所有权和宣称不支持 preview 的 renderer。
- [x] **步骤 5：将 presentation 默认值接入 `RenderOptions`**，除非明确传入 presentation，不改变现有 artifact cache identity。
- [x] **步骤 6：运行聚焦测试、`rtk npm.cmd run audit:render-host` 和 build**。
- [x] **步骤 7：提交**：`git add src/diagram src/rendering src/tests && git commit -m "feat(rendering): admit targets and presentation contracts"`。
...
- [x] **步骤 1：写失败 payload 测试**，覆盖 zone、稳定 node ID、edge 引用、focal 上限和正交连接。
- [x] **步骤 2：为三个 ID 增加生产 fixture**，使用 catalog-owned fixture 契约，不复制参考截图。
- [x] **步骤 3：实现确定性 topology 几何**，包括 zone 布局、文本换行、正交连接、稳定 SVG ID、明暗 token。
- [x] **步骤 4：注册 native renderer 与兼容 target**；不得添加 Mermaid、Drawio、Drawnix 或 Circuitikz 兼容声明。
- [x] **步骤 5：增加 preview/gallery 测试**，更新双语 support matrix。
- [x] **步骤 6：运行 `rtk npm.cmd run diagram:gallery`、`rtk npm.cmd run diagram:gallery:check`、build、全量 Jest、docs build 和 i18n audit。
- [x] **步骤 7：追加双语进度记录**，记录三个 ID、target 声明、fixture hash 和 unavailable consumer。
- [x] **步骤 8：提交**：`git add src docs scripts && git commit -m "feat(diagrams): add topology layout capabilities"`。
...
- [x] **步骤 1：写失败测试**，覆盖 lane/step/cell 上限、空 cell 不生成、focal handoff 唯一、matrix role/component 上限、封闭 permission level、focal cell 唯一。
- [x] **步骤 2：先实现 validator，再实现 renderer**，使 renderer 依赖稳定不变量。
- [x] **步骤 3：实现固定几何**，吸收参考公式、data-type chip 和 permission cell category，但不接受模型传入任意 renderer 样式。
- [x] **步骤 4：增加 fixture、生产 preview、gallery、双语 docs 和 manifest 行。
- [x] **步骤 5：运行聚焦测试、gallery check、docs build、build、全量 Jest 和 `git diff --check`。**
- [x] **步骤 6：更新双语进度并提交**：`feat(diagrams): add data-flow and access-matrix layouts`。
...
- [x] **步骤 1：写失败迁移测试**，证明旧 `data-chart` 与 `layoutHints.chartType` 仍可读取。
- [x] **步骤 2：增加 chart 上限与真实数值校验**，缺数值时拒绝，不填充估计值。
- [x] **步骤 3：将显式 chart variant 路由到现有 Vega-Lite adapter**，保留 `data-chart` auto/legacy。
- [x] **步骤 4：实现确定性 Gantt 几何**，包含 phase、task bar、milestone，v1 不生成依赖箭头。
- [x] **步骤 5：增加 fixture、preview、gallery、双语 docs 和 support matrix。
- [x] **步骤 6：运行完整回归并提交**：`feat(diagrams): add quantitative variants and gantt`。
...
- [x] **步骤 1：每批只准入 2–4 个 ID**，需要无界图算法的布局拒绝进入本批。
- [x] **步骤 2：按 family 实现明确预算**：stack 4–6 层、Venn 2–3 集合、ranked segments 4–6 段、loop 5–8 station 加一个 hub。
- [x] **步骤 3：loop 圆弧作为有文档的类型特例**；其他非同轴连接仍必须正交。
- [x] **步骤 4：增加生产 fixture、preview/gallery 证据、双语 docs 和 progress entry。
- [x] **步骤 5：运行全部门禁并按 family 独立提交**，失败 family 不阻塞已发布 family。
...
- [x] **步骤 1：每批完成后更新进度**，严格区分 shipped、partial、reference-only 与 unavailable consumer。
- [x] **步骤 2：运行 `rtk npm.cmd run diagram:gallery:check`、`rtk npm.cmd run docs:build`、`rtk npm.cmd run build`、`rtk npm.cmd test -- --runInBand`、`rtk npm.cmd run audit:i18n-ui` 和 `rtk git diff --check`。
- [x] **步骤 3：确认生成资产为最新，`main.js` 仍为 ignored/generated。
- [x] **步骤 4：运行 `rtk proxy git status --porcelain=v1 -b`；预期只显示 `## main...origin/main`。
- [x] **步骤 5：全部门禁通过后提交文档/发布证据，再推送 `origin/main`。
