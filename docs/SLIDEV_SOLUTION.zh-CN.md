...
5. 将整页渲染结果作为视觉 fallback 图片层，用来保留复杂 CSS、Mermaid、SVG、canvas、icon 和 layout 效果；
6. 使用 clean-room PresentationML writer 与 `fflate` 生成 `.pptx`；
7. 额外写出 `.pptx.report.json`，明确记录可编辑覆盖率和 fallback 数量。
...
6. sidecar report 包含 `editableTextSlideCount`、`textBoxCount`、`imageFallbackCount` 与 `pagesWithoutEditableText`
...
该流程会先导出 PPTX，再导出同一 deck 的 PNG reference，然后用 LibreOffice -> PDF -> `pdftoppm` 把 PPTX 回渲染成 PNG，并逐页计算 ImageMagick RMSE/AE。报告模式不会因为阈值超标而让总 `ok` 失败；需要收口时必须加：
...
当前实现有意把 Mermaid/SVG/canvas 和复杂装饰保留为图片 fallback。这能保持源 Mermaid 内容不被改写，也能最大化视觉一致性，但这些图形部分不是 PowerPoint 原生可编辑对象。因此用户合同应写清楚：这是“可编辑文本叠加在视觉 fallback 层上”，不是“把每个 Slidev/Vue/CSS 对象完美重建为 Office 原生可编辑元素”。
...
7. `docs/architecture.zh-CN.md` 的真实 PPTX 验证返回 `ok: true`，输出 27 页、236 个 slide XML `<a:t>` 文本节点、27 个视觉 fallback 图片，且没有缺少可编辑文本的页面。
...
6. PPTX 当前是务实的可编辑文本抽取层：文本可编辑，视觉通过 slide-level image fallback 尽量保留；表格、Mermaid、SVG、canvas 与 Vue component 内部尚未重建成 Office 原生可编辑对象。
7. PPTX visual diff gate 当前明确未通过。不能把“有 fallback 图片”和“有 editable text runs”解释成视觉保真已收口。
