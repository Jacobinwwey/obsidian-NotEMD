...
- 收起态工具栏变为设置内容列中的零高度 sticky 锚点；44x44px 按钮绝对定位在该锚点右上角，header 外壳透明且 `pointer-events: none`；
...
- live geometry 探针确认：收起态 header 是设置内容列中的零高度 `position: sticky` 锚点，外壳透明且不接收 pointer events，交互按钮为 `44x44px` 并绝对定位在内容右上角；首个设置项相较展开态上移原发现行高度 `145.958px`，证明收起后不再占据整行，也不会跑到窗口控制区。
