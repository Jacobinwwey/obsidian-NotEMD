...
- Translation/extraction utility boundary work has moved another step as well: `batchTranslateFolder()` now accepts an injected reporter instead of treating `ProgressModal` as the only carrier, and `extractOriginalText()` now returns its output path so the host adapter can own success completion explicitly.
...
- The remaining architectural gap has moved again: the next phase should prioritize selection-driven/export-surface registry coverage, deeper notice/result/vault-write side-effect tightening, and the still-inline sidebar/direct-read command surfaces in `src/main.ts` rather than more wrapper-only moves.
