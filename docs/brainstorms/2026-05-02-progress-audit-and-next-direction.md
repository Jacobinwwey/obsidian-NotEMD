...
- The remaining architectural gap has moved again: the next phase should prioritize utility-command registry coverage, deeper notice/result/vault-write side-effect tightening, and the still-inline long-tail command surfaces in `src/main.ts` rather than more wrapper-only moves.
