...
| 自动修正 | `SlidevDeckPatch` 已能在有界重试里执行 measured `zoom`、slot-zone 几何驱动的 local `<Transform>`、Mermaid / table / code 结构拆分、病态 table 的 record fallback、generic slot-marked layout 拆分，以及第一张 deck headmatter 页的结构拆分；这条 bounded retry loop 现在已被 verifier 与 `exportSlidesCommand()` 共同复用 | 已落地，继续扩展 |
...
6. 当前 source-preparation prompt 已经使用完整 skill references 并要求拆分密集内容；现在唯一保留的静态 Mermaid zoom 启发式只负责补缺省值，不再覆盖显式 `zoom`；重导出时，整页 `zoom` 由 measured safe-rect overflow 决定，局部 `<Transform>` 的 scale 则由 slot-owner geometry 与 scroll overflow 决定。
...
2. `slidevLayoutAudit.ts` 已经支持 rendered bbox、scroll overflow、slot-zone owner geometry 与部分结构化 patch；
...
4. `docs/SLIDEV_SOLUTION.*` 与 `docs/maintainer/slidev-export-workflow.*` 已更新，明确 hard gate 与 quality gate 需要拆开报告。

下一步：

1. Stage 1 先增强 rendered measurement：effective font、SVG text font、table/code min font、quality margin、content-area ratio 与 low-utilization；
2. Stage 2 再新增 clean-room `SlideGeometry` / `SlideLayoutPlan`，在 LLM 生成 deck 前做几何预算；
3. Stage 3/4 把 Mermaid/table/code 的 semantic split 接进 source preparation 和 patcher，减少低 `zoom` 作为最终修复手段；
4. Stage 5 扩大 fixtures，至少覆盖大型 flowchart、长 sequenceDiagram、宽表、长表、mixed code/table、component-heavy slot layout 与真实 `architecture.zh-CN.md`；
5. 任何实现切片都必须保留 strict standalone 与 full-deck browser audit，不得为了速度退回 CLI-only 或代表性抽样。
