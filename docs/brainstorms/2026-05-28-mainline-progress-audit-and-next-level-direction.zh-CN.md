...
### 2.11 2026-06-17 Slidev export 渲染后布局真值
...
6. `scripts/verify-slidev-export-workflow.cjs` 会用真实 `docs/architecture.zh-CN.md` 源文件跑生产模块，并记录 skill reference 数、本地 fork、deck 摘要、Playwright 抽样与 `.gitignore` 可见性。
...
| 渲染后布局 containment | 没有 DOM bbox、SVG/viewBox、表格自然宽度、scroll overflow、边缘像素或最小可读缩放审计 | P0 开放 |
| 自动修正 | 当前 guardrails 仍主要依赖 Markdown 启发式和静态 Mermaid `zoom`，没有 measure/patch/retry 闭环 | P0 开放 |
...
2. 它仍不能证明 Mermaid-heavy、table-heavy 或密集架构 deck 的视觉质量；
3. 下一步架构推进应是渲染反馈质量门：测量、有限 patch、fail-closed 报告。
4. 当前已经落地的实现真值比最初方案更具体：workspace-aware 的 local Slidev fork / Slidev skill / Playwright browser cache 解析，visible slide root 布局审计，以及基于 per-slide `zoom` 的 bounded patch/rebuild 都已进入真实维护者链路。
...
7. `npm run verify:slidev-export -- --json` 现在已经可以在真实 `docs/architecture.zh-CN.md` 上证明：
   - 命中 `/home/jacob/slidev/packages/slidev/bin/slidev.mjs`
   - 命中 `/home/jacob/slidev/skills/slidev`
   - 命中 Jacob 的 Playwright browser cache
   - 对 sampled HTML slides 执行 visible-root layout audit 与 zoom-only bounded patch
...
3. 在声称输出能稳住图、表、代码和密集文本之前，应新增 `SlidevRenderedMeasure`、`SlidevOverflowAudit` 与 `SlidevDeckPatch` 这些 NoteMD 自有模块；
4. 当前 landed patcher 还是 `zoom`-only；下一步应把 patch/retry 从单纯缩放推进到拆页、拆图或拆表，否则 sequence diagram 这类高纵深页仍会依赖多轮缩放收敛；
