...
| 自动修正 | `SlidevDeckPatch` 已能在有界重试里执行 measured `zoom`、slot-zone 几何驱动的 local `<Transform>`、table/code 结构拆分、病态 table 的 record fallback、generic slot-marked layout 拆分，以及第一张 deck headmatter 页的结构拆分；这条 bounded retry loop 现在已被 verifier 与 `exportSlidesCommand()` 共同复用；Mermaid 源 fence 保持，不自动拆原图 | 已落地，继续扩展 |
...
4. 当前已经落地的实现真值比最初方案更进一步：workspace-aware 的 local Slidev fork / Slidev skill / Playwright browser cache 解析、对 `https://github.com/Jacobinwwey/slidev.git` 本地 checkout 的实机确认、full-deck visible slide root 审计、现有 Slidev deck 的 isolated working-copy + sibling support sync 验证，以及覆盖 Mermaid、Markdown table、病态 table record fallback、code fence、密集文本、generic slot-marked layout、component-heavy custom slot 的 local `<Transform>` fallback 与第一张 deck headmatter 页的非 zoom-only patch/rebuild 都已进入真实维护者链路；已知坏掉的 standalone bundle 也会回退到 server-script HTML，而不再被当成成功产物；修复 NoteMD sanity detector 对 `$n` 这类 minified loader binding 的误判后，真实 `architecture.zh-CN.md` 严格 native standalone 现在已经以 `actualMode = "standalone"` 通过；
...
7. 再下一条 verifier-hardening 切片也补上了一个隐藏的 under-audit bug：即使 slot owner 用 `overflow-hidden` 把后代裁出了当前视口，这些 slot-owned descendant 现在仍会进入测量，因此 component-heavy custom layout 不会再因为“容器把超界子节点藏起来了”而被误判为通过；
8. 这次也把 overflow contract 进一步收窄：整页 `zoom` 与局部 `<Transform>` 的 scale 仍然来自 safe-rect-aware 的 measured fit，但 pass/fail 的 hard overflow 继续锚定在真实渲染后的 slide root，而不是把 safe inset 本身当成硬裁剪边界。这样既能抓住真实裁剪，又不会把合理的 edge-aligned layout 过度误杀；
9. 这次最关键的收口是：patch/rebuild loop 不再只是 verifier 能力，真实产品导出路径也已经复用同一条收敛链，因此 `HTML`/`PDF`/`PNG`/`MP4` 都会从同一个 converged prepared deck 导出；
10. 下一步架构推进已经更收窄：继续扩展 richer component-heavy custom Slidev layout 的结构化 patch，而不是退回代表性抽样页验证；当前仍未收口的主要情况不是“还没有检测驱动缩放”，而是多个 competing component-heavy slot zone 的 zone 级几何仍然接近打平且并非每个溢出 zone 都能安全 transform、或 owner surface 本身不形成稳定 structural split / local transform target 的 deck；同时必须把 native standalone 与兼容 fallback 拆成两条验收 lane：`server-script-fallback` 仍是有用兼容结果，但不能再满足 native standalone 成功声明。
...
2. verifier 与真实产品导出命令现在都会通过生产 TypeScript 模块执行 `prepareSlidevExportSource()` 加共享的 `convergeSlidevDeckLayout()` 工作流，而不再把 patch/rebuild 只留在 verifier 里；
...
5. `docs/SLIDEV_SOLUTION.md` 与 `docs/SLIDEV_HTML_FIX.md` 已从旧 server-only 叙述更新为 standalone 优先、server-script 兼容的当前真值，并补上更严格的 `--require-native-standalone` gate。
6. 当前 source-preparation prompt 已经使用完整 skill references，要求拆分密集 prose/table/code，并明确保持 Mermaid 源 fence；现在唯一保留的静态 Mermaid zoom 启发式只负责补缺省值，不再覆盖显式 `zoom`；重导出时，整页 `zoom` 由 measured safe-rect overflow 决定，局部 `<Transform>` 的 scale 则由 slot-owner geometry 与 scroll overflow 决定。
7. `node scripts/verify-slidev-export-workflow.cjs --json` 现在已经可以在真实 `docs/architecture.zh-CN.md` 上证明：
   ...
   - `HTML`、`PDF`、`PNG` 三条实机导出链可用，而且现在都从同一个 converged prepared deck 导出
   - 真实 HTML fixture 现在已以严格 native standalone 收敛到 `ok: true`、`actualMode = "standalone"`、`standaloneGate.passed = true`、`28` 个审计页、`overflowCount = 0`、`retryCount = 4`
   - 维护者本地结构化 overflow note 已能通过真实 Markdown table decomposition 与 code-fence chunking 收敛，而不只是单测改写
   - 若未来 native bundle 真的存在 loader gap，兼容 fallback 会通过 `htmlExport.actualMode = "server-script-fallback"` 与保留的 `index-standalone.failed.html` 形成可见证据
8. `docs/maintainer/slidev-standalone-acceptance-2026-06-18.*` 现在已经作为真实 `architecture.zh-CN.md` strict standalone run 的专门验收包落盘：其中记录了 report 路径、归档输出路径、本地 fork 证据、完整 skill references 证据、layout audit 摘要、早期 `$n` 误判的根因，以及大型 HTML/截图产物继续归档到 `/home/jacob/slidev-export-review/2026-06-18/standalone-strict/`、不提交进 `main` 的明确策略。

下一步：

1. Slidev export 相关改动必须把 `npm run verify:slidev-export` 纳入收尾证据；
2. 生成的 `docs/export/` 产物可以保留给本地查看，但除非任务明确要求，不应提交；真实验收产物可以归档到 repo 外，例如 `/home/jacob/slidev-export-review/`；
...
5. 产品面 smoke 仍有一个宿主限制：`obsidian command id=notemd:export-slides` 现在能证明命令被派发，但 Obsidian CLI 还没有导出完成握手，所以 verifier JSON 仍是更强的收尾信号；
6. 如果要给上游 Slidev skill 提 PR，范围应限制在通用 deck-conversion/export guardrails，例如完整 references、内置主题优先、frontmatter 闭合、大图/表格/代码块使用可读 transform、build 后浏览器抽样验证；不要上游 NoteMD 的 vault 路径、本地 fork 路径、layout audit 内部实现或 `architecture.zh-CN.md` fixture。
7. standalone 验收从现在开始应作为 tracked maintainer evidence package 维护，而不是停留在聊天摘要里：如果 strict report 或归档输出路径变化，就更新 `docs/maintainer/slidev-standalone-acceptance-2026-06-18.*`，或按同样结构新建带日期的验收文件。

### Batch H：把 Slidev export 从硬可见范围推进到演示质量门

优先级：`P0`

背景：

1. Batch G 已经证明真实 UI 等价 export、完整 Slidev skill references、本地 fork、strict standalone 与 full-deck rendered audit 可以跑通；
2. 但真实 `architecture.zh-CN.md` 输出仍暴露出新瓶颈：低 `zoom` 页没有裁切，却可能不适合演示；
3. `ref/infinite-canvas` 的参考价值是 world rect、union bounds、viewport fit 与节点几何，而不是它的 UI 或 AGPL-3.0 实现代码。

当前进展：

1. `convergeSlidevDeckLayout()` 已经是产品路径与 verifier 共用的最终事实门；
2. `slidevLayoutAudit.ts` 已经支持 rendered bbox、scroll overflow、slot-zone owner geometry、effective font、SVG/table/code 最小字号、quality margin 与 content-area ratio；
3. `docs/brainstorms/2026-06-20-slidev-layout-quality-and-canvas-roadmap.zh-CN.md` 已落盘为专项路线文档，对比了先前要求、当前代码、`ref/infinite-canvas` 可借鉴点、真实输出问题与下一阶段实现顺序；
4. `src/slideExport/slidevLayoutPlan.ts` 已新增 clean-room layout budget，把 world-rect / viewport-fit 思想转成 NoteMD 自有的 `SlideLayoutPlan`，并接入 deterministic outline、一次性导出 prompt 与基于大纲继续导出 prompt；
5. low effective font、tight margin 与 low content utilization 现在会对 table/code/prose 产生结构化 `recommendedPatch`；Mermaid 默认保留源 fence，记录 fit 指标而不是自动拆原图；
6. `docs/SLIDEV_SOLUTION.*` 与 `docs/maintainer/slidev-export-workflow.*` 已更新，明确 hard gate 与 quality gate 需要拆开报告。

下一步：

1. Stage 1/2 已落地第一切片，后续重点转向真实 `architecture.zh-CN.md` strict standalone 验收与阈值校准；
2. Stage 3/4 继续加强 Mermaid 源图保持的 fit/manual-review 证据、cell-level table rewrite 与 code semantic excerpt，不要退回手写固定 zoom；
3. Stage 5 继续扩大 fixtures，至少覆盖大型 flowchart、长 sequenceDiagram、宽表、长表、mixed code/table、component-heavy slot layout 与真实 `architecture.zh-CN.md`；
4. 任何实现切片都必须保留 strict standalone 与 full-deck browser audit，不得为了速度退回 CLI-only 或代表性抽样。

## 5.1 2026-07-02 主线 CI / GEO / CLI / Slidev 收口

本批次收口的风险类别不同于前面的布局质量和 provider control-plane 批次。新的根因不是 Slidev 布局收敛回退，而是环境探测以及相邻的 maintainer / release helper 仍带着平台相关的命令解析假设。

### 远端 main CI 结论

本次调查基线下，远端 `main` 没有当前 source/build failure：

1. 最近列出的 `main` 上 `Deploy Docusaurus to GitHub Pages` workflow 已是成功状态；
2. 重点调查的失败 run `27451762938` 失败在 `actions/deploy-pages@v4`，报错为 `HttpError: Not Found`，并明确提示需要启用 GitHub Pages；
3. 当前 `eb777ef` 没有关联 check-runs，legacy status endpoint 也没有 statuses，因此 legacy API 显示的 pending 不是失败中的 CI context。

解释：

1. 历史远端失败是 Pages settings / deployment availability 问题，不是 Docusaurus build 或 `audit:build` 源码失败；
2. 只要改动触及 `website/**` 或 `.github/workflows/deploy-docs.yml`，Pages deployment 仍是硬门禁；
3. 本批次若触及 website 文件，推送新的 `main` 后仍应观察 Pages workflow。

### 跨平台命令架构

当前实现已让插件 Slidev probe 与 Node maintainer scripts 使用同一类命令解析模型：

| 要求 | 当前实现 | 状态 |
|---|---|---|
| POSIX shell 行为不能回退 | Linux/macOS 保持 direct exec first，普通命令不加 shell wrapper | 已收口 |
| Windows `.cmd` / `.bat` shim 必须可用 | Windows 通过 `PATH` + `PATHEXT` 解析命令名，只对解析到的 batch 文件进入带 quoting 的 `cmd.exe /d /s /c call` 隔离路径 | 已收口 |
| Windows JSON/code payload 参数必须保真 | 不使用全局 `shell: true`；`.exe` / `.com` 直接运行，Node scripts 通过 `process.execPath` 运行 | 已收口 |
| release / repo-saga / maintainer helper scripts 必须共享同一规则 | `scripts/lib/cross-platform-command.js` 集中承载 Node 侧 process contract | 已收口 |
| Slidev HTML 输出路径识别必须适配 OS 分隔符与新版 Slidev 输出形态 | `src/slideExport/htmlExportPaths.ts` 集中承载 `index.html` 发现与路径归一化 | 已收口 |

这个设计是刻意的：Windows 需要 batch-shim adapter，但 Linux 与 macOS 本来就通过 direct exec 正常工作。把 Windows 全局改成 shell 虽然能遮住原来的 `spawn EINVAL`，但会引入更难排查的参数损坏问题。

### GEO 与 GitHub Pages 进展

当前网站架构已经超过最初 GEO roadmap 的基础要求：

1. root 与 zh-CN root 页面存在；
2. `llms.txt`、sitemap filtering、homepage JSON-LD、partial-language copy、provider-doc heading gate 与 measurement log 都由 `website/scripts/audit-build.cjs` 覆盖；
3. zh-CN critical-path 页面与 UI chrome 已翻译，fallback 页面继续保持 `noindex,follow` 并排除在 zh-CN sitemap 之外。

剩余工作不是继续做 source-only route rewrite，而是线上测量：

1. 在 Search Console 提交或刷新 root 与 zh-CN sitemap；
2. 检查 root、zh-CN root、FAQ、provider overview、一个 provider detail 与一个未发布 zh-CN fallback；
3. 抓取后重跑英文和中文 answer-engine prompts；
4. 在 measurement log 中记录精确日期、engine、prompt、citation 与 route 结果。

### 更新后的推进方向

下一步工程方向比“收尾 GEO 和 CLI”这个口号更窄：

1. path-based 写入操作只有在具备显式 public schema、副作用文档、确定性失败模式与用户文档后，才考虑 public CLI promotion；
2. Slidev export 继续保持 UI-equivalent 与 browser-rendered 验证，不退回只跑 `slidev build`；
3. Pages 真值继续以 built output 为门禁，而不是只做源码 review；
4. 任何未来 semantic/vector local-KB 扩张都应视作新的架构决策，不应从当前 MiniSearch lexical retrieval 自动推出；
5. release / repo-saga / maintainer scripts 继续使用共享跨平台 process adapter，不重新引入零散 `spawn`、`execFile` 或 shell-only 调用。

## 5.2 2026-07-03 仓库文档真值 / 布局对齐

仓库根目录此前已经累计了过多一次性的状态报告、迁移说明与调查文档，它们不再是 current-main 真值的最佳入口。

当前收口方向现在已经明确：

1. `docs/README.md` 是 canonical documentation hub。
2. 根目录的 `DOCUMENTATION_INDEX.md` 现在只保留为薄入口，而不是继续充当重复的长目录。
3. 历史性的根目录报告迁入 `docs/archive/root-history/`，不再继续堆在 `README.md` 旁边。
4. `GEO_ROADMAP.md` 暂时仍保留在根目录，因为当前 website audit/test 流程还会直接读取这个路径。
5. 这一批次的活跃真值应落在 `docs/` 下的 canonical progress docs、maintainer docs 与 bounded roadmap/closeout docs。

这不是单纯的外观整理，而是后续批次的操作规则变化：

1. 后续收口应更新 `docs/` 下的 canonical progress docs，而不是继续生成新的根目录 summary；
2. 根目录文档再次失控应被视为仓库真值维护回归，而不是无害的随手记。

## 5.3 2026-07-22 Drawnix 导出质量真值

当前代码已经存在 Drawnix target，但它的质量边界比先前进度文字暗示的更窄。`DrawnixRenderer` 接收的是通用 `SemanticFigureModel`：节点被拍平到固定三列矩形网格，`DiagramNode.children` 丢失，SVG preview 也由通用 editable-SVG 路径渲染。因此生成的架构产物只能证明 JSON 外壳兼容，不能证明它是 Drawnix 知识导图画布。

这会以有边界的方式调整优先级：

1. 完整 Drawnix 宿主嵌入继续排除；
2. 在替换实现落地前，面向外部的通用 Drawnix 支持表述必须收窄为已实现子集；
3. 下一项 Drawnix 交付是目标专用、可编辑的思维导图投影，不是再添加一个通用 renderer 或走字符串 converter 捷径；
4. 架构画布继续作为后续决策，因为它需要独立的分组、路由和碰撞语义；
5. 现有 Obsidian CLI 调用只属于集成验证路径，不构成扩大 public CLI contract 的理由。

详细的代码/方案对照、交付顺序、prompt 契约和验证矩阵见 `docs/brainstorms/2026-07-22-drawnix-knowledge-map-quality-and-delivery-plan.zh-CN.md`。
...
2. `docs/maintainer/slidev-standalone-acceptance-2026-06-18.*` 或最新带日期的 standalone 验收文件
...
Slidev export 收尾还必须基于 `docs/architecture.zh-CN.md` 输出非空 layout-audit report，并且 `overflow` 与 `unreadable-scale` 均为零。Batch H 实现后，还应额外检查 effective-font、quality-margin、content-area-ratio 与 pre/post split 指标，不能只用低 `zoom` 作为最终质量修复。
...
6. Slidev export 能否继续用真实 UI-equivalent workflow 验证，并把 hard overflow gate 推进到 effective-font / quality-margin / layout-planning gate，而不是退回到“CLI 能 build 就算按钮可用”或“缩进去就算可读”的弱证明；
