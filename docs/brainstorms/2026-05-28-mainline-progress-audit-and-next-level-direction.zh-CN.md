...
6. 当前 source-preparation prompt 已经使用完整 skill references 并要求拆分密集内容；现在唯一保留的静态 Mermaid zoom 启发式只负责补缺省值，不再覆盖显式 `zoom`，重导出时的 zoom 由 measured safe-rect overflow 决定。
