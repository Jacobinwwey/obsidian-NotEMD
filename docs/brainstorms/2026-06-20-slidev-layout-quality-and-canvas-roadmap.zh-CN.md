...
31. Stage 15 Mermaid no-split hardening 真实 `architecture.zh-CN.md` strict native standalone 验收包：`/home/jacob/slidev-export-review/2026-06-21-stage15-final-rerun/`
...
50. Stage 15 真实 `architecture.zh-CN.md` strict native standalone rerun 已通过并归档到 `/home/jacob/slidev-export-review/2026-06-21-stage15-final-rerun/`：`ok = true`，使用 `/home/jacob/slidev/packages/slidev/bin/slidev.mjs` 与 `/home/jacob/slidev/skills/slidev` 的 52 个 references，`actualMode = "standalone"`，`requiresLocalServer = false`，`standaloneGate.passed = true`，`slideCount = 27`，`hardOverflowCount = 0`，`lowEffectiveFontCount = 0`，`mermaidSourcePreservation.passed = true`，源文档与导出 deck 仍为 3 个 Mermaid fence 且 `changedFenceIndexes = []`；可审查 deck 已归档为 `architecture.stage15.slidev.zh-CN.md`，并在仓库内保留副本 `docs/slidev/architecture.stage15.slidev.zh-CN.md`，native standalone 输出为 `architecture.zh-CN-slides/index-standalone.html`，但 HTML/assets/screenshots 不进入提交。
...
29. Stage 15 真实 `architecture.zh-CN.md` strict native standalone 已重新验收并归档到 `/home/jacob/slidev-export-review/2026-06-21-stage15-final-rerun/`；报告继续证明本地 Slidev fork、52 个 skill references、native standalone、3 个 Mermaid fence byte-stable、`changedFenceIndexes = []`、0 hard overflow 与 0 low effective font。可审查输出为 `architecture.stage15.slidev.zh-CN.md` 和 `architecture.zh-CN-slides/index-standalone.html`；仓库内只保留 `docs/slidev/architecture.stage15.slidev.zh-CN.md` 这个 Markdown deck 副本，运行时生成目录已清理，避免测试产物进入提交。
