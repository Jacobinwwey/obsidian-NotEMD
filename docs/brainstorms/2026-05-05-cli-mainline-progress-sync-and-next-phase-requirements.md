...
2. The real remaining gap is one layer deeper: `src/main.ts` still owns `executeSaveMermaidDiagramCommand` and `executeArtifactDiagramCommand`, but the typed contract decision for `diagram.preview` plus provider connection-test is now landed. The remaining question is how much deeper save/artifact branch contract depth should move below those wrappers.
...
- R5. The next contract-tightening batch must now target the deeper diagram/provider command core, especially `executeSaveMermaidDiagramCommand`, `executeArtifactDiagramCommand`, and any richer save/artifact branch contract depth below the newly-landed `diagram.preview` / `provider.connection.test` schemas, while keeping the wrapper batch and write-heavy proof set stable, documented, and registry-aligned.
...
- [R5][Technical] Should the next deeper contract batch split save/artifact execution into additional typed operation boundaries, or keep those branches internal beneath `diagram.generate` / `diagram.preview`?
