...
| Expose diagnostics to users without pretending source is rendered | `RenderArtifact.diagnostics`, localized diagnostic summary counts, preview history entries, and source-only fallback are implemented | Implemented | Connect external renderer artifacts only when evidence distinguishes raw source from rendered output |
