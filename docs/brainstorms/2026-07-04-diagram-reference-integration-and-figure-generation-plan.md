...
- [x] Add non-interlaced 1/2/4/8-bit grayscale PNG smoke support through packed sample decoding and normalized gray values.
...
| Verify circuit output before visual repair | Compile-log diagnostics, topology-preserving repair guard, SVG structural smoke, 1/2/4/8-bit indexed-color/grayscale and RGB PNG foreground smoke, and preview diagnostics are in place | Partially implemented | Add OCR-level label recognition, precise pixel overlap gates, remaining PNG alpha/depth variants, and topology-preserving repair prompts |
