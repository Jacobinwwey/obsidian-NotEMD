...
- [x] Add arc path endpoint geometry for A/a arc endpoints so curved circuit-symbol path endpoints participate in bounded-canvas diagnostics.
- [x] Add Bezier path endpoint geometry for C/S/Q/T curve endpoints so dvisvgm curve endpoints participate in bounded-canvas diagnostics without claiming exact curve bounds.
...
| Verify circuit output before visual repair | Compile-log diagnostics, topology-preserving repair guard, SVG structural smoke, PNG foreground smoke, and preview diagnostics are in place | Partially implemented | Add OCR-level label recognition, exact curve/pixel overlap gates, and topology-preserving repair prompts |
