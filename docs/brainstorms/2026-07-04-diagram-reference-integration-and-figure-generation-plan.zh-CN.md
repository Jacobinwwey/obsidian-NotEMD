...
| 视觉修复前必须先验证电路输出 | compile-log diagnostics、topology-preserving repair guard、structured repair prompt handoff、repair acceptance evidence、deterministic layout-hint projection、SVG structural smoke、1/2/4/8-bit indexed-color、grayscale/RGB `tRNS` transparent samples、format-specific Adam7/interlaced 与 indexed bit-depth rejection guidance、1/2/4/8/16-bit grayscale 与 8/16-bit grayscale-alpha/RGB/RGBA PNG foreground smoke 已存在 | 部分实现 | 增加 OCR-level label recognition、精确 pixel overlap gates 与自动化 topology-preserving repair execution |
