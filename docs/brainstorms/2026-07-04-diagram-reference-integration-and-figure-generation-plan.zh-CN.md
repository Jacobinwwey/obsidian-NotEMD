...
2026-07-07 Draw.io/Drawnix source-only preview increment：看起来像 Draw.io XML（`mxfile` 或 `mxGraphModel`）的已保存 `.drawio` 文件，以及能解析为 Drawnix JSON、包含 `type: "drawnix"` 与 `elements` array 的已保存 `.drawnix` 文件，现在可以通过标准 Preview diagram 命令作为 source-only artifacts 打开。`RenderArtifactTarget` 只在已保存/source artifact 边界包含 `drawio` 与 `drawnix`；`SUPPORTED_RENDER_TARGETS` 保持不变，因此 planner 没有被扩宽。`src/rendering/targetLabel.ts`、`src/rendering/preview/previewExport.ts` 与 `src/fileUtils.ts` 为这些 artifact 提供稳定 label 与原生 `.drawio` / `.drawnix` 保存路径。这不是嵌入 diagrams.net、嵌入 Drawnix host，也不是证明插件内已经具备视觉编辑能力。
