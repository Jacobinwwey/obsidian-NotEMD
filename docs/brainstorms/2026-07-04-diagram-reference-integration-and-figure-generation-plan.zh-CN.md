...
| 视觉修复前必须先验证电路输出 | compile-log diagnostics、topology-preserving repair guard、SVG structural smoke、1/2/4/8-bit indexed-color、1/2/4/8/16-bit grayscale 与 8/16-bit grayscale-alpha/RGB/RGBA PNG foreground smoke 已存在 | 部分实现 | 增加 OCR-level label recognition、精确 pixel overlap gates、interlaced/16-bit indexed rejection guidance 与 topology-preserving repair prompts |
