...
- [x] 通过 PLTE palette decoding 与可选 tRNS alpha 增加非交错 8-bit indexed-color PNG smoke 支持。
...
| 视觉修复前必须先验证电路输出 | compile-log diagnostics、topology-preserving repair guard、SVG structural smoke、indexed-color/RGB PNG foreground smoke 与 preview diagnostics 已存在 | 部分实现 | 增加 OCR-level label recognition、精确 pixel overlap gates、更宽 PNG variants 与 topology-preserving repair prompts |
