...
| 视觉修复前必须先验证电路输出 | compile-log diagnostics、topology-preserving repair guard、SVG structural smoke、1/2/4/8-bit indexed-color 与 RGB PNG foreground smoke 与 preview diagnostics 已存在 | 部分实现 | 增加 OCR-level label recognition、精确 pixel overlap gates、更多 PNG grayscale variants 与 topology-preserving repair prompts |
