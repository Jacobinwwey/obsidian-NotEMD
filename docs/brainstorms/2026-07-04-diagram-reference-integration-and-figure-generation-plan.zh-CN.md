...
| renderer 执行必须跨平台 | `circuitikzCompileRunner.ts` 使用 `shell: false` 与 placeholder-expanded argument arrays | 已实现 | Windows/POSIX 行为继续放在参数数组里处理，不拼 shell command string |
| 视觉修复前必须先验证电路输出 | compile-log diagnostics、topology-preserving repair guard、SVG structural smoke、PNG foreground smoke 与 preview diagnostics 已存在 | 部分实现 | 增加 OCR-level label recognition、exact curve / pixel overlap gates 与 topology-preserving repair prompts |
| 向用户暴露 diagnostics，但不把源码伪装成渲染结果 | `RenderArtifact.diagnostics`、diagnostic summary counts、preview history entries 与 source-only fallback 已实现 | 已实现 | 只有 renderer evidence 能区分 raw source 与 rendered output 后，再接外部 renderer artifacts |
