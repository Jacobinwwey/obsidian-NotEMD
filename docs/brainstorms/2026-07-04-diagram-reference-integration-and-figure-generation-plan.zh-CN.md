...
1. 为两个 circuitikz golden families 增加 maintainer smoke fixtures，用真实本地 TikZJax/LaTeX 安装执行验证；
2. 随真实 renderer logs 暴露更多 failure shapes，继续扩展 compile-log diagnostics；
3. 增加 screenshot-level 检查：nonblank render、expected labels、bounded canvas、obvious overlap；
4. repair prompt 中锁定拓扑，避免视觉修复改变电路；
5. 只有在 renderer evidence 可用之后，才把 source-only preview session 接到外部 artifact outputs，继续区分 raw source 与真实视觉渲染状态；
6. 只有到这一步之后，再考虑更多 circuit families 或插件侧 circuit preview target。
