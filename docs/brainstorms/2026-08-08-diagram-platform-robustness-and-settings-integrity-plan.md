...
|---|---|---|---|
| Structure | `enrichDrawnixSourceCoverage()` builds a synthetic document root, preserves deep source and unmatched model branches, and uses `Additional concepts` for unmatched model roots; there is no fixed depth cap | Meaningful model branches must retain provenance; label equality is not identity | Preserve stable IDs and an explicit source/model mapping. Emit merge/remap/drop diagnostics instead of silently dropping structure. Keep forest-valid validation separate from the document-rooted policy. |
| Cross-root routing | `drawnixRelationRouter.ts` owns reserved-lane routing and bounded fallbacks; `drawnixCrossRootRouter.ts` is a deprecated compatibility re-export | A route can be geometrically valid for the SVG but still be reordered or obscured by the native Drawnix reader; route warnings are not a complete quality result | Validate route segments, inflated node obstacles, root-region crossings, and native label rectangles as one post-layout invariant. Fail closed when the invariant cannot be satisfied. |
