...
但这还不是 CLI-ready 终态。当前剩余问题已经从“命令包装太厚”转移到三类更实质的边界：

1. `src/operations/registry.ts` 仍只覆盖 diagnostics / diagram / profile，尚未吸纳 note-processing 能力。
2. `src/translate.ts`、`src/fileUtils.ts`、`src/extractOriginalText.ts` 仍持有 `App` / `Notice` / `ProgressModal` / vault 写入等宿主副作用，operation contract 还不够干净。
3. `src/main.ts` 虽已显著收缩，但仍保留 duplicate / Mermaid batch fix / formula fix 等未抽离命令编排。
...
**下一阶段优先级**
- R3. 下一阶段必须优先把 note-processing 能力纳入 `src/operations/registry.ts`、`src/operations/capabilityManifest.ts` 与 `src/cliContracts.ts`，而不是先增加新的 CLI 子命令。
- R4. 第一批应优先评估并建模以下能力：
...
- R5. 对上述能力的建模必须显式写出 `automationLevel`、`requiredContext`、`sideEffectClass`、输入 schema、结果 schema，以及哪些命令只保留为 interactive alias。

**宿主副作用收口**
- R6. `src/translate.ts` 的批量翻译流程不得继续把 `ProgressModal` 视为不可替代的执行载体。下一阶段必须把 reporter 注入与宿主选择权上提，为 CLI / maintainer automation 留出无 UI 路径。
- R7. `src/fileUtils.ts` 与 `src/extractOriginalText.ts` 中与 `Notice`、vault 落盘、目录创建、输出命名冲突处理相关的逻辑，必须继续向显式 host effect 或结果对象收口，避免 operation core 隐式写 UI 文案。
...
**剩余 `src/main.ts` 瘦身方向**
- R9. 下一批 `src/main.ts` 抽离优先级应固定为：
  1. duplicate / concept-note cleanup command
  2. batch Mermaid fix
  3. single / batch formula fix
  4. 其他仍内联持有 reporter/Notice 生命周期的命令
- R10. 抽离顺序必须由自动化价值和宿主副作用复杂度决定，而不是按文件类别机械拆分。
...
- note-processing 能力已被明确列入下一阶段 operation onboarding 范围，而不是继续停留在零散命令描述。
...
- 先完成 note-processing registry onboarding，再讨论 CLI transport 的进一步扩展。
- host adapter 抽离已经足够成熟，不应再把注意力浪费在重复搬运 `src/main.ts` wrapper 上；下一阶段真正要解决的是 schema、capability 与 side-effect 边界。
...
- 当前 capability registry 仍未纳入 note-processing operation definitions，这是下一阶段的主要结构缺口。

## 未决问题

### 延后到规划阶段
- [影响 R3][Technical] note-processing operation ID 应按 task family 归一，还是按 command 粒度一一建模？
- [影响 R6][Technical] `batchTranslateFolder()` 的 `ProgressModal` 应拆成 injected reporter，还是直接先补无 UI 分支？
- [影响 R7][Technical] `extractOriginalText` 与 concept extraction 的文件写入结果，是否要先标准化为 result object 再继续向 CLI 暴露？

## 下一步

-> 进入新的实施计划，优先完成 note-processing registry onboarding、translation/extraction utility side-effect 收口，以及剩余 `src/main.ts` command-host 瘦身。
