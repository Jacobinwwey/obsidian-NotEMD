...
- 同一轮 clean worktree 也已落地 note-processing registry onboarding、batch translation 的 injected reporter、original-text extraction 的输出路径返回，以及承接 duplicate cleanup / Mermaid fix / formula fix 的 `src/operations/utilityCommandHostAdapter.ts`。
...
1. `src/operations/registry.ts` 已吸纳第一批 note-processing operations，但仍未覆盖 duplicate cleanup、Mermaid repair、formula repair 等 utility commands。
...
3. `src/main.ts` 虽已显著收缩，但 note-processing 与 utility-host 两轮抽离之后仍有长尾命令面尚未进入 registry/contract 体系。
...
- R4. 上述第一批 note-processing onboarding 现在已经落地，对应 operation 为：
  - `translate.file`
...
- R5. 下一批 registry onboarding 必须把同样的建模纪律扩展到 utility commands 与其他 automation-facing surfaces，显式写出 `automationLevel`、`requiredContext`、`sideEffectClass`、输入 schema、结果 schema，以及 alias policy。
...
**剩余 `src/main.ts` 瘦身方向**
- R9. duplicate / concept-note cleanup、batch Mermaid fix 与 single / batch formula fix 现已抽入 `src/operations/utilityCommandHostAdapter.ts`。
...
- note-processing 能力现在已进入 registry-backed operation 范围，下一阶段 operation onboarding 已明确收敛为 utility commands 覆盖与更深的副作用边界整理。
...
- note-processing registry onboarding 第一批现已完成，下一阶段最高杠杆工作变为 utility-command registry 覆盖与更深的 result/side-effect 收口。
...
- 当前 capability registry 已纳入第一批 note-processing operation definitions；主要结构缺口已转向 utility commands 覆盖与更丰富的结果语义。

## 未决问题

### 延后到规划阶段
- [影响 R5][Technical] utility command IDs 是否也应先按 task family 归一后再进入 registry？
...
-> 进入新的实施计划，优先完成 utility-command registry 覆盖、更深的 utility side-effect 收口，以及剩余长尾 `src/main.ts` command-host 瘦身。
