...
| Automatic correction | `SlidevDeckPatch` now applies measured `zoom`, Mermaid / table / code structural splitting, pathological table record fallback, generic slot-marked layout splitting, component-heavy custom-slot local `<Transform>` fallback, and first-slide headmatter structural splitting in a bounded retry loop shared by the verifier and `exportSlidesCommand()` | Landed, keep extending |
...
4. current landed truth is already past the original plan wording: workspace-aware resolution for the local Slidev fork, Slidev skill roots, and Playwright browser cache is in place, the local checkout is confirmed against `https://github.com/Jacobinwwey/slidev.git`, full-deck visible-slide-root audit is in place, existing Slidev decks now go through isolated working copies with sibling support-entry mirroring, the patch loop now covers Mermaid, Markdown tables, pathological table fallback, code fences, dense text, generic slot-marked layouts, component-heavy custom-slot local `<Transform>` fallback, and first-slide headmatter instead of only zoom, and known-bad standalone bundles now fall back to server-script HTML rather than being treated as good output;
5. a new real custom-layout multi-zone fixture now also proves that when both `summary` and `details` are component-heavy zones, rendered text hints let the patcher attribute overflow to the actual `details` slot instead of retargeting a nonoverflowing sibling zone;
6. the real product export path now matches the verifier on the critical point that mattered most: the patch/rebuild loop is no longer verifier-only, so `HTML`/`PDF`/`PNG`/`MP4` export all reuse the same converged prepared deck.
7. the next architectural step is narrower now: extend structural patching to richer component-heavy custom layouts without regressing back to representative-slide-only audit; the main unresolved shape is when multiple competing component-heavy slot zones exist and rendered text hints still cannot safely identify the true overflow owner; separately, decide whether the current standalone fallback should remain product truth or be replaced by a stronger standalone bundling strategy.
...
6. the current source-preparation prompt uses the full skill references and asks the model to split dense content; the only remaining static Mermaid zoom heuristic is now a missing-value seed, not an overwrite of explicit per-slide `zoom`, and re-export zoom comes from measured safe-rect overflow.
7. `node scripts/verify-slidev-export-workflow.cjs --json` can now prove, on the real `docs/architecture.zh-CN.md` input, that:
   ...
   - the real HTML fixture now closes with `ok: true`, `28` audited slides, `overflowCount = 0`, and `retryCount = 4`
