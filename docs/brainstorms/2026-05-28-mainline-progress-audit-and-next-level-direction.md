...
6. this batch also closes the "where do re-export zoom parameters come from?" question: whole-slide `zoom` is derived from measured safe-rect overflow, while local `<Transform>` scale is derived from the specific out-of-bounds slot-owner surface and its scroll overflow. Neither path depends on fixed constants or manual LLM choice.
7. the real product export path now matches the verifier on the critical point that mattered most: the patch/rebuild loop is no longer verifier-only, so `HTML`/`PDF`/`PNG`/`MP4` export all reuse the same converged prepared deck.
8. the next architectural step is narrower now: extend structural patching to richer component-heavy custom layouts without regressing back to representative-slide-only audit; the main unresolved shape is no longer "lack of measured scaling" but cases where multiple competing component-heavy slot zones still end up in near-tied geometry or where the owner surface does not expose a stable structural split / local transform target. Separately, decide whether the current standalone fallback should remain product truth or be replaced by a stronger standalone bundling strategy.
