...
5. a new real custom-layout multi-zone fixture now also proves that when both `summary` and `details` are component-heavy zones, prepared working copies carry lightweight slot-owner wrappers and rendered text hints then let the patcher attribute overflow to the actual `details` slot instead of retargeting a nonoverflowing sibling zone;
