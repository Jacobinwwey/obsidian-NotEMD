...
## 15. Incremental Progress Update (Named File-Selection Profiles)

### 15.1 Current Code Truth vs Earlier Requirement Shape

The earlier requirement baseline for folder-task filtering is now extended in a bounded way rather than replaced:

1. global folder-task defaults remain the canonical compatibility base;
2. named saved profiles are additive reusable overlays, not a second competing execution path;
3. the new persisted setting is `folderTaskFileSelectionProfiles`, where each profile now carries:
   - `id`
   - `name`
   - optional `folderPathHint`
   ...
   - `fileFilterInvert`
4. precedence is now explicit and regression-locked:
   `per-run explicit override > selected saved profile > global defaults`.

### 15.2 Architecture Boundary Interpretation

This is the correct architectural reading of the new profile layer:

1. the profile system is a selector-layer convenience and consistency upgrade, not a workflow-runtime expansion;
2. `folderPathHint` is intentionally a picker prefill only, not a hidden execution binding, which prevents stale saved paths from silently hijacking later runs;
3. host adapters now prefer a profile-aware `getFolderTaskSelection(...)` path when available, but still fall back to the legacy `getFolderSelection()` path when a host does not implement the new surface;
4. the bounded maintainer bridge now exposes `fileSelectionProfileId` / `fileSelectionProfileName` for the batch-generate operation without widening the public-safe CLI slice.

### 15.3 Progress And Next Direction

Relative to the earlier plan, this means:

1. the original `includeSubfolders` and `relativePath/basename` requirements are no longer only globally configurable; they are now reusable at the profile layer as well;
2. cross-layer drift risk is reduced because interactive picker behavior, selector merging, host adapter execution, and maintainer CLI input parsing now share the same precedence model;
3. the next bounded move should be profile-surface hardening, not broader runtime scope:
   - keep profile export/import out of the public-safe slice unless secrecy and mutability rules are explicit;
   - if more folder operations join the maintainer bridge, require the same precedence and non-mutation tests by default;
   - refine picker UX only if it preserves the current "saved hint, editable runtime folder" contract.
