...
- 剩余架构缺口因此再次转移：下一批不应继续停留在 `src/main.ts` wrapper 搬运，而应优先处理 utility commands 的 registry 覆盖、更深的 notice/result/vault-write side-effect 收口，以及 `src/main.ts` 里剩余的长尾内联命令面。
