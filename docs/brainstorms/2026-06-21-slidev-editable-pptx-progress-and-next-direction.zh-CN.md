...
历史说明：M11 记录的是 visible-native 默认化之前的策略，已被 M26 取代。当时对旧 writer 的判断成立，但这不再是可接受的默认合同，因为它让可编辑文字只能通过渲染图下方的透明 DrawingML 文本被选中。

这个事实现在通过 sidecar 的 `editableLayerContract` 显式记录：

1. M26 之前的默认导出：`visualFidelityStrategy = frozen-background-first`，`visibleTextSource = background-image`，`editableTextShapeFill = transparent`，`editableTableTextFill = transparent`，`backgroundTextPolicy = preserve-rendered-text`，`textSelectionSurface = named-transparent-shapes`；
...
3. M26 之后的 Mermaid/SVG 默认策略：`mermaidSvgVisualPolicy = background-image`，`mermaidSvgTextPolicy = background-image-only`，`officeNativeMermaidSvgElementEditability = not-claimed`；
4. 字体可移植性策略：`fontPortabilityPolicy = report-only-no-default-font-embedding`。

visible-native 成为默认后，与 `oh-my-ppt` 的对比边界也随之变化。真正值得复用的不是另起一条截图路线，而是 native object extraction、标记 consumed DOM、在 fallback capture 前隐藏已建模 DOM，并用 residue/visual-diff gate 检测残留文字。这不等于默认重建 Mermaid/SVG 几何。
...
3. 默认路径不再输出透明 Mermaid/SVG label overlay；除非显式 visible-native 实验证明 parity，否则这些 label 继续由背景拥有；
...
## M25 root-specific 装饰诊断

这一轮完成 M24 下一步的前半段：把 `unsupported-root` 拆成 root-specific reason。它仍然不扩大抽取范围，也不改变 PPTX 渲染结果。目的不是马上增加可编辑 shape 数，而是让下一步抽取决策基于真实数据，而不是把 code、SVG、Mermaid、table internals 混在一个大桶里。

本轮落地内容：

1. decorative primitive skip 合同新增 `unsupported-code-root`、`unsupported-document-root`、`unsupported-mermaid-root`、`unsupported-svg-root`、`unsupported-table-root`，同时保留 legacy `unsupported-root`，用于兼容和未来未知 root bucket。
2. extractor 会在 generic unsupported-element 和 visibility 检查之前分类 protected root。这保留了 ownership 边界，也能解释一个带 paint 的候选为什么没有进入 generic decorative extraction。
3. 实际抽取面没有变化。M24 接受的候选在 M25 仍然接受；本轮只改进 report。
4. 测试现在用真实 Chromium 覆盖 table/code/Mermaid/SVG protected root diagnostics，report 聚合测试覆盖 root-specific reason。
...
3. `npm test -- --runInBand src/tests/pptxExportReport.test.ts` 通过。
4. `runuser -u jacob -- env HOME=/home/jacob PLAYWRIGHT_BROWSERS_PATH=/home/jacob/.cache/ms-playwright bash -lc 'cd /home/jacob/obsidian-NotEMD && npm test -- --runInBand src/tests/pptxDomExtractor.test.ts'` 使用真实 Chromium 通过。
5. 全量 Jest 通过：`190` suites，`1547` tests。root 的 Playwright cache 仍会打印既有 browser skip warning；上面的 jacob-run DOM extractor 命令才是真实浏览器验证。
6. `git diff --check` 通过。

真实 `architecture.zh-CN.md` 验收：

1. 真实导出命令通过：`npm run verify:slidev-export -- --vault docs --source architecture.zh-CN.md --format pptx --output-subfolder export/test-slidev-m25-root-diagnostics --sample-slides all --timeout-ms 240000 --no-screenshots --require-pptx-visual-match --json`。
2. Slidev 来自本地 fork：`52.16.0 (/home/jacob/slidev/packages/slidev/bin/slidev.mjs)`，并使用 `skillRootPath = /home/jacob/slidev/skills/slidev`，`skillReferenceCount = 52`。
3. 视觉与源内容 gate 均通过：`ok = true`，`pptxVisualGate.passed = true`，`referenceSource = "pptx-rendered-html-reference"`，`thresholdProfile = "visible-native-rendered-html"`，`mermaidSourcePreservation.passed = true`，`sourceFenceCount = 3`，`deckFenceCount = 3`，`changedFenceIndexes = []`。
4. 布局审计保持干净：`slideCount = 30`，`overflowCount = 0`，`unreadableCount = 0`，`hardOverflowCount = 0`，`retryCount = 4`，`mermaidSlideCount = 3`，`mermaidLowZoomCount = 2`，`mermaidManualReviewCount = 1`。
5. editable 输出关键数据保持稳定：`textBoxCount = 338`，`tableCount = 6`，`editableSolidRectangleCount = 116`，`editableCodeBackgroundRectangleCount = 115`，`editableDecorativeRectangleCount = 1`，`editableDecorativeLineCount = 0`，`decorativePrimitiveCandidateCount = 374`，`decorativePrimitiveAcceptedCount = 1`，`decorativePrimitiveSkippedCount = 373`。
6. M24 的 `unsupported-root = 371` 现在变成可行动分布：`unsupported-code-root = 115`，`unsupported-svg-root = 136`，`unsupported-table-root = 120`，`not-visible = 2`。这个真实源文件没有出现 `unsupported-mermaid-root` 候选；Mermaid 仍出现在 `fallbackOnlyElementKinds`，因为 SVG 几何仍由背景拥有。
7. PPTX XML 扫描显示 `alpha=0 = 0`，`alpha=8000 = 0`，`Visible Native Text = 324`，`Visible Native Code Text = 14`，`Visible Native Table = 6`，`Native Code Background Rectangle = 115`，`Native Decorative Rectangle = 1`，`Native Decorative Line = 0`，transparent-only editable objects 为 `0`，`<a:t...>` tags 为 `861`。
8. 生成物仍 ignored 且未被 git 跟踪：`docs/export/test-slidev-m25-root-diagnostics/` 显示为 ignored，`git ls-files` 在该输出目录下没有 tracked 文件。

与先前方案的对比：

1. M24 要求下一步先拆 `unsupported-root`；这一点已完成。
2. 真实数据改变了下一步优先级。最大的 bucket 是 SVG-root paint，不是 Mermaid-root paint。code-root 和 table-root 也很大，但 table internals 已有 native table 表达，因此 table-root paint 应该先做 border/fill fidelity review，而不是走 generic decorative extraction。
3. `unsupported-code-root = 115` 大概率是最合适的下一步抽取候选，因为它能复用已有 code-background object model，并且来源域足够窄。但范围必须限定为 code-highlight paint，不是任意 `pre` 子节点。
4. `unsupported-svg-root = 136` 不应被理解为应该重建 SVG geometry。更合理方向是显式 SVG sidecar 或 SVG embedding artifact path，并在 report 中区分 embedded SVG editability 与 Office-native vector editability。

已知风险：

1. root-specific skip reason 仍然只是数量，不是影响评分。SVG-root 候选可能是一个视觉内部的许多实现细节。
2. 诊断按 nearest protected root ownership 分类，不等于用户语义。例如 table cell fill 和 table text overlay 都属于 table ownership，但修复路径不同。
3. 保留 legacy `unsupported-root` 可以避免 schema churn，但后续代码不应把新的已知 protected root 继续塞进这个桶，除非有明确理由。
4. 本轮没有让新的几何变成可编辑对象。它是 observability slice，用 editable shape count 评价它是错的。

下一步：

从已经拆开的 root bucket 中选择一个目标。最稳的下一步是 `unsupported-code-root` 下的 code-highlight paint extraction，因为它能扩展现有 code-background primitive，不触碰 Mermaid source 或 SVG geometry。SVG 应作为单独 artifact/embedding 轨道推进；table-root paint 则应先对照 native table writer 审计，再考虑是否需要 generic extraction。

## M26 透明文字路径移除

这一轮处理的是 PPTX 可用性上的具体失败：文件可以通过 visual diff，但展示文字仍然来自整页图片，真正可编辑的文字藏在图片下方。这不能作为默认导出的合同。现在的合同在关键层面采用 `oh-my-ppt` 的做法：先建模 native text/table object，在捕获背景前隐藏已建模 DOM，背景只承接未建模的视觉残留。

实现状态：

1. 默认 PPTX writer 不再保留透明 text-shape 生成路径；
2. 非可见默认文字源会被跳过，而不是写成具名透明 editable shape；
3. native table text 只通过可见 native table writer 输出，不再存在透明 table writer；
4. Mermaid/SVG label 在默认路径继续由背景拥有，除非显式 visible-native 实验负责它们；
5. report 类型合同不再允许旧的 `background-image`/`transparent`/`named-transparent-shapes` 默认值；
6. M25 的 root-specific 装饰诊断继续保留，用真实 code/table/SVG bucket 决定下一步扩大范围。

与先前方案对比：

1. M24/M25 主要提升 observability，没有收紧 writer 合同。M26 改的是合同本身，防止默认 writer 静默回到旧的 fake-editable fallback。
2. 本轮复用 `oh-my-ppt` 的重点是 DOM ownership 和 hide-before-capture；没有复用的是宽泛 SVG/Mermaid reconstruction。
3. 这不会让所有视觉元素都变成可编辑对象。它解决的是更关键的误导：显示文字是图片，隐藏文字却宣称可编辑。

验证：

1. `npx tsc --noEmit --pretty false` 通过。
2. `git diff --check` 通过。
3. `npm test -- --runInBand src/tests/pptxWriter.test.ts src/tests/pptxExportReport.test.ts` 通过：`21` 个测试。
4. `runuser -u jacob -- env HOME=/home/jacob PLAYWRIGHT_BROWSERS_PATH=/home/jacob/.cache/ms-playwright bash -lc 'cd /home/jacob/obsidian-NotEMD && npm test -- --runInBand src/tests/pptxDomExtractor.test.ts'` 使用真实 Chromium 通过：`10` 个测试。
5. `npm run build` 通过。
6. 全量 Jest 通过：`190` suites，`1547` tests。root 运行仍会打印既有 Playwright cache skip warning；上面的 jacob-run DOM extractor 命令才是真实浏览器覆盖。
7. 真实 `architecture.zh-CN.md` 导出通过，并确认使用本地 Slidev fork `52.16.0 (/home/jacob/slidev/packages/slidev/bin/slidev.mjs)`、`skillRootPath = /home/jacob/slidev/skills/slidev`、`skillReferenceCount = 52`。
8. 真实 PPTX gate 通过：`ok = true`，`pptxVisualGate.passed = true`，`referenceSource = pptx-rendered-html-reference`，`thresholdProfile = visible-native-rendered-html`，`mermaidSourcePreservation.passed = true`，`sourceFenceCount = 3`，`deckFenceCount = 3`，`changedFenceIndexes = []`。
9. 真实 report 确认 visible-native 输出：`visibleTextLayer = native-text-and-background-image`，`editableLayerRenderMode = visible-native-text`，`editableTextShapeFill = visible`，`editableTableTextFill = visible`，`transparentOverlayTextSources = []`，`backgroundTextPolicy = hide-modeled-text-before-capture`，`mermaidSvgTextPolicy = background-image-only`。
10. native object 数量稳定：`slideCount = 30`，`textBoxCount = 338`，`tableCount = 6`，`visible native text = 324`，`visible native code text = 14`，`visible native table = 6`，`editableCodeBackgroundRectangleCount = 115`。
11. PPTX XML 扫描确认透明路径已经消失：`alpha=0 = 0`，`alpha=8000 = 0`，具名 `Editable ... Text = 0`，具名 `Editable Table = 0`，`<a:t...>` tags = `861`。
12. 背景残留采样通过：`sampledSlideCount = 30`，`checkedRegionCount = 437`，`suspiciousSlideCount = 0`，`suspiciousRegionCount = 0`，`maxTextLikePixelRatio = 0`。
13. 输出产物保留在本地 `docs/export/test-slidev-m26-visible-native-no-transparent/`：prepared Slidev markdown、`index.html`、`index-standalone.html`、PPTX、report、rendered-HTML reference PNG 与 diff sheets。该目录仍被 ignored，`git ls-files` 在该目录下没有 tracked 文件。
