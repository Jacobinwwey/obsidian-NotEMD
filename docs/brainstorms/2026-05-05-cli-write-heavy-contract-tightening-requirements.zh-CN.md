...
- `src/fileUtils.ts`，这里仍然集中了最重的写入侧行为：processed-file 保存、标题生成、Mermaid 修复、重复清理与 error-log persistence
...
因此，下一步最稳妥的动作应是把同样的 contract-tightening 模式推进到 `src/fileUtils.ts`，而不是回头重开已经落地的 translate/formula 切片，也不是现在就强行上全局抽象。
...
### 方案一：下一批直接收紧 `src/fileUtils.ts`

直接进入 `processFile`、`generateContentForTitle`、`batchGenerateContentForTitles`、`batchFixMermaidSyntaxInFolder` 与 `checkAndRemoveDuplicateConceptNotes`，等更大的 write-heavy 语义稳定后，再回头处理 direct command surfaces。

- 优点：直接攻击当前最大副作用集中区
- 缺点：`src/main.ts` 还会继续保留少量 direct surfaces 一轮
- 风险：第一批 `src/fileUtils.ts` 收口会同时暴露多种行为的结果命名差异
...
`content.extract-original-text`、`translate.*` 与 `formula.*` 已经提供了清晰 proof slice。现在最缺的证据不是“还能不能再搬 wrapper”，而是同样的 host-owned-success + family-local-result 模式能否在更大的 `src/fileUtils.ts` 家族上稳定成立。

## 需求

**当前事实同步**
- R1. 当前进度文档与架构文档必须把 `translate.*` 与 `formula.*` 描述为已交付 proof slice，而不是待完成工作。
...
**短期收口**
- R3. 下一批 P0 实施必须优先处理 `src/fileUtils.ts` 的 write-heavy flows，重点包括 `processFile`、`generateContentForTitle`、`batchGenerateContentForTitles`、`batchFixMermaidSyntaxInFolder` 与 `checkAndRemoveDuplicateConceptNotes`。
- R4. 这些流程必须返回结构化结果；在适用场景下显式覆盖 `created` / `overwritten` / `moved` / `skipped`、输出路径、聚合错误，以及对自动化足够可见的破坏性副作用。
...
- R6. `src/operations/registry.ts`、`src/operations/capabilityManifest.ts` 与 `src/cliContracts.ts` 必须同步更新，以承接 R4-R5 的 richer result schema；只有在 deterministic、非 UI 绑定时才允许引入可选输入。
...
- 维护者不需要再做代码考古，就能直接说清楚后三波架构推进顺序：先 `src/fileUtils.ts`，再剩余 direct surfaces，最后 packaging / semantic verification 后续硬化。
- 当前文档不再把 translation 或 formula contract tightening 写成待办。
- 下一轮 planning 可以直接从 `src/fileUtils.ts` batch 开始，而不需要重新发明 why、ordering 或 scope boundary。
...
- 把已交付的 `translate/formula` 切片当成证明，而不是下一目标。
...
- `content.extract-original-text`、`translate.*` 与 `formula.*` 已经提供目标模式：utility core 输出 richer result，成功 notice 交给 host adapter。
- 当前 registry 已覆盖下一批所需 operation IDs；剩余工作主要是 contract 深度与 host-side effect 上提。

## 未决问题

### 延后到规划阶段
- [影响 R4][Technical] `src/fileUtils.ts` 的哪些家族应在第一批里显式区分 `created`、`overwritten`、`moved`，哪些可以先保持 aggregate-only？
- [影响 R5][Technical] Mermaid repair 与 duplicate cleanup 是否要立即统一结果词汇，还是先保持 family-local 直到批次稳定？
