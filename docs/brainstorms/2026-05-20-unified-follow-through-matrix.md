...
| F. Release / repo-saga / clean-state hygiene | Current main now has release/repo-saga scripts plus shared repo-saga execution lock, tests, docs, local-artifact ignore guardrails, chronicle refresh authorship protection, and the current branch is clean again at shipped `1.9.2` after remote-main resync | Backup branch motivated the guardrails; the bounded serial-safety slice plus later release-truth tightening are now back on current main | Preserve serial execution discipline for repo-saga refresh flows, keep release-facing truth synchronized to the actually shipped branch head, and keep clean-state proof as a maintained finish invariant rather than letting it slip back into deferred cleanup work | Do not confuse “script still exists” with permission to run repo-saga refresh paths in parallel, and do not reintroduce “we will clean it up later” finish drift or stale version-truth wording | P0 |
