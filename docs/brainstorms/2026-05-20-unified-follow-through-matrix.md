...
1. live `origin/main` after the 2026-05-24 force rewrite, re-audited against the current `main` working tree on 2026-06-06 at `87a5a4c` (`1.9.2`);
...
| F. Release / repo-saga / clean-state hygiene | Current main now has release/repo-saga scripts plus shared repo-saga execution lock, tests, docs, local-artifact ignore guardrails, chronicle refresh authorship protection, shared release-contract ownership for assets/tags/notes/workflow-source branch/chronicle-target branch, and the current branch is clean again at shipped `1.9.2` after remote-main resync | Backup branch motivated the guardrails; the bounded serial-safety slice plus later release-truth tightening are now back on current main | Preserve serial execution discipline for repo-saga refresh flows, keep release-facing truth synchronized to the actually shipped branch head, keep workflow-source and chronicle-target branch truth contract-backed, and keep clean-state proof as a maintained finish invariant rather than letting it slip back into deferred cleanup work | Do not confuse “script still exists” with permission to run repo-saga refresh paths in parallel, do not let YAML-local branch defaults drift away from repo-owned release contracts, and do not reintroduce “we will clean it up later” finish drift or stale version-truth wording | P0 |
