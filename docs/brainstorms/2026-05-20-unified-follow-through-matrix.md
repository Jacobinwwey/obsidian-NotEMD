...
2. The regex / file selection / local-KB / chapter split lane has therefore moved from “maintainer examples still need alignment” to “helper/docs/tests are aligned to the current fixture lane’s real note/query diversity.” When that lane grows again, prioritize failure-state, low-signal query, and noisy-corpus evidence before adding task count.
