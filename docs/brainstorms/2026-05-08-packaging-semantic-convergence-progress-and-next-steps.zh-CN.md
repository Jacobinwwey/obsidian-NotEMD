...
2. 这层 process-level 覆盖现在会证明：
   ...
   - 隔离的 `--no-readme --tag <tag>` 真实运行仍会产出多语言编年史 SVG 与英文别名，同时保持 README 文件不被改动；
   - 缺失 `--tag` 值与未知参数现在会在触碰 repo-saga 状态前快速失败；
   - 这些入口行为都能在不依赖真实上游 clone/fetch/build 周期的前提下被稳定验证。
...
### 2026-06-09 Current-Main Progress-Sync 增量

这条轨道接下来的漂移风险已经不再主要来自代码，而是来自 current-main 真值文档之间的所有权断层。

1. `docs/brainstorms/2026-05-20-*` 与 `docs/brainstorms/2026-05-28-*` 这两组 canonical progress 文档，现在已经把上面的 helper-entrypoint proof 视为 current main 的已落地证据，而不是待完成跟进；
2. `docs/superpowers/plans/2026-04-14-diagram-rendering-platform-roadmap.*` 现在也会把：
   - 任务 0 读成仍然活跃的 packaging/source-build 瓶颈；
   - 任务 2 读成 compatibility-surface 收口，而不是 pipeline 是否存在；
   - 任务 3 读成带真实 Obsidian 验证门槛的 MermaidProcessor 降责；
3. 因此，在当前主线上，packaging 轨道仍然是首要架构瓶颈；release / chronicle / repo-saga helper 的入口级证明，则是支撑这条轨道的 contract-depth 证据，而不是另一条独立架构主线。

正确解释：

1. 这次仍然没有扩大已发货 topology；
2. 但它又补上了一类漂移控制：让 roadmap/progress 真值开始跟随同一套已检入 helper-entrypoint 证据，而不是只靠人工记忆。
