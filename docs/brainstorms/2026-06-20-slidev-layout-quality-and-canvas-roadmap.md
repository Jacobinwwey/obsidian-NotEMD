...
25. Real Stage 15 `architecture.zh-CN.md` strict native standalone archive: `/home/jacob/slidev-export-review/2026-06-21-stage15-final-rerun/`. The report is `ok = true`, uses `/home/jacob/slidev/packages/slidev/bin/slidev.mjs`, loads `/home/jacob/slidev/skills/slidev` with 52 references, outputs native standalone HTML, preserves 3/3 Mermaid fences with `changedFenceIndexes = []`, and closes with `slideCount = 27`, `hardOverflowCount = 0`, `lowEffectiveFontCount = 0`, `mermaidFitReviewCount = 3`, `mermaidLowZoomCount = 2`, and `mermaidManualReviewCount = 1`. The reviewable deck is archived as `architecture.stage15.slidev.zh-CN.md` and tracked in-repo at `docs/slidev/architecture.stage15.slidev.zh-CN.md`; runtime HTML/assets/screenshots are not committed.
...
| Mermaid content must not be modified | Prompt, plan, patcher, tests, source-preparation LLM candidate rejection, and verifier are source-preserving, including fence metadata | Keep exact fence comparison as a required gate; do not add Mermaid auto-splitting |
