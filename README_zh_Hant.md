...
這個季度版編年史保留了 [repo-saga](https://github.com/teee32/repo-saga) 的原始視覺樣式，而且季度切分現在直接由 repo-saga 自身生成。
...
Notemd 的設定頁面提供模糊搜尋功能、大型類別導航，以及為當前 Vault 儲存的每個設定的收藏項目。

- 圖表歷史記錄會儲存在 Vault 級別，以最新順序排列，可進行搜尋，並以 20 個為一組分頁顯示。移除某條歷史記錄並不會刪除已生成的檔案。
- 圖表預覽的匯出 PPI 設定可控制 PNG 和 PDF 的清晰度，而 SVG 則仍維持向量格式。
