...
Документация на других языках: [Языковой центр](./docs/i18n/README.md)
...
Если вам нравится использовать Notemd, пожалуйста, поставьте [⭐ звезду на GitHub](https://github.com/Jacobinwwey/obsidian-NotEMD) или [☕️ купите мне кофе](https://ko-fi.com/jacobinwwey).
...
3. **Откройте боковую панель**: нажмите на значок волшебной палочки Notemd в ленте слева, чтобы открыть боковую панель.
...
5. **Запустите быстрый рабочий процесс**: используйте стандартную кнопку **"One-Click Extract"**, чтобы одной точкой входа запустить обработку, пакетную генерацию и очистку Mermaid.
...
|---|---|---|---|
| `Язык интерфейса` | Только текст интерфейса плагина (настройки, боковая панель, уведомления, диалоги) | `auto` | Следует за языком Obsidian. Текущие UI-каталоги: `en`, `ar`, `de`, `es`, `fa`, `fr`, `id`, `it`, `ja`, `ko`, `nl`, `pl`, `pt`, `pt-BR`, `ru`, `th`, `tr`, `uk`, `vi`, `zh-CN`, `zh-TW`. |
| `Язык вывода задач` | Сгенерированный моделью вывод задач (ссылки, резюме, генерация, извлечение, целевой язык перевода) | `en` | Может быть глобальным или отдельным для каждой задачи, если включён режим `Использовать разные языки для задач`. |
| `Отключить автоперевод` | Не-Translate задачи сохраняют контекст исходного языка | `false` | Явные задачи `Translate` всё равно принудительно используют настроенный целевой язык. |
...
- Дополнительные детали и правила внесения вкладов отслеживаются в [Языковом центре](./docs/i18n/README.md).
...
- **Отслеживание прогресса**: обновления в реальном времени через боковую панель Notemd или модальное окно прогресса.
- **Отменяемые операции**: любую задачу обработки, запущенную из боковой панели, можно отменить отдельной кнопкой Cancel. Команды из палитры команд используют модальное окно, которое тоже можно отменить.
...
- **Панель режима разработчика**: в настройках появилась отдельная диагностическая панель только для разработчиков, скрытая до включения "Developer mode". Она позволяет выбирать диагностический путь вызова и запускать повторные проверки стабильности для выбранного режима.
- **Переработанная боковая панель**: встроенные действия сгруппированы по тематическим секциям с более ясными названиями, живым статусом, отменяемым прогрессом и копируемыми логами, чтобы уменьшить захламление боковой панели. Нижний блок с прогрессом и логами теперь остаётся видимым даже при раскрытии всех секций, а состояние готовности отображается понятнее.
- **Полировка взаимодействия и читаемости боковой панели**: кнопки боковой панели теперь дают более ясную обратную связь при наведении, нажатии и фокусе, а цветные CTA-кнопки (включая `One-Click Extract` и `Batch generate from titles`) используют более контрастный текст для лучшей читаемости в разных темах.
- **CTA только для действий с одним файлом**: цветной CTA-стиль теперь зарезервирован только для действий над одним файлом. Пакетные и папочные действия, а также смешанные рабочие процессы используют обычный стиль, чтобы снизить риск неверного запуска по области действия.
- **Собственные рабочие процессы в один клик**: встроенные утилиты боковой панели можно объединять в пользовательские кнопки с произвольными именами и цепочками действий. Стандартный рабочий процесс `One-Click Extract` включён из коробки.
...
- **Отключить автоперевод**: если этот параметр включён, не-Translate задачи больше не будут принудительно переводить output на заданный язык и сохранят контекст оригинального языка. Явная задача `Translate` продолжит перевод по настройкам.

### Веб-исследование и генерация контента
- **Веб-исследование и суммаризация**:
...
- **Генерация контента по заголовку**:
...
- **Суммаризировать в диаграмму Mermaid**:
...
1. **Активный провайдер**: выберите LLM-провайдера из выпадающего списка.
2. **Параметры провайдера**: настройте конкретные параметры выбранного провайдера:
   - **API-ключ**: обязателен для большинства облачных провайдеров (например, OpenAI, Anthropic, DeepSeek, Qwen, Qwen Code, Doubao, Moonshot, GLM, Z AI, MiniMax, Huawei Cloud MaaS, Baidu Qianfan, SiliconFlow, Google, Mistral, Azure OpenAI, OpenRouter, xAI, Groq, Together, Fireworks, Requesty). Для Ollama не нужен. Для LM Studio и универсального пресета `OpenAI Compatible` может быть необязательным, если endpoint допускает анонимный или placeholder-доступ.
   - **Базовый URL / endpoint**: API endpoint сервиса. Значения по умолчанию уже подставлены, но их может понадобиться изменить для локальных моделей (LMStudio, Ollama), шлюзов (OpenRouter, Requesty, OpenAI Compatible) или конкретных Azure deployment. Для Azure OpenAI поле **обязательно**.
   - **Модель**: конкретное имя/ID модели, например `gpt-4o`, `claude-3-5-sonnet-20240620`, `google/gemini-flash-1.5`, `grok-4`, `moonshotai/kimi-k2-instruct-0905`, `accounts/fireworks/models/kimi-k2p5`, `anthropic/claude-3-7-sonnet-latest`. Убедитесь, что модель доступна у вашего провайдера или endpoint.
   - **Температура**: управляет степенью случайности вывода модели (`0` = детерминированно, `1` = максимальная вариативность). Для структурных задач обычно лучше низкие значения, например `0.2-0.5`.
   - **Версия API (только Azure)**: обязательное поле для Azure OpenAI deployment, например `2024-02-15-preview`.
3. **Проверить подключение**: используйте кнопку "Проверить подключение" для активного провайдера, чтобы проверить настройки. OpenAI-compatible провайдеры теперь используют проверки с учётом типа провайдера: endpoint вроде `Qwen`, `Qwen Code`, `Doubao`, `Moonshot`, `GLM`, `Z AI`, `MiniMax`, `Huawei Cloud MaaS`, `Baidu Qianfan`, `SiliconFlow`, `Groq`, `Together`, `Fireworks`, `LMStudio` и `OpenAI Compatible` проверяют `chat/completions` напрямую, а провайдеры с надёжным `/models` endpoint по-прежнему могут сначала использовать список моделей. Если первая проверка падает из-за временного сетевого разрыва, например `ERR_CONNECTION_CLOSED`, Notemd автоматически переключается на стабильную последовательность повторов вместо немедленной ошибки.
4. **Управление конфигурациями провайдеров**: используйте кнопки "Export Providers" и "Import Providers", чтобы сохранять и загружать настройки LLM-провайдеров в файл `notemd-providers.json` внутри конфигурационной директории плагина и обратно. Это удобно для резервного копирования и совместного использования.
5. **Покрытие пресетов**: помимо исходных провайдеров, Notemd теперь включает готовые пресеты для `Qwen`, `Qwen Code`, `Doubao`, `Moonshot`, `GLM`, `Z AI`, `MiniMax`, `Huawei Cloud MaaS`, `Baidu Qianfan`, `SiliconFlow`, `xAI`, `Groq`, `Together`, `Fireworks`, `Requesty` и универсальной цели `OpenAI Compatible` для LiteLLM, vLLM, Perplexity, Vercel AI Gateway или кастомных прокси.
...
- **Использовать разные провайдеры для задач**:
  - **Отключено (по умолчанию)**: для всех задач используется один «активный провайдер».
  - **Включено**: можно выбрать отдельного провайдера и, при желании, переопределить имя модели для каждой задачи ("Add Links", "Research & Summarize", "Generate from Title", "Translate", "Extract Concepts"). Если поле переопределения модели для задачи оставить пустым, будет использоваться модель по умолчанию, заданная у выбранного для этой задачи провайдера.
- **Выбирать разные языки для разных задач**:
  - **Отключено (по умолчанию)**: для всех задач используется единый «язык вывода».
  - **Включено**: позволяет задать отдельный язык для каждой задачи ("Add Links", "Research & Summarize", "Generate from Title", "Summarise as Mermaid diagram", "Extract Concepts").
...
### Языковая архитектура (язык интерфейса и язык вывода задач)

- **Язык интерфейса** управляет только текстом интерфейса плагина: подписями в настройках, кнопками боковой панели, уведомлениями и диалогами.
- Региональные и письменные варианты теперь сопоставляются с ближайшим опубликованным каталогом, а не сразу откатываются на английский. Например, `fr-CA` использует французский, `es-419` — испанский, `pt-PT` — португальский, `zh-Hans` — упрощённый китайский, а `zh-Hant-HK` — традиционный китайский.
- **Язык вывода задач** управляет выводом, генерируемым моделью: ссылками, сводками, генерацией заголовков, Mermaid-сводкой, извлечением понятий и целевым языком перевода.
- **Режим языка по задачам** позволяет каждой задаче определять собственный язык вывода через единый слой политики, а не через разбросанные переопределения по модулям.
- **Отключить автоперевод** сохраняет контекст исходного языка для задач, отличных от Translate, при этом явные задачи Translate по-прежнему используют заданный целевой язык.
- Все Mermaid-связанные пути генерации следуют той же языковой политике и при необходимости по-прежнему могут запускать автоматическое исправление Mermaid.

### Настройки стабильных API-вызовов
- **Включить стабильные API-вызовы (логика повторов)**:
  - **Отключено (по умолчанию)**: единичная ошибка API немедленно останавливает текущую задачу.
  - **Включено**: неудачные LLM API-вызовы автоматически повторяются, что полезно при нестабильной сети или ограничениях по частоте.
  - **Резервный путь проверки подключения**: даже если обычные вызовы не работают в стабильном режиме, test connection после первой временной сетевой ошибки переключается на тот же сценарий повторов.
  - **Резервный транспорт во время выполнения (с учётом среды)**: длительные запросы задач, временно оборванные `requestUrl`, сначала повторяют ту же попытку через резервный транспорт с учётом среды. На desktop это Node `http/https`, вне desktop — браузерный `fetch`. Такие резервные попытки используют потоковый разбор с учётом протокола для OpenAI-compatible SSE, Azure OpenAI SSE, Anthropic Messages SSE, Google Gemini SSE и Ollama NDJSON, так что медленные gateway успевают вернуть части тела ответа раньше. Остальные прямые OpenAI-style entrypoint используют этот же общий резервный путь.
  - **Порядок стабильного режима для OpenAI-compatible**: в стабильном режиме каждая попытка для OpenAI-compatible теперь идёт по порядку `direct streaming -> direct non-stream -> requestUrl (со streamed fallback при необходимости)` и только потом засчитывается как неудачная. Это снижает число слишком агрессивных отказов, когда нестабилен только один transport mode.
- **Интервал повторов (секунды)**: отображается только при включённой стабильности. Задает время между попытками повтора, диапазон 1-300 секунд. По умолчанию: 5.
- **Максимальное число повторов**: отображается только при включённой стабильности. Максимум попыток повтора, диапазон 0-10. По умолчанию: 3.
- **Режим отладки ошибок API**:
  - **Отключено (по умолчанию)**: обычная краткая отчётность об ошибках.
  - **Включено**: включает подробный журнал ошибок для всех провайдеров и задач, в том числе Translate, Search и Connection Tests. Сохраняются HTTP status code, сырой response text, временная шкала транспорта, обезличенные request URL и headers, длительность попытки, response headers, частичные response body, частично разобранный stream output и stack trace.
- **Режим разработчика**:
  - **Отключено (по умолчанию)**: скрывает все диагностические элементы управления только для разработчиков.
  - **Включено**: показывает отдельную панель диагностики разработчика в настройках.
- **Диагностика провайдера для разработчика (долгий запрос)**:
  - **Режим диагностического вызова**: позволяет выбрать путь выполнения для проверки. Для OpenAI-compatible доступны принудительные режимы (`direct streaming`, `direct buffered`, `requestUrl-only`) помимо обычных режимов выполнения.
  - **Запустить диагностику**: запускает одну проверку долгого запроса в выбранном режиме вызова и создаёт `Notemd_Provider_Diagnostic_*.txt` в корне vault.
  - **Запустить тест стабильности**: повторяет проверку заданное число раз (1-10) в выбранном режиме вызова и сохраняет агрегированный отчёт о стабильности.
  - **Тайм-аут диагностики**: настраиваемый тайм-аут на один запуск (15-3600 секунд).
  - **Почему это полезно**: такая проверка быстрее ручного воспроизведения, когда провайдер проходит "Test connection", но падает на реальных длинных задачах, например при переводе через медленный gateway.
...
- **Customize Processed File Save Path**:
  ...
  - **Включено**: позволяет указать собственный путь сохранения.
...
- **Use Custom Output Filename for 'Add Links'**:
  ...
  - **Включено**: позволяет настроить output filename через настройку ниже.
...
- **Remove Code Fences on Add Links**:
  ...
  - **Включено**: удаляет code fence перед обработкой Add Links.
...
#### Вывод файла журнала понятий
- **Generate Concept Log File**:
  - **Disabled (по умолчанию)**: log file не создаётся.
  - **Включено**: после обработки создаётся log file со списком новых concept-note в формате:
    ```
    создать xx md-файлов с понятиями
    1. concepts1
    2. concepts2
    ...
    n. conceptsn
...
- **Customize Log File Save Path**: отображается только если включён "Generate Concept Log File".
  ...
  - **Включено**: позволяет задать отдельную папку для log file.
...
- **Customize Log File Name**: отображается только если включён "Generate Concept Log File".
  ...
  - **Включено**: позволяет указать своё имя файла.
...
#### Задача извлечения понятий
- **Создавать минимальные concept note**:
  - **Включено (по умолчанию)**: новые concept-note будут содержать только заголовок, например `# Concept`.
  - **Выключено**: concept-note могут включать дополнительное содержимое, например обратную ссылку "Linked From", если это не запрещено настройкой ниже.
- **Добавлять обратную ссылку "Linked From"**:
  - **Выключено (по умолчанию)**: обратная ссылка на исходный документ во время извлечения не добавляется.
  - **Включено**: добавляет секцию "Linked From" со ссылкой на исходный файл.

#### Извлечение конкретного исходного текста
...
- **Enable Batch Parallelism**:
  ...
  - **Включено**: позволяет обрабатывать несколько файлов параллельно, что существенно ускоряет крупные batch-job.
...
- **Customise Translation File Save Path**:
  ...
  - **Включено**: позволяет указать **относительный путь** внутри vault, например `Translations`, куда будут сохраняться translated file. Папки будут созданы автоматически.
- **Use custom suffix for translated files**:
  ...
  - **Включено**: позволяет указать свой суффикс.
...
- **Enable Research in "Generate from Title"**:
  ...
  - **Включено**: перед генерацией выполняется web research через настроенный **Web Research Provider**, а результаты передаются модели как контекст.
- **Auto-run Mermaid Syntax Fix after Generation**:
  - **Включено (по умолчанию)**: после Mermaid-related workflow, таких как Process, Generate from Title, Batch Generate from Titles, Research & Summarize, Summarise as Mermaid и Translate, автоматически запускается проход исправления Mermaid syntax.
...
- **Use Custom Output Folder for 'Generate from Title'**:
  ...
  - **Включено**: позволяет указать своё имя подпапки, куда будут перемещаться completed file.
...
#### Кнопки рабочих процессов в один клик
...
- **Enable Custom Prompts for Specific Tasks**:
  ...
  - **Включено**: включает возможность задавать custom prompt для задач, перечисленных ниже. Это мастер-переключатель всей функции.
- **Use Custom Prompt for [Task Name]**:
  ...
  - **Включено**: задача использует текст, который вы укажете в соответствующем поле "Custom Prompt".
...
- **Enable Focused Learning Domain**:
  ...
  - **Включено**: позволяет указать одну или несколько предметных областей, чтобы улучшить контекстное понимание модели.
...
### Быстрые рабочие процессы и боковая панель

- Откройте боковую панель Notemd, чтобы получить доступ к сгруппированным разделам действий для основной обработки, генерации, перевода, работы со знаниями и утилит.
- Используйте область **Быстрые сценарии** в верхней части боковой панели для запуска пользовательских многошаговых кнопок.
- Стандартный рабочий процесс **One-Click Extract** запускает `Process File (Add Links)` -> `Batch Generate from Titles` -> `Batch Mermaid Fix`.
- Ход рабочего процесса, журналы по шагам и ошибки отображаются прямо в боковой панели; закреплённый нижний блок не даёт индикатору прогресса и области логов исчезнуть при раскрытии секций.
- Карточка прогресса сохраняет текст состояния, отдельную плашку с процентом и оставшееся время в читаемом виде, а те же пользовательские рабочие процессы можно перенастраивать из настроек.
...
1. **Через боковую панель**:
   ...
   - Прогресс отображается в боковой панели. Задачу можно отменить кнопкой "Cancel Processing".
   - *Примечание по обработке папок:* файлы обрабатываются в фоне и не открываются в редакторе.
...
1. **Суммаризировать в диаграмму Mermaid**:
...
2. **Translate Note/Selection**:
   ...
   - Запустите `Notemd: Translate Note/Selection` через палитру команд или кнопку в боковой панели.
   - Появится модальное окно, в котором можно подтвердить или изменить **Target Language**; по умолчанию используется значение из настроек.
   - Для перевода плагин использует настроенный **LLM Provider** с учётом настроек нескольких моделей.
   ...
   - Задачу можно отменить через кнопку в боковой панели или кнопку отмены в модальном окне.
...
4. **Research & Summarize Topic**:
   - Выделите текст в заметке **или** убедитесь, что у заметки есть заголовок; он и станет темой поиска.
   - Запустите `Notemd: Research and Summarize Topic` через палитру команд или кнопку в боковой панели.
   - Плагин использует настроенный **Search Provider** (Tavily/DuckDuckGo) и соответствующий **LLM Provider** с учётом настроек нескольких моделей, чтобы найти и суммаризировать информацию.
   - Сгенерированная сводка дописывается в текущую заметку.
   - Задачу можно отменить через кнопку в боковой панели или через модальное окно.
...
5. **Generate Content from Title**:
   ...
   - Запустите `Notemd: Generate Content from Title` через палитру команд или кнопку в боковой панели.
   - Плагин использует подходящий **LLM Provider** согласно настройкам нескольких моделей, чтобы сгенерировать контент по заголовку заметки, заменяя существующее содержимое.
   - Если включена настройка **"Enable Research in 'Generate from Title'"**, перед генерацией выполняется веб-исследование через выбранный **Web Research Provider**, а результаты включаются в prompt.
   - Задачу можно отменить через боковую панель или модальное окно.

6. **Batch Generate Content from Titles**:
   - Запустите `Notemd: Batch Generate Content from Titles` через палитру команд или кнопку в боковой панели.
   - Выберите папку с заметками, которые хотите обработать.
   - Плагин пройдёт по каждому `.md` файлу в папке, исключая `_processed.md` и файлы внутри назначенной папки "complete", сгенерирует контент по заголовку и заменит существующее содержимое. Файлы обрабатываются в фоне и не открываются в редакторе.
   - Успешно обработанные файлы перемещаются в настроенную папку "complete".
   - Команда учитывает настройку **"Enable Research in 'Generate from Title'"** для каждого файла.
   - Задачу можно отменить через боковую панель или модальное окно.
   - Прогресс и результаты (число изменённых файлов, ошибки) выводятся в журнал боковой панели или модального окна.
...
7. **Check and Remove Duplicate Concept Notes**:
   ...
   - Запустите `Notemd: Check and Remove Duplicate Concept Notes` через палитру команд или кнопку в боковой панели.
   ...
   - Если потенциальные дубликаты найдены, открывается модальное окно со списком файлов, причиной попадания и конфликтующими файлами.
   ...
   - Ход и результат операции отображаются в журнале боковой панели или модального окна.
...
8. **Extract Concepts (Pure Mode)**:
   ...
   - **Один файл**: откройте файл и запустите `Notemd: Extract concepts (create concept notes only)` через палитру команд или нажмите кнопку **"Extract concepts (current file)"** в боковой панели.
   - **Папка**: запустите `Notemd: Batch extract concepts from folder` через палитру команд или нажмите **"Extract concepts (folder)"** в боковой панели, затем выберите папку для обработки всех заметок внутри неё.
...
10. **Extract Concepts and Generate Titles**:
    - Эта команда объединяет две мощные функции в один рабочий процесс.
    ...
    - Такой рабочий процесс позволяет сначала быстро пополнить базу знаний новыми concept-note из исходного документа, а затем сразу наполнить эти заметки AI-содержимым за один запуск.
...
### Диагностика для разработчиков и журнал отладки (опционально)
...
- **Проблемы соединения с LM Studio/Ollama**:
  - **Проверка соединения не проходит**: убедитесь, что локальный сервер запущен и нужная модель действительно загружена/доступна.
...
- Каждый проект сохраняет собственную лицензию и условия; Notemd доступен по [лицензии MIT](./LICENSE).
...
- [Процесс релиза (английский)](./docs/maintainer/release-workflow.md)
- [Процесс релиза (упрощённый китайский)](./docs/maintainer/release-workflow.zh-CN.md)
...
*Notemd v1.9.6 — усиливайте граф знаний Obsidian с помощью ИИ.*
...
_Последнее обновление для тега релиза `1.9.6`: 2026-08-09. Дата последнего коммита: 2026-08-09._
<!-- repo-chronicle:end -->
...
## Обнаружение настроек, история диаграмм, CircuitikZ и безопасные папки пакетной обработки

Страница настроек Notemd предоставляет поиск с учётом содержимого полей, навигацию по большому количеству категорий, складываемую панель инструментов для обнаружения настроек, а также функцию добавления в избранное для каждой настройки, сохраняемую для текущего Vault. Нажмите **★ Favorites**, чтобы открыть специальный список; каждая запись переносит пользователя к соответствующей настройке и может быть удалена без выхода с страницы настроек.

- История диаграмм хранится в области Vault, отсортированная по новизне; её можно искать и просматривать по группам по 20 записей. Удаление записи из истории не удаляет сгенерированные файлы.
- Экспорт предварительного просмотра диаграммы в формате PPI производится по умолчанию в разрешении 300 пикселей, но поддерживает значения от 72 до 600. Этот параметр влияет только на растровую обработку PNG; SVG и PDF остаются векторными.
- При экспорте полных визуальных элементов Mermaid по желанию также записываются исходный код Mermaid, файлы SVG и сопутствующие манифесты; даже при отключении Mermaid он остаётся доступен в превью формата Drawnix.
- Тип диаграммы и формат исходного файла выбираются отдельно друг от друга.
- Пользователи на ПК могут использовать необязательную нативную среду компиляции CircuitikZ для повторного использования системных инструментов Tectonic/pdflatex, выбрать собственный компилятор или явно установить фиксированную версию среды выполнения Tectonic. Для превью, SVG, PNG и экспорта в формате PDF LaTeX не требуется.
- Расширенный режим выбора файлов пакетной обработки позволяет использовать сохранённые профили выбора и просматривать правила обработки.
- Если папка назначения для пакетной обработки отсутствует, её можно создать после подтверждения; также есть опция запоминания автоматического создания таких папок в будущем.
...
- Режим разработчика показывает диагностическую информацию по поставщикам сервисов и предоставляет расширенные инструменты для устранения неполадок.
