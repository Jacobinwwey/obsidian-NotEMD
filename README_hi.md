...
दूसरी भाषाओं में दस्तावेज़ पढ़ें: [Language Hub](./docs/i18n/README.md)
...
- आधिकारिक दस्तावेज़ अंग्रेज़ी और सरलीकृत चीनी में बनाए रखे जाते हैं, और 30 से अधिक भाषाओं के लिए पूर्ण समर्थन उपलब्ध है।
- सभी समर्थित भाषाएँ ऊपर हेडर में लिंक की गई हैं।
- अधिक विवरण और योगदान दिशानिर्देश [Language Hub](./docs/i18n/README.md) में उपलब्ध हैं।
...
- **Disable auto translation**: जब यह विकल्प सक्षम होता है, तो गैर-`Translate` कार्य अब आउटपुट को किसी विशिष्ट भाषा में बाध्य नहीं करते, जिससे मूल भाषा संदर्भ सुरक्षित रहता है। स्पष्ट "Translate" कार्य फिर भी कॉन्फ़िगरेशन के अनुसार अनुवाद करेगा।
...
1.  **Active Provider**: dropdown menu से वह LLM provider चुनें जिसे आप उपयोग करना चाहते हैं।
2.  **Provider Settings**: चयनित provider के लिए विशिष्ट settings कॉन्फ़िगर करें:
...
3.  **Test Connection**: सक्रिय provider के लिए "Test Connection" बटन का उपयोग करके अपनी settings सत्यापित करें। OpenAI-compatible providers अब provider-aware checks का उपयोग करते हैं: `Qwen`, `Qwen Code`, `Doubao`, `Moonshot`, `GLM`, `Z AI`, `MiniMax`, `Huawei Cloud MaaS`, `Baidu Qianfan`, `SiliconFlow`, `Groq`, `Together`, `Fireworks`, `LMStudio` और `OpenAI Compatible` जैसे endpoints सीधे `chat/completions` को probe करते हैं, जबकि विश्वसनीय `/models` endpoint वाले providers पहले model listing का उपयोग कर सकते हैं। यदि पहली probe `ERR_CONNECTION_CLOSED` जैसी transient network disconnect से विफल होती है, तो Notemd तुरंत विफल होने के बजाय automatic stable retry sequence पर चला जाता है।
4.  **Manage Provider Configurations**: "Export Providers" और "Import Providers" बटन का उपयोग करके अपने LLM provider settings को plugin configuration directory के भीतर `notemd-providers.json` फ़ाइल में save/load करें। इससे backup और sharing आसान हो जाती है।
5.  **Preset Coverage**: मूल providers के अतिरिक्त, Notemd अब `Qwen`, `Qwen Code`, `Doubao`, `Moonshot`, `GLM`, `Z AI`, `MiniMax`, `Huawei Cloud MaaS`, `Baidu Qianfan`, `SiliconFlow`, `xAI`, `Groq`, `Together`, `Fireworks`, `Requesty` और LiteLLM, vLLM, Perplexity, Vercel AI Gateway या custom proxies के लिए generic `OpenAI Compatible` target शामिल करता है।
...
-   **Use Different Providers for Tasks**:
    *   **Disabled (Default)**: सभी कार्यों के लिए ऊपर चुने गए एक ही "Active Provider" का उपयोग करता है।
    *   **Enabled**: आपको प्रत्येक कार्य ("Add Links", "Research & Summarize", "Generate from Title", "Translate", "Extract Concepts") के लिए अलग provider चुनने और वैकल्पिक रूप से model name override करने देता है। यदि किसी task का model override field खाली छोड़ा जाता है, तो उस task के चयनित provider का डिफ़ॉल्ट मॉडल उपयोग होगा।
-   **Select different languages for different tasks**:
    *   **Disabled (Default)**: सभी कार्यों के लिए एक ही "Output language" उपयोग करता है।
    *   **Enabled**: आपको प्रत्येक कार्य ("Add Links", "Research & Summarize", "Generate from Title", "Summarise as Mermaid diagram", "Extract Concepts") के लिए अलग भाषा चुनने देता है।
...
### भाषा आर्किटेक्चर (UI Locale बनाम Task Output Language)

-   **UI Locale** केवल प्लगइन इंटरफ़ेस टेक्स्ट (Settings labels, sidebar buttons, notices और dialogs) को नियंत्रित करता है। डिफ़ॉल्ट `auto` mode Obsidian की वर्तमान UI भाषा का अनुसरण करता है।
-   **Task Output Language** मॉडल द्वारा उत्पन्न task output (लिंक, सारांश, title generation, Mermaid summary, concept extraction, translation target) को नियंत्रित करता है।
...
-   **Disable auto translation** गैर-`Translate` कार्यों को source-language context में रखता है, जबकि explicit `Translate` कार्य अभी भी configured target language लागू करते हैं।
...
-   **Enable Stable API Calls (Retry Logic)**:
    *   **Disabled (Default)**: एक API call failure वर्तमान task को रोक देगा।
    *   **Enabled**: विफल LLM API calls को स्वचालित रूप से retry करता है (intermittent network issues या rate limits के लिए उपयोगी)।
...
-   **API Error Debugging Mode**:
    *   **Disabled (Default)**: मानक, संक्षिप्त error reporting का उपयोग करता है।
    *   **Enabled**: सभी providers और tasks (Translate, Search और Connection Tests सहित) के लिए विस्तृत error logging सक्रिय करता है (DeepSeek के verbose output के समान)। इसमें HTTP status codes, raw response text, request transport timelines, sanitized request URLs और headers, elapsed attempt durations, response headers, partial response bodies, parsed partial stream output और स्टैक ट्रेस शामिल हैं, जो API connection issues और upstream gateway resets की troubleshooting के लिए महत्वपूर्ण हैं।
-   **Developer Mode**:
    *   **Disabled (Default)**: सभी developer-only diagnostic controls को सामान्य उपयोगकर्ताओं से छुपाता है।
    *   **Enabled**: Settings में समर्पित developer diagnostics panel दिखाता है।
...
-   **Customize Processed File Save Path**:
    *   **Disabled (Default)**: processed files (जैसे `YourNote_processed.md`) मूल नोट के *उसी folder* में सहेजी जाती हैं।
    *   **Enabled**: आपको custom save location निर्दिष्ट करने देता है।
...
-   **Use Custom Output Filename for 'Add Links'**:
    *   **Disabled (Default)**: 'Add Links' command द्वारा बनाई गई processed files डिफ़ॉल्ट `_processed.md` suffix का उपयोग करती हैं (जैसे `YourNote_processed.md`)।
    *   **Enabled**: आपको नीचे दी गई setting के माध्यम से output filename अनुकूलित करने देता है।
...
-   **Remove Code Fences on Add Links**:
    *   **Disabled (Default)**: link जोड़ते समय code fences **(\`\\\`\`)** content में रखे जाते हैं, और **(\`\\\`markdown)** अपने आप हटाया जाता है।
    *   **Enabled**: link जोड़ने से पहले content से code fences हटा देता है।
...
-   **Customize Concept Note Path**:
    *   **Disabled (Default)**: `[[linked concepts]]` के लिए notes का automatic creation अक्षम रहता है।
    *   **Enabled**: आपको वह folder निर्दिष्ट करने देता है जहाँ नई concept notes बनाई जाएँगी।
...
-   **Generate Concept Log File**:
    *   **Disabled (Default)**: कोई log file नहीं बनाई जाती।
    *   **Enabled**: प्रोसेसिंग के बाद नई concept notes सूचीबद्ध करने वाली log file बनाता है। प्रारूप इस प्रकार है:
        ```
...
-   **Customize Log File Save Path**: (केवल तब दिखाई देता है जब "Generate Concept Log File" enabled हो)
    *   **Disabled (Default)**: log file **Concept Note Folder Path** में (यदि निर्दिष्ट हो) या अन्यथा vault root में सहेजी जाती है।
    *   **Enabled**: आपको log file के लिए custom folder निर्दिष्ट करने देता है।
...
-   **Customize Log File Name**: (केवल तब दिखाई देता है जब "Generate Concept Log File" enabled हो)
    *   **Disabled (Default)**: log file का नाम `Generate.log` होता है।
    *   **Enabled**: आपको log file के लिए custom name निर्दिष्ट करने देता है।
...
-   **Enable Batch Parallelism**:
    *   **Disabled (Default)**: batch processing tasks (जैसे "Process Folder" या "Batch Generate from Titles") files को एक-एक करके (serially) प्रोसेस करते हैं।
    *   **Enabled**: प्लगइन को कई files एक साथ प्रोसेस करने देता है, जिससे बड़े batch jobs तेज़ हो सकते हैं।
...
-   **Enable Duplicate Detection**: processed content में duplicate words की बुनियादी जाँच को toggle करता है (परिणाम console में)। (Default: Enabled)
...
-   **Customise Translation File Save Path**:
    *   **Disabled (Default)**: translated files मूल नोट के *उसी folder* में सहेजी जाती हैं।
    *   **Enabled**: आपको अपने vault के भीतर एक *relative path* (जैसे `Translations`) निर्दिष्ट करने देता है जहाँ translated files सहेजी जाएँगी। यदि folders मौजूद न हों तो बनाए जाएँगे।
-   **Use custom suffix for translated files**:
    *   **Disabled (Default)**: translated files डिफ़ॉल्ट `_translated.md` suffix का उपयोग करती हैं (जैसे `YourNote_translated.md`)।
    *   **Enabled**: आपको custom suffix निर्दिष्ट करने देता है।
...
-   **Enable Research in "Generate from Title"**:
    *   **Disabled (Default)**: "Generate from Title" केवल title को input के रूप में उपयोग करता है।
    *   **Enabled**: configured **Web Research Provider** का उपयोग करके web research करता है और निष्कर्षों को title-based generation के दौरान LLM के लिए context के रूप में शामिल करता है।
-   **Auto-run Mermaid Syntax Fix after Generation**:
    *   **Enabled (Default)**: Process, Generate from Title, Batch Generate from Titles, Research & Summarize, Summarise as Mermaid और Translate जैसी Mermaid-संबंधित workflows के बाद Mermaid syntax-fixing pass स्वचालित रूप से चलाता है।
    *   **Disabled**: generated Mermaid output को तब तक untouched छोड़ देता है जब तक आप `Batch Mermaid Fix` manually न चलाएँ या उसे किसी custom workflow में न जोड़ें।
...
-   **Use Custom Output Folder for 'Generate from Title'**:
    *   **Disabled (Default)**: सफलतापूर्वक generated files मूल folder के parent के सापेक्ष `[OriginalFolderName]_complete` नामक subfolder में ले जाई जाती हैं (या यदि मूल folder root था तो `Vault_complete`)।
    *   **Enabled**: आपको उस subfolder के लिए custom name निर्दिष्ट करने देता है जहाँ complete files move की जाएँगी।
...
-   **Enable Custom Prompts for Specific Tasks**:
    *   **Disabled (Default)**: प्लगइन सभी operations के लिए built-in default prompts का उपयोग करता है।
    *   **Enabled**: नीचे सूचीबद्ध tasks के लिए custom prompts सेट करने की क्षमता सक्रिय करता है। यह इस feature का master switch है।
...
-   **Use Custom Prompt for [Task Name]**: (केवल ऊपर वाला विकल्प enabled होने पर दिखाई देता है)
    ...
    *   **Enabled**: यह task नीचे दिए गए corresponding "Custom Prompt" text area में आपके द्वारा प्रदान किए गए पाठ का उपयोग करेगा।
...
-   **Enable Focused Learning Domain**:
    *   **Disabled (Default)**: LLM को भेजे जाने वाले prompts मानक, सामान्य-उद्देश्य निर्देशों का उपयोग करते हैं।
    *   **Enabled**: आपको एक या अधिक अध्ययन क्षेत्रों को निर्दिष्ट करने देता है ताकि LLM की contextual understanding सुधर सके।
...
-   custom multi-step buttons लॉन्च करने के लिए sidebar के शीर्ष पर **Quick Workflows** क्षेत्र का उपयोग करें।
...
-   **LM Studio/Ollama connection issues**:
    *   **Test Connection Fails**: सुनिश्चित करें कि local server (LM Studio या Ollama) चल रहा हो और सही model loaded/available हो।
...
*Notemd v1.8.0 - AI के साथ अपने Obsidian knowledge graph को बेहतर बनाएँ।*
