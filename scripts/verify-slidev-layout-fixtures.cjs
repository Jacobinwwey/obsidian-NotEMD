...
function createCustomVueComponentTreeStressDeck() {
    return [
        '---',
        'theme: default',
        'mdc: true',
        '---',
        '',
        '# Custom Vue Component Tree Stress',
        '',
        '---',
        ...
        '  :cards="[',
        "    { label: 'Ingest', value: '128', tone: 'teal' },",
        "    { label: 'Plan', value: '256', tone: 'blue' },",
        "    { label: 'Audit', value: '512', tone: 'amber' },",
        "    { label: 'Export', value: '1024', tone: 'violet' },",
        '  ]"',
        '>',
        '  <MetricPanel',
        '    label="Ingest"',
        ...
        '  />',
        '  <template #footer>',
        '    <div class="stage10-footnote">Vue component tree regression fingerprint remains one measured surface.</div>',
        '  </template>',
        '</DashboardGrid>',
        '',
    ].join('\n');
...
function createMixedComponentProseStressDeck() {
    return [
        '---',
        'theme: default',
        'mdc: true',
        '---',
        '',
        '# Mixed Component Prose Stress',
        '',
        '---',
        'layout: dashboard-shell',
        '---',
        '',
        '## Mixed Component Surface',
        '',
        '<DashboardGrid',
        '  class="stage12-dashboard"',
        '  :cards="[',
        "    { label: 'Ingest', value: '128', tone: 'teal' },",
        "    { label: 'Plan', value: '256', tone: 'blue' },",
        "    { label: 'Audit', value: '512', tone: 'amber' },",
        ...
        '  ]"',
        '>',
        '  <MetricPanel label="Ingest" value="128" tone="teal" />',
        '  <MetricPanel label="Plan" value="256" tone="blue" />',
        '  <MetricPanel label="Audit" value="512" tone="amber" />',
        '  <MetricPanel label="Export" value="1024" tone="violet" />',
        '  <template #footer>',
        '    <div class="stage12-footnote">Mixed component prose regression fingerprint remains on the component slide.</div>',
        '  </template>',
        '</DashboardGrid>',
        '',
        'Markdown prose stays readable after the component surface is separated from this custom layout slide.',
        '',
...
const FIXTURES = [
    ...
    },
    {
        ...
        expectedSurfaceFingerprint: 'Single surface regression fingerprint',
        expectedMermaidBlocks: 0,
    },
    {
        id: 'custom-vue-component-tree-stress',
        sourcePath: 'custom-vue-component-tree-stress.md',
        sourceMarkdown: createCustomVueComponentTreeStressDeck(),
        files: [
            {
            ...
            },
            {
                ...
                content: [
                    '<template>',
                    '  <section class="stage10-grid">',
                    '    <div class="stage10-grid__items">',
                    '      <slot />',
                    '    </div>',
                    '    <footer class="stage10-grid__footer">',
                    '      <slot name="footer" />',
                    '    </footer>',
                    '  </section>',
                    '</template>',
            ...
            },
            {
                ...
                content: [
                    '<template>',
                    '  <article class="stage10-panel" :class="`stage10-panel--${tone}`">',
                    '    <h2>{{ label }}</h2>',
                    '    <strong>{{ value }}</strong>',
                    '    <p>Measured local Transform keeps this component tree inside the custom shell.</p>',
                    '  </article>',
                    '</template>',
                    '',
                    '<script setup>',
                    ...
                    '  padding-left: 16px;',
                    '}',
                    '.stage10-panel--blue { border-left-color: #2563eb; }',
                    '.stage10-panel--amber { border-left-color: #b45309; }',
                    '.stage10-panel--violet { border-left-color: #7c3aed; }',
                    '.stage10-panel h2 {',
                    '  font-size: 30px;',
                    '  margin: 0 0 14px;',
                    '}',
                    '.stage10-panel strong {',
                    '  display: block;',
                    '  font-size: 38px;',
                    '  margin-bottom: 12px;',
                    '}',
        ...
        expectSingleSurfaceTransform: true,
        expectNoWholeSlideZoom: true,
        expectedLayout: 'dashboard-shell',
        expectedSurfaceFingerprint: 'Vue component tree regression fingerprint',
        expectedMermaidBlocks: 0,
    },
    {
        id: 'mixed-component-prose-stress',
        sourcePath: 'mixed-component-prose-stress.md',
        sourceMarkdown: createMixedComponentProseStressDeck(),
        files: [
            {
                path: 'layouts/dashboard-shell.vue',
                content: [
            ...
            },
            {
                ...
                content: [
                    '<template>',
                    '  <section class="stage12-grid">',
                    '    <div class="stage12-grid__items">',
                    '      <slot />',
                    '    </div>',
                    '    <footer class="stage12-grid__footer">',
                    '      <slot name="footer" />',
                    '    </footer>',
                    '  </section>',
                    '</template>',
            ...
            },
            {
                ...
                content: [
                    '<template>',
                    '  <article class="stage12-panel" :class="`stage12-panel--${tone}`">',
                    '    <h2>{{ label }}</h2>',
                    '    <strong>{{ value }}</strong>',
                    '    <p>Measured local Transform keeps the custom component surface separate from prose.</p>',
                    '  </article>',
                    '</template>',
                    '',
                    '<script setup>',
                    ...
                    '  padding-left: 16px;',
                    '}',
                    '.stage12-panel--blue { border-left-color: #2563eb; }',
                    '.stage12-panel--amber { border-left-color: #b45309; }',
                    '.stage12-panel--violet { border-left-color: #7c3aed; }',
                    '.stage12-panel h2 {',
                    '  font-size: 30px;',
                    '  margin: 0 0 14px;',
                    '}',
                    '.stage12-panel strong {',
                    '  display: block;',
                    '  font-size: 38px;',
                    '  margin-bottom: 12px;',
                    '}',
        ...
        expectedSurfaceFingerprint: 'Mixed component prose regression fingerprint',
        expectedProseFingerprint: 'Markdown prose stays readable',
        expectedMermaidBlocks: 0,
    },
    {
...
function assertFixtureReport(fixture, report, sourceMarkdown) {
    ...
    if (fixture.expectSingleSurfaceTransform) {
        ...
        assert(
            deckMarkdown.includes(fixture.expectedSurfaceFingerprint || 'Single surface regression fingerprint'),
            `${fixture.id}: single component surface content disappeared`
        );
    }
    ...
    if (fixture.expectedProseFingerprint) {
        assert(deckMarkdown.includes(fixture.expectedProseFingerprint), `${fixture.id}: prose content disappeared during convergence`);
    }
    if (fixture.expectedTransformZones) {
