...
Example:
  ...
  node scripts/export-diagram-artifact.js --input knowledge-map.json --target drawnix --output architecture.drawnix --preview-svg-output architecture.drawnix.svg
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    switch (token) {
      ...
      case '--drawnix-delivery':
        ...
        break;
      case '--source-label':
        args.drawnixSourceLabel = argv[++index];
        break;
...
function normalizeDrawnixDelivery(value) {
  // Kept as a no-op parser for existing automation. Drawnix now has one
  // canonical filename-rooted tree artifact and no delivery variants.
  void value;
  return undefined;
}

function normalizeDrawnixSourceLabel(value, fallback) {
  if (value === undefined || value === null || value === '') {
    return fallback;
  }
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error('--source-label must be a non-empty string when provided.');
  }
  return value.trim();
}
...
async function run(args, repoRoot = path.resolve(__dirname, '..')) {
  ...
  normalizeDrawnixDelivery(args.drawnixDelivery);
  normalizeDrawnixSourceLabel(args.drawnixSourceLabel, path.basename(inputPath));
