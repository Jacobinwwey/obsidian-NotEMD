...
- Extended the generic `OpenAI Compatible` preset so known official hosts can also reuse upstream provider token-cap metadata for bare model IDs. That keeps `Fetch model list`, model-aware max-token guidance, and automatic chunk-size syncing aligned even when users choose plain IDs such as `gpt-4o`, `qwen3-coder-plus`, or `mimo-v2.5-pro` instead of provider-prefixed gateway IDs.
...
- Added model-aware auto-sync for token defaults: when `Max tokens` and `Chunk word count` are still following the default/auto-managed baseline, changing the selected model now updates them to the model's known ceiling and the matching recommended chunk size. Once the user manually diverges, later model changes no longer overwrite those values.
