...
আরও ভাষায় ডকুমেন্টেশন পড়ুন: [Language Hub](./docs/i18n/README.md)
...
|---|---|---|---|
| `UI Locale` | শুধুমাত্র প্লাগইনের UI টেক্সট (settings, sidebar, notices, dialogs) | `auto` | Obsidian-এর locale অনুসরণ করে; বর্তমান UI catalog হলো `en`, `ar`, `de`, `es`, `fa`, `fr`, `id`, `it`, `ja`, `ko`, `nl`, `pl`, `pt`, `pt-BR`, `ru`, `th`, `tr`, `uk`, `vi`, `zh-CN`, `zh-TW`। |
| `Task Output Language` | LLM-সৃষ্ট task output (লিংক, সারাংশ, জেনারেশন, এক্সট্র্যাকশন, অনুবাদের লক্ষ্য ভাষা) | `en` | `Use different languages for tasks` চালু থাকলে এটি global বা per-task হতে পারে। |
| `Disable auto translation` | `Translate` ছাড়া অন্যান্য task source-language context ধরে রাখে | `false` | Explicit `Translate` task এখনো configured target language প্রয়োগ করে। |
| লোকেল fallback | অনুপস্থিত UI key সমাধান | locale -> `en` | কিছু key অনূদিত না হলেও UI স্থিতিশীল রাখে। |

- অফিসিয়াল ডকুমেন্টেশন ইংরেজি এবং সরলীকৃত চীনা ভাষায় রক্ষণাবেক্ষণ করা হয়, এবং ৩০টিরও বেশি ভাষার পূর্ণ সমর্থন রয়েছে।
- সমর্থিত সব ভাষার লিঙ্ক উপরের হেডারে দেওয়া আছে।
- আরও বিস্তারিত ও অবদান নির্দেশিকা [Language Hub](./docs/i18n/README.md)-এ সংরক্ষিত।
...
- **Disable auto translation**: এই অপশন চালু থাকলে `Translate` ছাড়া অন্য task আর আউটপুটকে নির্দিষ্ট ভাষায় বাধ্য করে না, ফলে মূল ভাষার context বজায় থাকে। explicit "Translate" task তবু configuration অনুযায়ী অনুবাদ চালায়।
...
1.  **Active Provider**: dropdown menu থেকে আপনি যে LLM provider ব্যবহার করতে চান তা নির্বাচন করুন।
2.  **Provider Settings**: নির্বাচিত provider-এর জন্য নির্দিষ্ট settings কনফিগার করুন:
...
-   **Use Different Providers for Tasks**:
    *   **Disabled (Default)**: উপর থেকে নির্বাচিত একটি "Active Provider" সব task-এর জন্য ব্যবহার করে।
    *   **Enabled**: প্রতিটি task ("Add Links", "Research & Summarize", "Generate from Title", "Translate", "Extract Concepts")-এর জন্য নির্দিষ্ট provider নির্বাচন এবং প্রয়োজনে model name override করার সুযোগ দেয়। যদি কোনো task-এর model override field ফাঁকা থাকে, তাহলে ওই task-এর জন্য নির্বাচিত provider-এর default model ব্যবহৃত হবে।
-   **Select different languages for different tasks**:
    *   **Disabled (Default)**: সব task-এর জন্য একটি "Output language" ব্যবহার করে।
    *   **Enabled**: প্রতিটি task ("Add Links", "Research & Summarize", "Generate from Title", "Summarise as Mermaid diagram", "Extract Concepts")-এর জন্য আলাদা ভাষা নির্বাচন করতে দেয়।
...
### ভাষা স্থাপত্য (UI Locale বনাম Task Output Language)

-   **UI Locale** শুধুমাত্র প্লাগইন ইন্টারফেসের টেক্সট (Settings labels, sidebar buttons, notices এবং dialogs) নিয়ন্ত্রণ করে। ডিফল্ট `auto` mode Obsidian-এর বর্তমান UI language অনুসরণ করে।
-   **Task Output Language** model-generated task output (লিঙ্ক, সারাংশ, title generation, Mermaid summary, concept extraction, translation target) নিয়ন্ত্রণ করে।
...
-   **Disable auto translation** non-`Translate` task-কে source-language context-এ রাখে, কিন্তু explicit `Translate` task এখনও configured target language প্রয়োগ করে।
...
-   **Enable Stable API Calls (Retry Logic)**:
    *   **Disabled (Default)**: একটি API call failure বর্তমান task বন্ধ করে দেবে।
    *   **Enabled**: failed LLM API call-গুলো স্বয়ংক্রিয়ভাবে retry করে (intermittent network issue বা rate limit-এর জন্য উপযোগী)।
...
-   **Customize Processed File Save Path**:
    *   **Disabled (Default)**: processed file (যেমন `YourNote_processed.md`) মূল note-এর *একই folder*-এ সেভ হয়।
    *   **Enabled**: custom save location নির্ধারণ করতে দেয়।
...
-   **Use Custom Output Filename for 'Add Links'**:
    *   **Disabled (Default)**: 'Add Links' command দ্বারা তৈরি processed file ডিফল্ট `_processed.md` suffix ব্যবহার করে (যেমন `YourNote_processed.md`)।
    *   **Enabled**: নিচের setting ব্যবহার করে output filename কাস্টমাইজ করতে দেয়।
...
-   **Remove Code Fences on Add Links**:
    *   **Disabled (Default)**: link যোগ করার সময় code fence **(\`\\\`\`)** content-এ থাকে, এবং **(\`\\\`markdown)** স্বয়ংক্রিয়ভাবে মুছে যায়।
    *   **Enabled**: link যোগ করার আগে content থেকে code fence সরিয়ে দেয়।
...
-   **Customize Concept Note Path**:
    *   **Disabled (Default)**: `[[linked concepts]]`-এর জন্য note-এর automatic creation বন্ধ থাকে।
    *   **Enabled**: নতুন concept note কোথায় তৈরি হবে সেই folder নির্ধারণ করতে দেয়।
...
-   **Generate Concept Log File**:
    *   **Disabled (Default)**: কোনো log file তৈরি হয় না।
    *   **Enabled**: processing-এর পর newly created concept note-এর তালিকা সম্বলিত log file তৈরি করে। ফরম্যাট:
        ```
...
-   **Customize Log File Save Path**: (শুধুমাত্র "Generate Concept Log File" enabled হলে দৃশ্যমান)
    *   **Disabled (Default)**: log file **Concept Note Folder Path**-এ (যদি নির্ধারিত থাকে) বা নতুবা vault root-এ সেভ হয়।
    *   **Enabled**: log file-এর জন্য custom folder নির্ধারণ করতে দেয়।
...
-   **Customize Log File Name**: (শুধুমাত্র "Generate Concept Log File" enabled হলে দৃশ্যমান)
    *   **Disabled (Default)**: log file-এর নাম `Generate.log`।
    *   **Enabled**: log file-এর জন্য custom name নির্ধারণ করতে দেয়।
...
-   **Enable Batch Parallelism**:
    *   **Disabled (Default)**: batch processing task (যেমন "Process Folder" বা "Batch Generate from Titles") file-গুলো একটির পর একটি (serially) প্রসেস করে।
    *   **Enabled**: plugin-কে একাধিক file সমান্তরালে প্রসেস করার সুযোগ দেয়, যা বড় batch job অনেক দ্রুত করতে পারে।
...
-   **Enable Duplicate Detection**: processed content-এ duplicate word চেককে on/off করে (ফলাফল console-এ যায়)। (Default: Enabled)
...
-   **Customise Translation File Save Path**:
    *   **Disabled (Default)**: translated file মূল note-এর *একই folder*-এ সেভ হয়।
    *   **Enabled**: আপনার vault-এর মধ্যে *relative path* (যেমন `Translations`) নির্ধারণ করতে দেয়, যেখানে translated file সেভ হবে। folder না থাকলে তৈরি হবে।
-   **Use custom suffix for translated files**:
    *   **Disabled (Default)**: translated file ডিফল্ট `_translated.md` suffix ব্যবহার করে (যেমন `YourNote_translated.md`)।
    *   **Enabled**: custom suffix নির্ধারণ করতে দেয়।
...
-   **Enable Research in "Generate from Title"**:
    *   **Disabled (Default)**: "Generate from Title" শুধুমাত্র title-কে input হিসেবে ব্যবহার করে।
    *   **Enabled**: configured **Web Research Provider** ব্যবহার করে web research করে এবং title-based generation-এর সময় তার ফলাফল LLM-এর জন্য context হিসেবে যোগ করে।
-   **Auto-run Mermaid Syntax Fix after Generation**:
    *   **Enabled (Default)**: Process, Generate from Title, Batch Generate from Titles, Research & Summarize, Summarise as Mermaid এবং Translate-এর মতো Mermaid-সম্পর্কিত workflow-এর পরে Mermaid syntax fix স্বয়ংক্রিয়ভাবে চালায়।
    *   **Disabled**: generated Mermaid output অপরিবর্তিত রাখে, যদি না আপনি `Batch Mermaid Fix` manually চালান বা custom workflow-তে যোগ করেন।
...
-   **Use Custom Output Folder for 'Generate from Title'**:
    *   **Disabled (Default)**: সফলভাবে generated file-গুলো মূল folder-এর parent-এর সাপেক্ষে `[OriginalFolderName]_complete` নামে subfolder-এ move হয় (বা মূল folder root হলে `Vault_complete`)।
    *   **Enabled**: complete file move করার subfolder-এর জন্য custom name নির্ধারণ করতে দেয়।
...
-   **Enable Custom Prompts for Specific Tasks**:
    *   **Disabled (Default)**: plugin সব operation-এর জন্য built-in default prompt ব্যবহার করে।
    *   **Enabled**: নিচে দেওয়া task-গুলোর জন্য custom prompt সেট করার সুবিধা সক্রিয় করে। এটি এই feature-এর master switch।
...
-   **Use Custom Prompt for [Task Name]**: (শুধু উপরোক্ত অপশন enabled হলে দৃশ্যমান)
    ...
    *   **Enabled**: এই task নিচের সংশ্লিষ্ট "Custom Prompt" text area-তে দেওয়া আপনার কাস্টম নির্দেশনা ব্যবহার করবে।
...
-   **Enable Focused Learning Domain**:
    *   **Disabled (Default)**: LLM-এ পাঠানো prompt standard, general-purpose instruction ব্যবহার করে।
    *   **Enabled**: আপনাকে এক বা একাধিক study field নির্ধারণ করতে দেয়, যাতে LLM-এর contextual understanding উন্নত হয়।
...
### Quick workflow ও sidebar

-   মূল প্রসেসিং, জেনারেশন, অনুবাদ, জ্ঞান ও utility action-এর grouped section-এ পৌঁছাতে Notemd sidebar খুলুন।
-   custom multi-step button চালাতে sidebar-এর শীর্ষে থাকা **Quick Workflows** এলাকা ব্যবহার করুন।
...
-   **LM Studio/Ollama connection issue**:
    *   **Test Connection Fails**: নিশ্চিত করুন local server (LM Studio বা Ollama) চালু আছে এবং সঠিক model loaded/available আছে।
...
*Notemd v1.8.0 - AI-এর সাহায্যে আপনার Obsidian knowledge graph আরও উন্নত করুন।*
