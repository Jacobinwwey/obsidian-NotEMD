...
- **本地知识检索**: `从标题批量生成`、`研究与总结`、`生成图形` 现在可选接入本地知识库检索，上下文完全在插件内本地构建，不依赖云检索服务或常驻外部进程。
...
#### 本地知识检索
- **启用本地知识检索**:
    *   **关闭（默认）**: `从标题批量生成`、`研究与总结`、`生成图形` 不注入本地检索上下文。
    *   **开启**: 会从已配置的 vault 相对路径下索引 Markdown / Text 文件，并将最匹配的本地上下文注入上述任务。
- **知识库路径**: 每行一个 vault 相对路径。索引与检索均在插件本地运行，不依赖外部服务。
