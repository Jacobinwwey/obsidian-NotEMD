...
- **批量Mermaid修复**: 对选定文件夹内所有Markdown文件应用Mermaid和LaTeX语法校正。
    ...
    - **智能检测**: 在尝试修复之前，使用 `mermaid.parse` 智能检查文件是否存在语法错误，节省处理时间并避免不必要的编辑。
            -   **高级修复模式**: 包含针对包含空格、特殊字符或嵌套括号的未加引号节点标签的稳健修复（例如，将 `Node[标签 [文本]]` 转换为 `Node["标签 [文本]"]`），确保兼容恒星演化路径等复杂图表。同时修正格式错误的连接线标签（例如将 `--["标签["-->` 修正为 `-- "标签" -->`）。另外转换行内注释（`Consensus --> Adaptive; # Some advanced consensus` 转换为 `Consensus -- "Some advanced consensus" --> Adaptive`）和修复行尾不完全引号（以 `;"` 结尾的行替换为 `"]`）。
        -   **注释转换**: 自动将 `note right/left of` 样式的注释转换为标准的 Mermaid 连接线标签（例如，将 `note right of A: text` 转换为 `A -- "text" --> B`），从而改善图表布局和可读性。
        -   **畸形箭头修复**: 深度调试模式现在可以识别并纠正箭头语法被错误地包含在引号内的畸形箭头标签（例如，将 `A -- "标签 -->" B` 修正为 `A -- "标签" --> B`）。
        -   **标准化管道标签**: 自动修复和标准化包含管道符的连接线标签，确保它们被正确引用（例如，将 `-->|文本|` 转换为 `-->|"文本"|`，将 `-->|数学|^2|` 转换为 `-->|"数学|^2"|`）。
        -   **合并双重标签**: 检测并合并单条边上令人困惑的双重标签（例如，`A -- "标签1" -->|"标签2"| B`），将其转换为单个清晰的标签（`A -- "标签1<br>(标签2)" --> B`）。
        -   **行内子图修复**: 将用作标签的行内子图（例如 `A --> B; subgraph "标签" end;`）转换为标准的带标签连接线（`A -- "标签" --> B;`），解决了部分 LLM 输出的常见语法错误。
        -   **提取特定原始内容**:    - 在设置中定义问题列表。
