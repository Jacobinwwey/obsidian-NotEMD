...
- **运行时桌面传输回退**: 当长耗时 Provider 请求被 `requestUrl` 以 `ERR_CONNECTION_CLOSED` 等瞬时网络错误中断时，Notemd 现在会先在同一次调用内切换到桌面端 `http/https` 传输，再在必要时进入配置好的稳定重试序列，从而降低慢速网关或反向代理下的误报失败。
...
- 启用稳定 API 调用（重试逻辑）(Enable Stable API Calls (Retry Logic))
  ...
  - **运行时传输回退（桌面端）**: 在桌面环境中，如果长耗时任务请求被 `requestUrl` 瞬时断开，Notemd 会先用 Node `http/https` 传输重试同一次调用；只有该回退也失败时，才进入常规稳定重试序列。
