...
- **实验性图表流水线**:
    ...
    - Drawnix Mermaid 图会把经安全清理的 SVG 与源文本内联到 `.drawnix` metadata，因此仅包含 Mermaid 的图不会再生成独立 `.assets` 目录；重新生成时只会删除带合法 manifest 且确认为插件所有的旧 Mermaid companion，无法安全内联的外部二进制或未解析图形仍会保留 companion。Drawnix 关系标签使用节点感知的正交路由、测量换行和最后绘制的独立标签层，长解释不会再被节点矩形遮蔽；原生箭头文本与 SVG 预览共享同一套换行结果。标题/摘要页眉是受保护的路由区域，并在最后用不透明防护带绘制，外侧虚线通道不会再遮盖它。
...
| Drawnix | `_diagram.drawnix` 内联 Mermaid 源图形；仅主预览或未内联/未解析媒体使用 SVG/媒体 companion | SVG companion | 是 | 是 | 是 | 是 | 默认单一文档根节点、节点感知的带标签箭头、避开无关 root 的跨 root 路由，Mermaid 预览自包含；插件内不嵌入 Drawnix 或 Plait runtime。 |
