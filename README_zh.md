...
- 启用稳定 API 调用（重试逻辑）(Enable Stable API Calls (Retry Logic))
  ...
  - **运行时传输回退（环境感知）**: 如果长耗时任务请求被 `requestUrl` 瞬时断开，Notemd 会先使用与当前环境匹配的回退传输重试同一次调用：桌面端走 Node `http/https`，非桌面环境走浏览器 `fetch`。该回退阶段现在会按协议解析各类流式输出，覆盖 OpenAI-compatible / Azure OpenAI 的 SSE、Anthropic Messages SSE、Google Gemini SSE，以及 Ollama 的 NDJSON，让慢速网关尽早返回 body 分片；其余直连的 OpenAI 风格 Provider 入口也会复用这条共享回退链路。只有该回退也失败时，才进入常规稳定重试序列。
...
- **开发者 Provider 诊断（长请求）(Developer Provider Diagnostic (Long Request))**:
    *   **运行诊断 (Run Diagnostic)**: 在设置页对当前活动 Provider 发起一次长请求诊断，并把完整报告 `Notemd_Provider_Diagnostic_*.txt` 写入仓库根目录。
    *   **报告内容**: 包含 Provider 上下文、重试/调试模式运行参数、总耗时、响应预览和运行时日志（含深度调试细节）。
