...
Notemd 通过与各种大型语言模型 (LLM) 集成来增强您的 Obsidian 工作流程，以处理您的笔记、自动为关键概念生成维基链接、创建相应的概念笔记、执行网络研究等等。
...
- **多模型配置**: 为不同任务（添加链接、研究、生成标题）使用不同的LLM提供商*和*特定模型，或为所有任务使用单个提供商。
...
-   **为任务使用不同提供商**:
    ...
    *   **启用**: 允许您为每个任务 (“添加链接”、“研究与摘要”、“从标题生成”) 选择特定的提供商*并*可选择覆盖模型名称。如果任务的模型覆盖字段留空，它将使用为该任务选定提供商配置的默认模型。
...
*Notemd v1.3.0 - 使用AI增强您的Obsidian知识图谱。*
