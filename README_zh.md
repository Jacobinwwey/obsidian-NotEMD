...
- **重构后的侧边栏**: 内置操作按用途分组展示，并提供更清晰的标签、实时状态、可取消进度和可复制日志，显著减少按钮堆叠带来的混乱。
...
3. **测试连接 (Test Connection)**: 使用活动提供商的“测试连接”按钮来验证您的设置。OpenAI-compatible 提供商现在会按 provider 特性自动选择测试策略：`Qwen`、`Doubao`、`Moonshot`、`GLM`、`MiniMax`、`Baidu Qianfan`、`SiliconFlow`、`Groq`、`Together`、`Fireworks`、`LMStudio` 与 `OpenAI Compatible` 会直接探测 `chat/completions`，而具备稳定 `/models` 端点的服务仍会优先走模型列表探测。
...
-   每一步的状态、日志和失败信息都会显示在侧边栏中，自定义工作流也可以在设置页中重新配置。
