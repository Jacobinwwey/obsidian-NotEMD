...
- เอกสารทางการดูแลเป็นภาษาอังกฤษและภาษาจีนตัวย่อ พร้อมรองรับมากกว่า 30 ภาษาอย่างครบถ้วน
- ภาษาที่รองรับทั้งหมดลิงก์ไว้ที่ส่วนหัวด้านบนแล้ว
...
- **Disable auto translation**: เมื่อเปิดตัวเลือกนี้ งานที่ไม่ใช่ Translate จะไม่บังคับให้ผลลัพธ์ออกมาเป็นภาษาใดภาษาหนึ่งอีกต่อไป และจะคงบริบทภาษาต้นฉบับไว้ ส่วนงาน "Translate" แบบ explicit จะยังแปลตามค่าที่กำหนดไว้เหมือนเดิม
...
1.  **Active Provider**: เลือกผู้ให้บริการ LLM ที่คุณต้องการใช้จากเมนูแบบเลื่อนลง
2.  **Provider Settings**: ตั้งค่ารายละเอียดเฉพาะของผู้ให้บริการที่เลือก:
...
-   **Use Different Providers for Tasks**:
    *   **Disabled (Default)**: ใช้ "Active Provider" เพียงตัวเดียว (ที่เลือกไว้ด้านบน) สำหรับทุกงาน
    *   **Enabled**: ให้คุณเลือกผู้ให้บริการเฉพาะ *และ* เลือกระบุชื่อโมเดล override แยกตามงานได้ ("Add Links", "Research & Summarize", "Generate from Title", "Translate", "Extract Concepts") หากปล่อยช่อง model override ของงานนั้นว่างไว้ ระบบจะใช้โมเดล default ของผู้ให้บริการที่เลือกสำหรับงานนั้น
-   **Select different languages for different tasks**:
    *   **Disabled (Default)**: ใช้ "Output language" ตัวเดียวสำหรับทุกงาน
    *   **Enabled**: ให้คุณเลือกภาษาเฉพาะสำหรับแต่ละงาน ("Add Links", "Research & Summarize", "Generate from Title", "Summarise as Mermaid diagram", "Extract Concepts")
...
### สถาปัตยกรรมภาษา (UI Locale เทียบกับภาษาผลลัพธ์ของงาน)

-   **UI Locale** ควบคุมเฉพาะข้อความอินเทอร์เฟซของปลั๊กอิน (ป้ายกำกับใน Settings, ปุ่มในแถบข้าง, notices และ dialogs) ค่าเริ่มต้น `auto` จะตามภาษาของ UI ใน Obsidian ปัจจุบัน
-   **Task Output Language** ควบคุมผลลัพธ์ของงานที่โมเดลสร้างขึ้น (ลิงก์, บทสรุป, การสร้างชื่อเรื่อง, บทสรุป Mermaid, การสกัดแนวคิด, ภาษาเป้าหมายของการแปล)
-   **Per-task language mode** ให้แต่ละงาน resolve ภาษาผลลัพธ์ของตัวเองผ่าน policy layer เดียว แทนที่จะใช้ override กระจัดกระจายในแต่ละโมดูล
-   **Disable auto translation** ช่วยให้งานที่ไม่ใช่ Translate คงบริบทภาษาต้นฉบับไว้ ขณะที่งาน Translate แบบ explicit จะยังคงบังคับใช้ภาษาเป้าหมายที่ตั้งไว้
-   เส้นทางการสร้างที่เกี่ยวกับ Mermaid จะใช้ language policy ชุดเดียวกัน และยังคงเรียก Mermaid auto-fix ได้เมื่อเปิดใช้งาน

### การตั้งค่าการเรียก API แบบเสถียร
-   **Enable Stable API Calls (Retry Logic)**:
    *   **Disabled (Default)**: หากการเรียก API ครั้งเดียวล้มเหลว งานปัจจุบันจะหยุดทันที
    *   **Enabled**: ลองใหม่สำหรับการเรียก LLM API ที่ล้มเหลวโดยอัตโนมัติ (มีประโยชน์เมื่อมีปัญหาเครือข่ายเป็นครั้งคราวหรือเจอ rate limit)
...
-   **API Error Debugging Mode**:
    *   **Disabled (Default)**: ใช้การรายงานข้อผิดพลาดแบบมาตรฐานและกระชับ
    *   **Enabled**: เปิดการบันทึกข้อผิดพลาดแบบละเอียด (คล้าย verbose output ของ DeepSeek) สำหรับทุกผู้ให้บริการและทุกงาน (รวมถึง Translate, Search และ Connection Tests) ซึ่งรวมถึง HTTP status codes, raw response text, request transport timelines, request URLs และ headers ที่ sanitize แล้ว, เวลาของแต่ละความพยายาม, response headers, partial response bodies, parsed partial stream output และสแต็กเทรซ ซึ่งจำเป็นมากสำหรับ troubleshooting ปัญหาการเชื่อมต่อ API และการรีเซ็ตของ upstream gateway
-   **Developer Mode**:
    *   **Disabled (Default)**: ซ่อน control ด้าน diagnostics ที่ใช้เฉพาะนักพัฒนาทั้งหมดจากผู้ใช้ทั่วไป
    *   **Enabled**: แสดงแผง developer diagnostics โดยเฉพาะใน Settings
...
-   **Customize Processed File Save Path**:
    *   **Disabled (Default)**: ไฟล์ที่ประมวลผลแล้ว (เช่น `YourNote_processed.md`) จะถูกบันทึกไว้ใน *โฟลเดอร์เดียวกัน* กับโน้ตต้นฉบับ
    *   **Enabled**: ให้คุณระบุตำแหน่งบันทึกแบบกำหนดเองได้
...
-   **Use Custom Output Filename for 'Add Links'**:
    *   **Disabled (Default)**: ไฟล์ที่สร้างจากคำสั่ง 'Add Links' จะใช้ suffix เริ่มต้น `_processed.md` (เช่น `YourNote_processed.md`)
    *   **Enabled**: ให้คุณกำหนดชื่อไฟล์ผลลัพธ์เองผ่านการตั้งค่าด้านล่าง
...
-   **Remove Code Fences on Add Links**:
    *   **Disabled (Default)**: จะเก็บ code fences **(\`\\\`\`)** ไว้ในเนื้อหาระหว่างเพิ่มลิงก์ และ **(\`\\\`markdown)** จะถูกลบออกโดยอัตโนมัติ
    *   **Enabled**: ลบ code fences ออกจากเนื้อหาก่อนทำการเพิ่มลิงก์
...
-   **Customize Concept Note Path**:
    *   **Disabled (Default)**: ปิดการสร้างโน้ตสำหรับ `[[linked concepts]]` แบบอัตโนมัติ
    *   **Enabled**: ให้คุณระบุโฟลเดอร์สำหรับสร้างโน้ตแนวคิดใหม่ได้
...
-   **Generate Concept Log File**:
    *   **Disabled (Default)**: จะไม่สร้างไฟล์ log
    *   **Enabled**: สร้างไฟล์ log ที่แสดงรายการโน้ตแนวคิดใหม่หลังการประมวลผล โดยมีรูปแบบดังนี้:
        ```
...
-   **Customize Log File Save Path**: (แสดงเมื่อเปิด "Generate Concept Log File" เท่านั้น)
    *   **Disabled (Default)**: ไฟล์ log จะถูกบันทึกไว้ใน **Concept Note Folder Path** (ถ้ามีระบุ) หรือที่ root ของ vault หากไม่มี
    *   **Enabled**: ให้คุณระบุโฟลเดอร์สำหรับไฟล์ log เองได้
...
-   **Customize Log File Name**: (แสดงเมื่อเปิด "Generate Concept Log File" เท่านั้น)
    *   **Disabled (Default)**: ไฟล์ log จะใช้ชื่อ `Generate.log`
    *   **Enabled**: ให้คุณกำหนดชื่อไฟล์ log เองได้
...
-   **Enable Batch Parallelism**:
    *   **Disabled (Default)**: งานประมวลผลแบบกลุ่ม (เช่น "Process Folder" หรือ "Batch Generate from Titles") จะประมวลผลทีละไฟล์แบบอนุกรม
    *   **Enabled**: อนุญาตให้ปลั๊กอินประมวลผลหลายไฟล์พร้อมกัน ซึ่งช่วยเร่งงานกลุ่มขนาดใหญ่ได้มาก
...
-   **Customise Translation File Save Path**:
    *   **Disabled (Default)**: ไฟล์ที่แปลแล้วจะถูกบันทึกไว้ใน *โฟลเดอร์เดียวกัน* กับโน้ตต้นฉบับ
    *   **Enabled**: ให้คุณระบุ *path แบบ relative* ภายใน vault (เช่น `Translations`) ที่จะใช้บันทึกไฟล์แปล ระบบจะสร้างโฟลเดอร์ให้หากยังไม่มี
-   **Use custom suffix for translated files**:
    *   **Disabled (Default)**: ไฟล์แปลจะใช้ suffix เริ่มต้น `_translated.md` (เช่น `YourNote_translated.md`)
    *   **Enabled**: ให้คุณกำหนด suffix เองได้
...
-   **Enable Research in "Generate from Title"**:
    *   **Disabled (Default)**: "Generate from Title" จะใช้เพียงชื่อเรื่องเป็นอินพุต
    *   **Enabled**: ทำเว็บรีเสิร์ชผ่าน **Web Research Provider** ที่กำหนด แล้วใส่ผลลัพธ์เป็นบริบทให้ LLM ระหว่างการสร้างจากชื่อเรื่อง
-   **Auto-run Mermaid Syntax Fix after Generation**:
    *   **Enabled (Default)**: รันขั้นตอนแก้ Mermaid syntax โดยอัตโนมัติหลัง workflow ที่เกี่ยวกับ Mermaid เช่น Process, Generate from Title, Batch Generate from Titles, Research & Summarize, Summarise as Mermaid และ Translate
...
-   **Use Custom Output Folder for 'Generate from Title'**:
    *   **Disabled (Default)**: ไฟล์ที่สร้างสำเร็จจะถูกย้ายไปยัง subfolder ชื่อ `[OriginalFolderName]_complete` ซึ่งอยู่ relative กับโฟลเดอร์แม่ของโฟลเดอร์เดิม (หรือ `Vault_complete` หากโฟลเดอร์เดิมคือ root)
    *   **Enabled**: ให้คุณกำหนดชื่อ subfolder ที่จะใช้ย้ายไฟล์ที่ประมวลผลสำเร็จเองได้
...
-   **Enable Custom Prompts for Specific Tasks**:
    *   **Disabled (Default)**: ปลั๊กอินจะใช้ prompt เริ่มต้นที่มีมาให้สำหรับทุกการทำงาน
    *   **Enabled**: เปิดความสามารถในการตั้ง custom prompt สำหรับงานที่ระบุด้านล่าง นี่คือ master switch ของฟีเจอร์นี้
...
-   **Use Custom Prompt for [Task Name]**: (แสดงเมื่อเปิดตัวเลือกด้านบนเท่านั้น)
    ...
    *   **Enabled**: งานนี้จะใช้ข้อความที่คุณกรอกในช่อง "Custom Prompt" ด้านล่าง
...
-   **Enable Focused Learning Domain**:
    *   **Disabled (Default)**: prompt ที่ส่งไปยัง LLM จะใช้คำสั่งมาตรฐานทั่วไป
    *   **Enabled**: ให้คุณระบุสาขาวิชาหนึ่งหรือหลายสาขาเพื่อช่วยให้ LLM เข้าใจบริบทได้ดีขึ้น
...
-   ใช้พื้นที่ **Quick Workflows** ด้านบนของแถบข้างเพื่อเปิดใช้ปุ่มหลายขั้นตอนแบบกำหนดเอง
...
-   **ปัญหาการเชื่อมต่อ LM Studio/Ollama**:
    *   **Test Connection Fails**: ให้แน่ใจว่า local server (LM Studio หรือ Ollama) กำลังทำงานและมีการโหลด/เปิดให้ใช้โมเดลที่ถูกต้องอยู่
...
*Notemd v1.8.0 - ยกระดับกราฟความรู้ใน Obsidian ของคุณด้วย AI*
