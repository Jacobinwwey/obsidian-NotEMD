...
export function renderArtifactMarkup(payload: RenderWebviewPayload): string {
    ...
    if (payload.artifact.target === 'mermaid' && payload.artifact.mimeType === 'text/vnd.mermaid') {
        return `<section class="notemd-render-shell notemd-render-shell--mermaid" data-render-target="${escapeHtml(payload.artifact.target)}" data-render-theme="${escapeHtml(payload.resolvedTheme)}" data-theme-source="${escapeHtml(payload.theme)}" data-source-intent="${escapeHtml(payload.artifact.sourceIntent)}">
    <header class="notemd-render-header">${escapeHtml(previewTitle)}</header>
    ${sourceMarkup}
    <div class="notemd-render-host-body">
        <div id="notemd-mermaid-mount" class="notemd-render-mount" hidden></div>
        <p id="notemd-mermaid-error" class="notemd-render-error" hidden></p>
        <details class="notemd-render-fallback" open>
            <summary>Source</summary>
            <pre id="notemd-mermaid-source" class="notemd-render-body">${escapeHtml(formatArtifactContent(payload))}</pre>
        </details>
    </div>
</section>
<script id="notemd-render-host-bootstrap">${buildMermaidRenderBootstrap()}</script>`;
    }
