...
function renderNode(node: SemanticFigureNode): string {
    ...
    return `<g id="${escapeAttribute(node.id)}" class="notemd-editable-svg-node" data-drawio-type="node" data-drawio-role="${escapeAttribute(node.role)}" data-drawio-id="${escapeAttribute(node.id)}">
        <rect data-drawio-ignore="part-of-parent-node" x="${node.x}" y="${node.y}" width="${node.width}" height="${node.height}" rx="14" />
        ${renderMultilineSvgText(node.label, centerX, labelY, 18, 24)}
        <text data-drawio-ignore="part-of-parent-node" x="${centerX}" y="${node.y + node.height - 18}" text-anchor="middle" class="notemd-editable-svg-node-role">${escapeHtml(node.role)}</text>
    </g>`;
}
...
function renderEdgePath(edge: SemanticFigureEdge): string {
    ...
    return `<g id="${escapeAttribute(edge.id)}" class="notemd-editable-svg-edge" data-drawio-type="edge" data-drawio-id="${escapeAttribute(edge.id)}" data-drawio-source="${escapeAttribute(edge.sourceId)}" data-drawio-target="${escapeAttribute(edge.targetId)}">
        <path data-drawio-ignore="part-of-parent-edge" d="${path}" marker-end="url(#notemd-editable-svg-arrow)" />
        ${label ? `<text data-drawio-ignore="part-of-parent-edge" x="${edge.labelX}" y="${edge.labelY}" text-anchor="middle" class="notemd-editable-svg-edge-label">${escapeHtml(label)}</text>` : ''}
    </g>`;
}
    ...
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${model.width} ${model.height}" role="img" aria-labelledby="notemd-editable-svg-title notemd-editable-svg-desc" data-notemd-renderer="${NOTEMD_EDITABLE_SVG_RENDERER_VERSION}">
        ...
        <defs>
            <marker id="notemd-editable-svg-arrow" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="8" markerHeight="8" orient="auto-start-reverse">
                <path data-drawio-ignore="marker-shape" d="M 0 0 L 10 5 L 0 10 z" />
            </marker>
        </defs>
        <rect data-drawio-ignore="canvas-background" class="notemd-editable-svg-canvas" x="0" y="0" width="${model.width}" height="${model.height}" rx="18" />
        <text data-drawio-ignore="document-title" x="72" y="48" class="notemd-editable-svg-title">${escapeHtml(model.title)}</text>
        ${model.summary ? `<text data-drawio-ignore="document-summary" x="72" y="78" class="notemd-editable-svg-summary">${escapeHtml(model.summary)}</text>` : ''}
