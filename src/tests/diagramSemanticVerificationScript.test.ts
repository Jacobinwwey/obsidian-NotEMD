...
`,
        ...
        test('parses top-level on mapping declarations when on key line has trailing comment', () => {
            const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'notemd-release-workflow-on-key-comment-mapping-'));
            const workflowPath = path.join(tempRoot, 'release.yml');
            fs.writeFileSync(
                workflowPath,
                `on: # release triggers
...
`,
        ...
        test('parses on-sequence declarations when on key line has trailing comment', () => {
            const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'notemd-release-workflow-on-key-comment-sequence-'));
            const workflowPath = path.join(tempRoot, 'release.yml');
            fs.writeFileSync(
                workflowPath,
                `on: # release triggers
...
`,
        ...
        test('parses multiline flow-style on array syntax for workflow_dispatch detection', () => {
            const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'notemd-release-workflow-multiline-on-array-'));
            const workflowPath = path.join(tempRoot, 'release.yml');
            fs.writeFileSync(
                workflowPath,
                `on: [
...
`,
        ...
        test('parses multiline flow-style on array object items for release trigger facts', () => {
            const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'notemd-release-workflow-multiline-on-array-objects-'));
            const workflowPath = path.join(tempRoot, 'release.yml');
            fs.writeFileSync(
                workflowPath,
                `on: [
...
`,
        ...
        test('parses top-level push tags list when tags key line has trailing comment', () => {
            const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'notemd-release-workflow-top-level-push-tags-comment-'));
            const workflowPath = path.join(tempRoot, 'release.yml');
            fs.writeFileSync(
                workflowPath,
                `on:
  push:
    tags: # keep release tags readable
      - "*.*.*"
  workflow_dispatch:
jobs:
  publish:
...
`,
        ...
        test('parses top-level push tags multiline flow-style array syntax for release trigger facts', () => {
            const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'notemd-release-workflow-top-level-push-tags-flow-array-'));
            const workflowPath = path.join(tempRoot, 'release.yml');
            fs.writeFileSync(
                workflowPath,
                `on:
...
`,
        ...
        test('flags v-prefixed top-level push tags multiline flow-style array syntax as release trigger guard violation', () => {
            const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'notemd-release-workflow-top-level-push-tags-flow-array-v-prefix-'));
            const workflowPath = path.join(tempRoot, 'release.yml');
            fs.writeFileSync(
                workflowPath,
                `on:
...
`,
        ...
        test('parses top-level multiline flow-style push object with comma-separated fields for release trigger facts', () => {
            const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'notemd-release-workflow-top-level-push-flow-object-comma-'));
            const workflowPath = path.join(tempRoot, 'release.yml');
            fs.writeFileSync(
                workflowPath,
                `on:
...
`,
        ...
        test('parses on-sequence push mapping tags list when tags key line has trailing comment', () => {
            const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'notemd-release-workflow-sequence-push-tags-comment-'));
            const workflowPath = path.join(tempRoot, 'release.yml');
            fs.writeFileSync(
                workflowPath,
                `on:
...
`,
        ...
        test('parses on-sequence push mapping tags multiline flow-style array syntax for release trigger facts', () => {
            const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'notemd-release-workflow-sequence-push-tags-flow-array-'));
            const workflowPath = path.join(tempRoot, 'release.yml');
            fs.writeFileSync(
                workflowPath,
                `on:
...
`,
        ...
        test('parses on-sequence multiline flow-style push object with comma-separated fields for release trigger facts', () => {
            const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'notemd-release-workflow-sequence-push-flow-object-comma-'));
            const workflowPath = path.join(tempRoot, 'release.yml');
            fs.writeFileSync(
                workflowPath,
                `on:
...
`,
        ...
        test('parses mixed quoted-key sequence/object hybrid declarations when push flow-object uses multiline tags flow-array with closure comma', () => {
            const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'notemd-release-workflow-mixed-hybrid-flow-array-close-comma-'));
            const workflowPath = path.join(tempRoot, 'release.yml');
            fs.writeFileSync(
                workflowPath,
                `'on':
