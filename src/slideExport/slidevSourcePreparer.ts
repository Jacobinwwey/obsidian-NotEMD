...
export function buildDeterministicSlidevOutline(markdown: string, fallbackTitle: string): string {
	...
	const lines = [
		...
		'- Mermaid rule: preserve each source Mermaid fence as one diagram; do not split, rewrite, or derive one Mermaid diagram into several generated diagrams',
...
function buildSlidevDeckPrompt(
	...
	const system = [
		...
		'Preserve each source Mermaid fence as one Mermaid diagram. This is a hard invariant: do not split, rewrite, or derive one Mermaid diagram into several generated diagrams.',
		...
		'For large Mermaid diagrams, preserve the source diagram and use layout, diagram-focused slides, Transform containers, speaker notes, or adjacent explanation outside the fence; do not create alternative Mermaid diagrams, and do not choose Mermaid zoom manually because rendered audit owns measured fit.',
