...
- **Disable auto translation**: ha ez be van kapcsolva, a nem `Translate` feladatok nem erőltetik többé a kimeneteket egy adott nyelvre, így megőrzik az eredeti nyelvi kontextust. Az explicit "Translate" feladat továbbra is a konfiguráció szerint fordít.
...
1.  **Active Provider**: válassza ki a használni kívánt LLM-szolgáltatót a legördülő listából.
2.  **Provider Settings**: konfigurálja a kiválasztott szolgáltatóhoz tartozó konkrét beállításokat:
...
-   **Use Different Providers for Tasks**:
    *   **Disabled (Default)**: a fenti egyetlen "Active Provider" szolgáltatót használja minden feladathoz.
    *   **Enabled**: lehetővé teszi, hogy minden feladathoz külön szolgáltatót válasszon, és opcionálisan felülírja a modell nevét ("Add Links", "Research & Summarize", "Generate from Title", "Translate", "Extract Concepts"). Ha egy feladat modellfelülírási mezője üres marad, akkor az adott feladathoz kiválasztott szolgáltató alapértelmezett modelljét használja.
-   **Select different languages for different tasks**:
    *   **Disabled (Default)**: minden feladathoz ugyanazt az "Output language" beállítást használja.
    *   **Enabled**: lehetővé teszi, hogy minden feladathoz külön nyelvet válasszon ("Add Links", "Research & Summarize", "Generate from Title", "Summarise as Mermaid diagram", "Extract Concepts").
...
### Nyelvi architektúra (UI Locale vs Task Output Language)

-   **UI Locale** csak a bővítmény felületi szövegét szabályozza (Settings-címkék, oldalsáv-gombok, értesítések és párbeszédablakok). Az alapértelmezett `auto` mód az Obsidian aktuális felületi nyelvét követi.
-   **Task Output Language** a modell által generált feladatkimenetet szabályozza (hivatkozások, összefoglalók, címgenerálás, Mermaid-összefoglaló, fogalomkivonatolás, fordítási célnyelv).
...
-   **Disable auto translation** a nem `Translate` feladatokat a forrásnyelvi kontextusban tartja, miközben az explicit `Translate` feladatok továbbra is a konfigurált célnyelvet kényszerítik.
...
-   **Enable Stable API Calls (Retry Logic)**:
    *   **Disabled (Default)**: egyetlen API-hívási hiba leállítja az aktuális feladatot.
    *   **Enabled**: automatikusan újrapróbálja a sikertelen LLM API-hívásokat (hasznos időszakos hálózati problémák vagy rate limit esetén).
...
-   **API Error Debugging Mode**:
    *   **Disabled (Default)**: szabványos, tömör hibajelentést használ.
    *   **Enabled**: részletes hibalogolást aktivál (hasonlóan a DeepSeek részletes kimenetéhez) minden szolgáltatóhoz és feladathoz (beleértve a Translate, Search és Connection Tests feladatokat). Ide tartoznak a HTTP-állapotkódok, nyers válaszszövegek, kérés-szállítási idővonalak, megtisztított kérés-URL-ek és fejlécek, próbálkozási idők, válaszfejlécek, részleges választestek, elemzett részleges stream-kimenet és veremnyomok, ami kulcsfontosságú az API-kapcsolati problémák és a felsőbb gateway-ek visszaállításainak hibakereséséhez.
-   **Developer Mode**:
    *   **Disabled (Default)**: elrejti az összes csak fejlesztőknek szánt diagnosztikai vezérlőt a normál felhasználók elől.
    *   **Enabled**: megjelenít egy dedikált fejlesztői diagnosztikai panelt a Settings felületen.
...
-   **Customize Processed File Save Path**:
    *   **Disabled (Default)**: a feldolgozott fájlok (például `YourNote_processed.md`) ugyanabba a mappába kerülnek, mint az eredeti jegyzet.
    *   **Enabled**: lehetővé teszi egyéni mentési hely megadását.
...
-   **Use Custom Output Filename for 'Add Links'**:
    *   **Disabled (Default)**: az 'Add Links' parancs által létrehozott feldolgozott fájlok az alapértelmezett `_processed.md` utótagot kapják (például `YourNote_processed.md`).
    *   **Enabled**: lehetővé teszi a kimeneti fájlnév testreszabását az alábbi beállítással.
...
-   **Remove Code Fences on Add Links**:
    *   **Disabled (Default)**: a kódkeretek **(\`\\\`\`)** megmaradnak a tartalomban a linkek hozzáadásakor, míg a **(\`\\\`markdown)** rész automatikusan törlődik.
    *   **Enabled**: eltávolítja a kódkereteket a tartalomból a linkek hozzáadása előtt.
...
-   **Customize Concept Note Path**:
    *   **Disabled (Default)**: a `[[linked concepts]]` jegyzetek automatikus létrehozása ki van kapcsolva.
    *   **Enabled**: lehetővé teszi annak a mappának a megadását, ahová az új fogalomjegyzetek kerülnek.
...
-   **Generate Concept Log File**:
    *   **Disabled (Default)**: nem készül naplófájl.
    *   **Enabled**: létrehoz egy naplófájlt, amely a feldolgozás után felsorolja az újonnan létrehozott fogalomjegyzeteket. A formátum a következő:
        ```
...
-   **Customize Log File Save Path**: (csak akkor látható, ha a "Generate Concept Log File" be van kapcsolva)
    *   **Disabled (Default)**: a naplófájl a **Concept Note Folder Path** mappába kerül (ha meg van adva), egyébként a vault gyökerébe.
    *   **Enabled**: lehetővé teszi egyéni naplómappa megadását.
...
-   **Customize Log File Name**: (csak akkor látható, ha a "Generate Concept Log File" be van kapcsolva)
    *   **Disabled (Default)**: a naplófájl neve `Generate.log`.
    *   **Enabled**: lehetővé teszi egyéni naplófájlnév megadását.
...
-   **Enable Batch Parallelism**:
    *   **Disabled (Default)**: a kötegelt feldolgozási feladatok (például "Process Folder" vagy "Batch Generate from Titles") egyesével, sorosan dolgozzák fel a fájlokat.
    *   **Enabled**: lehetővé teszi, hogy a bővítmény több fájlt párhuzamosan dolgozzon fel, ami jelentősen felgyorsíthatja a nagy kötegelt munkákat.
...
-   **Customise Translation File Save Path**:
    *   **Disabled (Default)**: a lefordított fájlok ugyanabba a mappába kerülnek, mint az eredeti jegyzet.
    *   **Enabled**: lehetővé teszi egy *relatív útvonal* megadását a vaulton belül (például `Translations`), ahová a lefordított fájlok kerülnek. A mappák automatikusan létrejönnek, ha nem léteznek.
-   **Use custom suffix for translated files**:
    *   **Disabled (Default)**: a lefordított fájlok az alapértelmezett `_translated.md` utótagot kapják (például `YourNote_translated.md`).
    *   **Enabled**: lehetővé teszi egyéni utótag megadását.
...
-   **Enable Research in "Generate from Title"**:
    *   **Disabled (Default)**: a "Generate from Title" csak a címet használja bemenetként.
    *   **Enabled**: webes kutatást végez a konfigurált **Web Research Provider** segítségével, és a találatokat kontextusként hozzáadja az LLM-nek küldött címalapú generáláshoz.
-   **Auto-run Mermaid Syntax Fix after Generation**:
    *   **Enabled (Default)**: automatikusan lefuttat egy Mermaid-szintaxisjavító lépést a Mermaidhez kapcsolódó munkafolyamatok után, mint például Process, Generate from Title, Batch Generate from Titles, Research & Summarize, Summarise as Mermaid és Translate.
    *   **Disabled**: a generált Mermaid-kimenetet érintetlenül hagyja, hacsak nem futtatja kézzel a `Batch Mermaid Fix` műveletet, vagy nem adja hozzá egy egyéni munkafolyamathoz.
...
-   **Use Custom Output Folder for 'Generate from Title'**:
    *   **Disabled (Default)**: a sikeresen generált fájlok egy `[OriginalFolderName]_complete` nevű almappába kerülnek az eredeti mappa szülőkönyvtárához képest (vagy `Vault_complete` mappába, ha az eredeti mappa a gyökér volt).
    *   **Enabled**: lehetővé teszi, hogy egyéni nevet adjon meg annak az almappának, ahová a kész fájlok kerüljenek.
...
-   **Enable Custom Prompts for Specific Tasks**:
    *   **Disabled (Default)**: a bővítmény minden művelethez a beépített alapértelmezett promptokat használja.
    *   **Enabled**: aktiválja annak lehetőségét, hogy a lent felsorolt feladatokhoz egyéni promptokat állítson be. Ez ennek a funkciónak a főkapcsolója.
...
-   **Use Custom Prompt for [Task Name]**: (csak akkor látható, ha a fenti beállítás engedélyezve van)
    ...
    *   **Enabled**: ez a feladat az alábbi, megfelelő "Custom Prompt" szövegmezőben megadott szöveget használja.
...
-   **Enable Focused Learning Domain**:
    *   **Disabled (Default)**: az LLM-nek küldött promptok a standard, általános célú utasításokat használják.
    *   **Enabled**: lehetővé teszi, hogy egy vagy több szakterületet adjon meg az LLM kontextuális megértésének javítására.
...
-   Használja az oldalsáv tetején található **Quick Workflows** területet egyéni, több lépésből álló gombok indításához.
...
-   **LM Studio/Ollama kapcsolati problémák**:
    *   **Test Connection Fails**: győződjön meg róla, hogy a helyi szerver (LM Studio vagy Ollama) fut, és a megfelelő modell be van töltve / elérhető.
