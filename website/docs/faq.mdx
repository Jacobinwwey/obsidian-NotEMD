...
faqItems:
  ...
  - question: What LLM providers does Notemd support?
    answer: Notemd currently ships 36 provider definitions, including OpenAI, Anthropic, Google, xAI, Mistral, DeepSeek, Qwen, Ollama, LM Studio, Azure OpenAI, OpenRouter, and China-focused providers such as Doubao, Moonshot, and GLM.
  ...
  - question: Does Notemd support Chinese or other languages?
    answer: The plugin UI supports 21 languages. Task output language is independently configurable, allowing mixed setups like Chinese UI with English output or vice versa.
  ...
  - question: What diagram types can Notemd generate?
    answer: Notemd exposes 33 executable catalog rows spanning Mermaid, JSON Canvas, Vega-Lite, editable HTML/SVG, Draw.io XML, Drawnix JSON, and constrained CircuitikZ artifacts. Five exact reference grammars remain roadmap-only; see the diagram capability catalog and real-Vault examples for the row-level contract.
...
<TLDR>
**Quick answers to the most common Notemd questions.** Covers installation (5 min setup), 36 provider definitions, local/cloud model options, one-click workflows, a 21-language plugin UI, wiki-linking automation, research integration, PDF reading workflows, diagram generation, and open-source licensing. All features work offline with Ollama.
</TLDR>
...
Notemd ships **36 provider definitions**:
