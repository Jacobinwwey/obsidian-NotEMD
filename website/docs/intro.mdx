...
Notemd integrates **36 provider definitions** across cloud, gateway, and local runtimes (including OpenAI, Anthropic, Google, DeepSeek, Qwen, Ollama, and more) into your Obsidian workflow to automate knowledge extraction, organization, translation, research, and diagram generation.
...
- Supports 21 plugin UI languages
...
- **Constrained circuit diagrams**: CircuitikZ generation uses typed `CircuitSpec` payloads, deterministic golden templates, render diagnostics, and topology/layout validation rather than unrestricted LLM TikZ
...
The current implementation supports **33 executable diagram catalog rows** across Mermaid, JSON Canvas, Vega-Lite, HTML fallback, editable HTML/SVG, Draw.io XML artifacts, Drawnix JSON, and constrained CircuitikZ output. Each row has a production fixture and deterministic preview; the real-Vault learning set under [`docs/diagram-examples/`](https://github.com/Jacobinwwey/obsidian-NotEMD/tree/main/docs/diagram-examples) records bilingual inputs and generated results. Five exact reference grammars remain roadmap-only because their layout contracts are not claimed. Circuit diagrams stay deliberately constrained: typed `CircuitSpec` plus golden templates is safer than unrestricted LaTeX generation, while TikZJax remains an external preview/authoring boundary rather than a bundled runtime.
...
- Each project retains its own license and terms; Notemd is available under the [MIT License](https://github.com/Jacobinwwey/obsidian-NotEMD/blob/main/LICENSE).
