...
## Notemd settings discovery and favorites

The settings page is designed for repeated use rather than one-time configuration:

- Use the field-aware **Search settings** input to find a setting by its name, description, alias, or visible category. Results are ranked by signal and jump directly to the concrete setting.
- Use **★ Favorites** to keep frequently adjusted settings in the current Vault. Clicking it opens a dedicated list with the setting name, description, category, and a direct navigation target. Remove a favorite from the list without leaving the settings page.
- The discovery toolbar is expanded by default. Use the chevron button to collapse it into a floating top-right control when you need more vertical space; the current query is preserved and restored when expanded.
- The favorites list is independent from the favorites-only filter. Favorites use stable setting IDs, survive Obsidian restarts, and stale IDs are discarded safely when a setting no longer exists.

The **Diagram preview export PPI** setting defaults to **300 PPI** and accepts **72-600**. It controls PNG rasterization only; SVG and PDF preserve vector geometry. The **Also export complete Mermaid visuals** setting controls optional external Mermaid companions and does not control Mermaid visibility in Drawnix previews.
...
1. **Select provider** from dropdown (36 definitions available)
2. **Enter API Key**
...
Options: 21 plugin UI languages
