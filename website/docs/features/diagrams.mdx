...
| circuitikz | `CircuitSpec -> circuitikz` prototype exports common-source and CMOS inverter golden templates, parses compile logs, can run explicit local renderers plus `--expected-artifact`, SVG `--expected-svg-text`, and PNG nonblank screenshot smoke checks without shell parsing, and now has generic preview diagnostics plus source-only fallback through `RenderArtifact.diagnostics` and the preview modal | Add real-environment smoke fixtures, overlap checks, and topology-preserving repair |
