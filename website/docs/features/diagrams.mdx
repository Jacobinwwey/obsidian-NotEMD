...
The preview modal also has an artifact diagnostics panel. Renderers and smoke checks can attach `RenderArtifact.diagnostics`; the modal shows severity, diagnostic kind, message, and repair advice next to the preview. For artifacts that have source content but cannot be rendered inline or through the HTML iframe path, the modal now falls back to a source-only preview instead of forcing an empty iframe. This gives circuitikz compile/render smoke, SVG text-token checks, PNG blank-screenshot checks, and future overlap reports a visible UI surface without making TikZJax or LaTeX a hard plugin runtime dependency or pretending source text is a verified visual render.
...
SVG smoke verifies the `<svg>` root, positive dimensions or `viewBox`, at least one visible drawing element, any requested text tokens, obvious elements outside the `viewBox`, and obvious overlapping positioned `<text>` labels. The geometry pass is now transform-aware geometry for common group and element `transform` attributes, so translated, scaled, rotated, skewed, or matrix-transformed SVG boxes are checked after transform composition. If a renderer converts labels into reusable path glyphs instead of searchable `<text>`, the smoke report records `pathOnlyGlyphUseCount` and fails the requested text token through `render-svg-text-path-only` instead of pretending the label is simply absent. Other failures are reported through `render-svg-invalid`, `render-svg-dimension-missing`, `render-svg-no-visible-elements`, `render-svg-text-missing`, `render-svg-out-of-bounds`, or `render-svg-text-overlap`. Text-token and text-overlap checks should only be treated as structural smoke for renderers that preserve labels as searchable SVG text; path-only SVG output still needs the later screenshot/OCR gate to prove label identity and legibility.

If the renderer emits `.png`, the same expected-artifact path becomes a first screenshot smoke: Notemd decodes non-interlaced 8-bit grayscale/RGB/RGBA PNG files, checks positive dimensions, records foreground bounds as `foregroundBounds`, fails with `render-png-blank` when every visible pixel matches the top-left background color, and fails with `render-png-content-clipped` when foreground content touches the image boundary. This catches blank screenshots and obvious canvas clipping without adding a platform-specific shell dependency. It is not yet OCR-level overlap detection or topology-preserving image repair.
...
| circuitikz | `CircuitSpec -> circuitikz` prototype exports common-source and CMOS inverter golden templates, rejects repair topology drift through `--topology-reference`, parses compile logs, can run explicit local renderers plus `--expected-artifact`, SVG `--expected-svg-text`, `render-svg-text-path-only` / `pathOnlyGlyphUseCount` classification for path-only labels, transform-aware geometry for SVG bounded-canvas/text-overlap smoke, and PNG nonblank / `render-png-content-clipped` screenshot smoke checks without shell parsing, includes aggregate maintainer smoke fixtures through `npm run diagram:smoke-circuitikz`, and has generic preview diagnostics plus source-only fallback through `RenderArtifact.diagnostics` and the preview modal | Add OCR-level label recognition, pixel-level overlap checks, and automated topology-preserving repair |
