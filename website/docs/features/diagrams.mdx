...
The compile runner uses `shell: false`, expands `{tex}`, `{outputDir}`, and `{jobName}` placeholders into argument-array values, reads the generated `{jobName}.log`, and returns `compileExecution` plus `compileDiagnostics` in the CLI JSON output. It still does not bundle LaTeX or make TikZJax a plugin runtime dependency.
...
| circuitikz | `CircuitSpec -> circuitikz` prototype exports common-source and CMOS inverter golden templates, parses compile logs, and can run explicit local renderers without shell parsing through `npm run diagram:export-circuitikz` | Add real-environment smoke fixtures, screenshot feedback, and topology-preserving repair |
