...
SVG smoke verifies the `<svg>` root, positive dimensions or `viewBox`, at least one visible drawing element, and any requested text tokens. Failures are reported through `render-svg-invalid`, `render-svg-dimension-missing`, `render-svg-no-visible-elements`, or `render-svg-text-missing`. Text-token checks should only be used with renderers that preserve labels as searchable SVG text; path-only SVG output still needs the later screenshot/OCR gate.
...
| circuitikz | `CircuitSpec -> circuitikz` prototype exports common-source and CMOS inverter golden templates, parses compile logs, can run explicit local renderers plus `--expected-artifact`, SVG `--expected-svg-text`, and PNG nonblank screenshot smoke checks without shell parsing, includes aggregate maintainer smoke fixtures through `npm run diagram:smoke-circuitikz`, and has generic preview diagnostics plus source-only fallback through `RenderArtifact.diagnostics` and the preview modal | Add bounded-canvas checks, overlap checks, and topology-preserving repair |
