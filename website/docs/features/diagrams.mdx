...
| circuitikz | `CircuitSpec -> circuitikz` prototype exports common-source and CMOS inverter golden templates, parses compile logs, and can run explicit local renderers plus `--expected-artifact` and SVG `--expected-svg-text` smoke checks without shell parsing through `npm run diagram:export-circuitikz` | Add real-environment smoke fixtures, screenshot feedback, and topology-preserving repair |
