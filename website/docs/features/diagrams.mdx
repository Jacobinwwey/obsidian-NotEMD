...
| circuitikz | `CircuitSpec -> circuitikz` prototype exports common-source and CMOS inverter golden templates, rejects repair topology drift through `--topology-reference`, parses compile logs, can run explicit local renderers plus `--expected-artifact`, SVG `--expected-svg-text`, transform-aware geometry for SVG bounded-canvas/text-overlap smoke, and PNG nonblank / `render-png-content-clipped` screenshot smoke checks without shell parsing, includes aggregate maintainer smoke fixtures through `npm run diagram:smoke-circuitikz`, and has generic preview diagnostics plus source-only fallback through `RenderArtifact.diagnostics` and the preview modal | Add path-only/OCR-level overlap checks and automated topology-preserving repair |
