...
| circuitikz | `CircuitSpec -> circuitikz` prototype exports common-source and CMOS inverter golden templates, parses compile logs, and can run explicit local renderers plus `--expected-artifact`, SVG `--expected-svg-text`, and PNG nonblank screenshot smoke checks without shell parsing through `npm run diagram:export-circuitikz` | Add real-environment smoke fixtures, overlap checks, and topology-preserving repair |
