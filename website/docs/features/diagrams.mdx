...
The preview modal also has an artifact diagnostics panel. Renderers and smoke checks can attach `RenderArtifact.diagnostics`; the modal shows severity, diagnostic kind, message, and repair advice next to the preview. This gives circuitikz compile/render smoke, SVG text-token checks, PNG blank-screenshot checks, and future overlap reports a visible UI surface without making TikZJax or LaTeX a hard plugin runtime dependency.
...
| circuitikz | `CircuitSpec -> circuitikz` prototype exports common-source and CMOS inverter golden templates, parses compile logs, can run explicit local renderers plus `--expected-artifact`, SVG `--expected-svg-text`, and PNG nonblank screenshot smoke checks without shell parsing, and now has a generic preview diagnostics UI through `RenderArtifact.diagnostics` | Add real-environment smoke fixtures, overlap checks, and topology-preserving repair |
