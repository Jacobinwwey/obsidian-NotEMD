...
If the renderer emits `.png`, the same expected-artifact path becomes a first screenshot smoke: Notemd decodes non-interlaced 8-bit grayscale/RGB/RGBA PNG files, checks positive dimensions, and fails with `render-png-blank` when every visible pixel matches the top-left background color. This catches blank screenshots without adding a platform-specific shell dependency. It is not yet overlap detection or topology-preserving image repair.
