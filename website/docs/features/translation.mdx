...
<TLDR>
**Notemd translates text between 21 plugin UI languages using LLM-powered translation.** Supports single-selection translation, full-note translation, and batch folder translation. Each translation task can use a dedicated provider and model via per-task settings. Output language is independently configurable from the UI language. Results are appended or written to a new file depending on your preference.
