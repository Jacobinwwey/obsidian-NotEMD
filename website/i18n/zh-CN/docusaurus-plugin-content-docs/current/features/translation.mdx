...
<TLDR>
**Notemd** 能够借助 LLM 提供的翻译技术，在 21 种插件 UI 语言之间实现文本互译。它支持单条文本翻译、整篇文档翻译以及批量文件夹翻译功能。每项翻译任务均可通过任务专属设置选择不同的服务提供商和模型。输出语言可独立于 UI 语言进行配置。根据您的需求，翻译结果可以追加到现有文件中，也可以写入新的文件中。
