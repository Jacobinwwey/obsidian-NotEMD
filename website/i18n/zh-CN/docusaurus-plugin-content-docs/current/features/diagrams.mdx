...
repair brief 使用 schema `notemd.circuitikz.repair-brief.v1`，携带 source `CircuitSpec`、topology signature、compile/render diagnostics、允许修改项、禁止拓扑修改项和下一步验证步骤。它是后续 repair loop 的 handoff format，不代表 Notemd 已经运行自动视觉修复。
