...
SVG smoke 会检查 `<svg>` root、正的尺寸或 `viewBox`、至少一个可见绘图元素，以及显式要求的文本 tokens。失败会通过 `render-svg-invalid`、`render-svg-dimension-missing`、`render-svg-no-visible-elements` 或 `render-svg-text-missing` 报告。文本 token 检查只适合保留 searchable SVG text 的 renderer；如果 renderer 输出 path-only SVG，仍然需要后续 screenshot/OCR gate。
...
| circuitikz | `CircuitSpec -> circuitikz` 原型已经可通过 `npm run diagram:export-circuitikz` 导出 common-source 与 CMOS inverter golden templates、解析 compile logs，并在不经过 shell parsing 的情况下运行显式本地 renderer、`--expected-artifact`、SVG `--expected-svg-text` 与 PNG nonblank screenshot smoke checks；现在还通过 `npm run diagram:smoke-circuitikz` 提供聚合 maintainer smoke fixtures，并通过 `RenderArtifact.diagnostics` 和 preview modal 具备通用 preview diagnostics 与 source-only fallback | 增加 bounded-canvas checks、overlap checks 与 topology-preserving repair |
