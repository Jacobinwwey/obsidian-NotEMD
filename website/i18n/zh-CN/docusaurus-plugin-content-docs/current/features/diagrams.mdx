...
preview modal 现在也有 artifact diagnostics panel。renderer 和 smoke checks 可以把诊断挂到 `RenderArtifact.diagnostics`；modal 会在预览旁显示 severity、diagnostic kind、message 与 repair advice。这样 circuitikz compile/render smoke、SVG text-token checks、PNG blank-screenshot checks 和未来 overlap reports 都有用户可见的 UI surface，同时不把 TikZJax 或 LaTeX 变成插件 runtime 硬依赖。
