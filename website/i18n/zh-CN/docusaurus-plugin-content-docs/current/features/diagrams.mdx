...
```bash
npm run diagram:export-circuitikz -- --input cmos-inverter.json --output cmos-inverter.tex --compile-executable pdflatex --compile-arg -interaction=nonstopmode --compile-arg -halt-on-error --compile-arg -output-directory={outputDir} --compile-arg {tex}
```

compile runner 使用 `shell: false`，把 `{tex}`、`{outputDir}` 和 `{jobName}` placeholders 展开为参数数组值，读取生成的 `{jobName}.log`，并在 CLI JSON output 中返回 `compileExecution` 与 `compileDiagnostics`。它仍然不捆绑 LaTeX，也不把 TikZJax 变成插件 runtime 依赖。
...
| circuitikz | `CircuitSpec -> circuitikz` 原型已经可通过 `npm run diagram:export-circuitikz` 导出 common-source 与 CMOS inverter golden templates、解析 compile logs，并在不经过 shell parsing 的情况下运行显式本地 renderer | 增加真实环境 smoke fixtures、screenshot feedback 与 topology-preserving repair |
