...
SVG smoke 会检查 `<svg>` root、正的尺寸或 `viewBox`、至少一个可见绘图元素、显式要求的文本 tokens、明显跑出 `viewBox` 的元素，以及明显重叠的定位 `<text>` labels。geometry pass 现在具备 transform-aware geometry，能对常见 group 与 element `transform` 属性做组合后再检查 translated、scaled、rotated、skewed 或 matrix-transformed SVG boxes。失败会通过 `render-svg-invalid`、`render-svg-dimension-missing`、`render-svg-no-visible-elements`、`render-svg-text-missing`、`render-svg-out-of-bounds` 或 `render-svg-text-overlap` 报告。文本 token 与 text-overlap 检查只应被当作保留 searchable SVG text 的 renderer 的结构 smoke；如果 renderer 输出 path-only SVG，仍然需要后续 screenshot/OCR gate。
...
| circuitikz | `CircuitSpec -> circuitikz` 原型已经可通过 `npm run diagram:export-circuitikz` 导出 common-source 与 CMOS inverter golden templates、通过 `--topology-reference` 拒绝 repair topology drift、解析 compile logs，并在不经过 shell parsing 的情况下运行显式本地 renderer、`--expected-artifact`、SVG `--expected-svg-text`、带 transform-aware geometry 的 SVG bounded-canvas / text-overlap smoke 与 PNG nonblank / `render-png-content-clipped` screenshot smoke checks；现在还通过 `npm run diagram:smoke-circuitikz` 提供聚合 maintainer smoke fixtures，并通过 `RenderArtifact.diagnostics` 和 preview modal 具备通用 preview diagnostics 与 source-only fallback | 增加 path-only/OCR-level overlap checks 与自动化 topology-preserving repair |
