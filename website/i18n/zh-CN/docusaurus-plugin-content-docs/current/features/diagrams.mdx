...
| circuitikz | `CircuitSpec -> circuitikz` 原型已经可通过 `npm run diagram:export-circuitikz` 导出 common-source 与 CMOS inverter golden templates、解析 compile logs，并在不经过 shell parsing 的情况下运行显式本地 renderer、`--expected-artifact`、SVG `--expected-svg-text` 与 PNG nonblank screenshot smoke checks；现在也通过 `RenderArtifact.diagnostics` 和 preview modal 具备通用 preview diagnostics 与 source-only fallback | 增加真实环境 smoke fixtures、overlap checks 与 topology-preserving repair |
