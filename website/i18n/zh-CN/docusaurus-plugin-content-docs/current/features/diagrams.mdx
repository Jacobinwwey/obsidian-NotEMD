...
preview modal 现在也有 artifact diagnostics panel。renderer 和 smoke checks 可以把诊断挂到 `RenderArtifact.diagnostics`；modal 会在预览旁显示 severity、diagnostic kind、message 与 repair advice。对于有 source content 但不能 inline 渲染、也不能走 HTML iframe path 的 artifact，modal 会回退到 source-only preview，而不是强行塞进空 iframe。这样 circuitikz compile/render smoke、SVG text-token checks、PNG blank-screenshot checks 和未来 overlap reports 都有用户可见的 UI surface，同时不把 TikZJax 或 LaTeX 变成插件 runtime 硬依赖，也不把源码文本伪装成已经验证过的视觉渲染。
...
SVG smoke 会检查 `<svg>` root、正的尺寸或 `viewBox`、至少一个可见绘图元素、显式要求的文本 tokens、明显跑出 `viewBox` 的元素，以及明显重叠的定位 `<text>` labels。geometry pass 现在具备 transform-aware geometry，能对常见 group 与 element `transform` 属性做组合后再检查 translated、scaled、rotated、skewed 或 matrix-transformed SVG boxes。如果 renderer 把 label 转成可复用 path glyphs 而不是 searchable `<text>`，smoke report 会记录 `pathOnlyGlyphUseCount`，并用 `render-svg-text-path-only` 让被请求的文本 token 失败，而不是把它伪装成普通缺失。其他失败会通过 `render-svg-invalid`、`render-svg-dimension-missing`、`render-svg-no-visible-elements`、`render-svg-text-missing`、`render-svg-out-of-bounds` 或 `render-svg-text-overlap` 报告。文本 token 与 text-overlap 检查只应被当作保留 searchable SVG text 的 renderer 的结构 smoke；如果 renderer 输出 path-only SVG，仍然需要后续 screenshot/OCR gate 证明 label identity 与 legibility。

如果 renderer 输出 `.png`，同一个 expected-artifact 路径会变成第一层 screenshot smoke：Notemd 会解码非交错的 8-bit grayscale/RGB/RGBA PNG 文件，检查正尺寸，把前景包围盒记录为 `foregroundBounds`，在所有可见像素都与左上角背景色一致时以 `render-png-blank` 失败，并在前景内容贴到图像边界时以 `render-png-content-clipped` 失败。这能在不引入平台特定 shell 依赖的情况下捕获空白截图和明显画布裁剪，但它还不是 OCR 级 overlap detection 或 topology-preserving image repair。
...
| circuitikz | `CircuitSpec -> circuitikz` 原型已经可通过 `npm run diagram:export-circuitikz` 导出 common-source 与 CMOS inverter golden templates、通过 `--topology-reference` 拒绝 repair topology drift、解析 compile logs，并在不经过 shell parsing 的情况下运行显式本地 renderer、`--expected-artifact`、SVG `--expected-svg-text`、用于 path-only labels 的 `render-svg-text-path-only` / `pathOnlyGlyphUseCount` 分类、带 transform-aware geometry 的 SVG bounded-canvas / text-overlap smoke 与 PNG nonblank / `render-png-content-clipped` screenshot smoke checks；现在还通过 `npm run diagram:smoke-circuitikz` 提供聚合 maintainer smoke fixtures，并通过 `RenderArtifact.diagnostics` 和 preview modal 具备通用 preview diagnostics 与 source-only fallback | 增加 OCR-level label recognition、pixel-level overlap checks 与自动化 topology-preserving repair |
