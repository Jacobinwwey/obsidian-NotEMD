...
| circuitikz | `CircuitSpec -> circuitikz` 原型已经可通过 `npm run diagram:export-circuitikz` 导出 common-source 与 CMOS inverter golden templates、解析 compile logs，并在不经过 shell parsing 的情况下运行显式本地 renderer 与 `--expected-artifact` smoke checks | 增加真实环境 smoke fixtures、screenshot feedback 与 topology-preserving repair |
