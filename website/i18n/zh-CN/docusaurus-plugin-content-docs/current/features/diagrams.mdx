...
| circuitikz | `CircuitSpec -> circuitikz` 原型已经可通过 `npm run diagram:export-circuitikz` 导出 common-source 与 CMOS inverter golden templates，并把已有 compile logs 解析为 diagnostics | 增加可选 renderer execution、screenshot feedback 与 topology-preserving repair |
