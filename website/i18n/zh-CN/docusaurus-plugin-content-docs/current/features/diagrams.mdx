...
SVG smoke 会检查 `<svg>` root、正的尺寸或 `viewBox`、至少一个可见绘图元素、显式要求的文本 tokens、明显跑出 `viewBox` 的元素、明显重叠的定位 `<text>` / `<tspan>` labels，以及通过 `render-svg-label-overlap` 报告的明显 text labels 与 drawing elements 重叠。geometry pass 现在具备 transform-aware geometry，能对常见 group 与 element `transform` 属性做组合后再检查 translated、scaled、rotated、skewed 或 matrix-transformed SVG boxes。它覆盖针对 A/a arc endpoints 的 arc path endpoint geometry、针对 C/S/Q/T curve extrema 的 exact Bezier curve bounds、`polyline` / `polygon` drawing geometry，也会解析 `<use href="#...">` 引用形成的 path-only glyph placement，因此 label 被 renderer 转成可复用 glyph path 后，如果实际放置的 glyph geometry 跑出 `viewBox`，仍会触发 bounded-canvas failure。同一个 `<text>` parent 下的多个 positioned `tspan` labels 会被作为独立 label boxes 比较，避免 LaTeX-style SVG output 把不同标签折叠成一个 text node 后漏检。`<defs>` 内的 definition-only glyph paths 不会被计为可见绘图元素。label-vs-drawing 检查使用小的 drawing-box tolerance，因此细导线和 polygonal component outlines 仍可被当作潜在 label-legibility failure。通过 `<use href="#...">` 解析出的 path-only glyph labels 也会与 drawing boxes 比较；当可复用 glyph geometry 压到导线或器件时，会以 `render-svg-path-glyph-overlap` 失败。如果 renderer 把 label 转成可复用 path glyphs 而不是 searchable `<text>`，smoke report 会记录 `pathOnlyGlyphUseCount`，并用 `render-svg-text-path-only` 让被请求的文本 token 失败，而不是把它伪装成普通缺失。其他失败会通过 `render-svg-invalid`、`render-svg-dimension-missing`、`render-svg-no-visible-elements`、`render-svg-text-missing`、`render-svg-out-of-bounds`、`render-svg-text-overlap`、`render-svg-label-overlap` 或 `render-svg-path-glyph-overlap` 报告。文本 token 与 overlap 检查只应被当作保留 searchable SVG text 的 renderer 的结构 smoke；如果 renderer 输出 path-only SVG，仍然需要后续 screenshot/OCR gate 证明 label identity 与 legibility，而且 arc endpoint checks 不声称 exact arc bounds。
