...
如果 renderer 输出 `.png`，同一个 expected-artifact 路径会变成第一层 screenshot smoke：Notemd 会解码非交错的 8-bit grayscale/RGB/RGBA PNG 文件，检查正尺寸，并在所有可见像素都与左上角背景色一致时以 `render-png-blank` 失败。这能在不引入平台特定 shell 依赖的情况下捕获空白截图，但它还不是 overlap detection 或 topology-preserving image repair。
