...
### Obsidian AI 知识管理指南：核心说明

- 超过 100 词的笔记
- 论文、技术文档、会议记录等概念密集内容
- 内容稳定后处理，不建议反复处理草稿

### Obsidian AI 知识管理指南：配置与模型
...
启用后，每个概念笔记会记录哪些源笔记提及了它。Obsidian 原生的反向链接面板也会显示这些反向连接。
...
| 设置 | 推荐 | 原因 |
|---|---|---|
| `conceptNoteFolder` | `concepts/` 或 `🧠 concepts/` | 保持 vault 整洁 |
| `extractConceptsAddBacklink` | On | 启用反向查找 |
| `extractConceptsMinimalTemplate` | Off | 完整模板带 Linked From |
| 按任务指定模型 | DeepSeek | 概念抽取不需要昂贵模型 |
| 同义词抑制 | On | 同一设置同时影响链接和抽取 |
...
Notemd 本身支持 21 种 UI 语言。翻译目标语言可按任务配置。常见语言对：EN↔ZH、EN↔JA、EN↔KO、EN↔DE、EN↔FR、EN↔ES。

英文深入页：[Translation](https://jacobinwwey.github.io/obsidian-NotEMD/docs/features/translation)

---

## 5. Obsidian AI 知识管理指南：图表与渲染
...
| 设置 | 推荐 | 原因 |
|---|---|---|
| `enableExperimentalDiagramPipeline` | On | 通过 spec-first 提升质量 |
| `experimentalDiagramCompatibilityMode` | `best-fit` | 按意图选择原生目标格式 |
| `summarizeToMermaidProvider` | GPT-4o 或 Claude | 图表规格需要空间推理能力 |
| `autoMermaidFixAfterGenerate` | On | 自动捕获 LLM 语法错误 |
| 本地知识增强 | 专业领域建议开启 | 利用 vault 上下文提升准确性 |

英文深入页：[Diagram Generation](https://jacobinwwey.github.io/obsidian-NotEMD/docs/features/diagrams)

---

## 6. Obsidian AI 知识管理指南：工作流与任务
...
## Obsidian AI 知识管理指南：翻译与语言

### Obsidian AI 知识管理指南：工作流与任务

1. 导入 PDF 内容（或粘贴）
2. **Research** — 获取主题的 web 上下文
3. **Add Links** — 识别并链接关键概念
4. **Extract Concepts** — 创建独立笔记
5. **Generate Diagram** — 可视化论文结构

### Obsidian AI 知识管理指南：工作流与任务

1. 撰写每日笔记
2. **Add Links** — 把今天的想法和已有概念连接起来
3. 概念笔记通过 backlink 自动更新

### Obsidian AI 知识管理指南：工作流与任务

1. 创建包含论文/笔记的文件夹
2. **批量 Add Links** — 处理整个文件夹
3. **去重概念** — 清理近似重复的笔记
4. **Generate Diagram** — 整篇文献的思维导图
