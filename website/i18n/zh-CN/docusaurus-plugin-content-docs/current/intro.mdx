...
Notemd 将覆盖云端、网关和本地运行时的 **36 个 provider 定义**（包括 OpenAI、Anthropic、Google、DeepSeek、Qwen、Ollama 等）集成到您的 Obsidian 工作流中，从而实现知识提取、整理、翻译、研究以及图表生成的自动化处理。
...
- **受约束的电路图**：CircuitikZ 生成使用类型化 `CircuitSpec`、确定性的 golden template、渲染诊断和拓扑/布局校验，而不是不受限制的 LLM TikZ。
...
当前实现支持 **33 个可执行图表目录行**，覆盖 Mermaid、JSON Canvas、Vega-Lite、HTML fallback、editable HTML/SVG、Draw.io XML、Drawnix JSON 和受约束的 CircuitikZ 输出。每行都有生产 fixture 与确定性预览；[`docs/diagram-examples/`](https://github.com/Jacobinwwey/obsidian-NotEMD/tree/main/docs/diagram-examples) 保存双语输入与生成结果。5 个精确参考 grammar 仍只在路线图中保留，因为尚未宣称其布局契约。电路图继续保持受约束：使用类型化 `CircuitSpec` 与 golden template 比直接生成不受限 LaTeX 更可靠；TikZJax 仍属于外部预览/编写边界，不作为内置 runtime 打包。
...
- 各项目均保留自己的许可证和条款；Notemd 使用 [MIT 许可证](https://github.com/Jacobinwwey/obsidian-NotEMD/blob/main/LICENSE)。
