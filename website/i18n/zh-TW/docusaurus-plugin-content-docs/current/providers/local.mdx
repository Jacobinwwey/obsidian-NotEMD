...
|----------|----------|---------------|---------|-------|
| Ollama | `localhost:11434/api` | `llama3` | 不需要 | NDJSON串流；透過`/api/tags`進行模型探索 |
| LMStudio | `localhost:1234/v1` | `local-model` | 不需要 | 傳送 `"EMPTY"` 作為承載令牌；相容於 OpenAI |
...
- [LLM 提供商](/docs/providers/overview) -- 完整的設定與每個任務的模型配置
- [故障排除](/docs/advanced/troubleshooting) -- 解決當本機伺服器未運作時出現的 connection-refused 錯誤
