...
id: diagrams
title: 図表
description: "スペックファーストのアーキテクチャを利用して、ノートからMermaid、JSON Canvas、Vega-Lite、HTML、編集可能な図形アーティファクト、Draw.io、Drawnix、および制約付きcircuitikzの図表を生成します。"
author:
...
<TLDR>
**Notemdは、スペックファーストのパイプラインを通じてノートから図表を生成します。** LLMがレンダラーに依存しない `DiagramSpec` 形式のJSONを出力し、専用のアダプターがそれをMermaid、JSON Canvas、Vega-Lite、HTML、編集可能なHTML/SVG、Draw.io、Drawnix、または制約付きcircuitikzの出力に変換します。9種類のインテントタイプ、自動的なフォールバックチェーン、SVG/PNG/PDFへのエクスポート機能を備えたリアルタイムプレビュー、セマンティック検証、およびローカル知識による強化生成にも対応しています。
</TLDR>
...
この分離は非常に重要であり、`flowchart`インテントはMarkdownノート向けにMermaidとして、堅牢なフォールバック用にHTMLとして、後工程での編集用にEditable HTML/SVGとして、またはSVGレビュー用の付随ファイルを含むDraw.io/Drawnix形式のソースアーティファクトとしてレンダリングできるようになりました。一方、`circuit`インテントはCircuitikzを経由して処理され、検証済みの `CircuitSpec` が必要です。これは任意のTikZテキストを生成するリクエストではありません。
## 使用方法
...
現在の実装では、サードパーティ製エディタへのサポートをアーティファクトの境界に留めつつ、明示的なレンダーターゲットを提供しています。
...
| 意味論的監査 | `scripts/diagram-semantic-verification.js` およびレンダラー／CLI テストによって、Mermaid、JSON Canvas、Vega-Lite、編集可能な HTML/SVG、Draw.io、Drawnix、制約付き Circuitikz がチェックされる |
