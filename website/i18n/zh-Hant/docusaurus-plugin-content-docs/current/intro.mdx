...
id: intro
title: Notemd 簡介
description: 專為 Obsidian 打造的 AI 驅動型知識庫建構工具，具備持續性的筆記轉換、工作流程功能以及以規格為優先的圖表生成功能
...
<TLDR>
**Notemd**（Note + EMD — 增強型 Markdown 文件）是一個開源的 Obsidian 外掛，能將以 LLM 為基礎的閱讀內容轉換為持續性的知識。與會話結束後洞察即消失的聊天式 AI 不同，Notemd 會將結果**直接寫入您的資料庫**，呈現為維基連結、概念筆記、研究摘要、翻譯內容、工作流程及圖表。它專為那些希望將閱讀、研究及視覺化說明累積成結構化且持續演進的知識圖譜的研究人員、學生及知識工作者所設計。
</TLDR>

## 什麼是 Notemd？

Notemd 將 **30 多個大型語言模型**（OpenAI、Anthropic、Google、DeepSeek、Qwen、Ollama 等）整合到您的 Obsidian 工作流程中，用於自動化知識提取、整理、翻譯、研究以及圖表生成。

### 關鍵差異：暫時性知識與持久性知識

| 面向方面 | 以聊天為基礎的 AI（ChatGPT 等） | Notemd |
|--------|-------------------------------|--------|
| **結果存放處** | 聊天記錄（會消失） | 您的 Obsidian 金庫（會持續存在） |
...
- **Draw.io / Drawnix 物件邊界**：來自相同語意圖形模型的、供維護者使用的匯出路徑
- **電路圖規劃路線**： circuitikz/TikZJax 的支援是以黃金參考、限制條件提示、渲染回饋以及拓撲/佈局驗證為核心來設計，而非依賴原始且無限制的 LLM TikZ
- **預覽診斷**：渲染瑕疵可能會顯示編譯/渲染相關的診斷資訊，且非內聯式來源亦可在不需插件端 LaTeX 執行環境的情況下被檢視。
- 對 Mermaid 錯誤的自動語法修正
...
Notemd的圖表生成方式正從「要求模型撰寫一個語法字串」轉變為分層式處理流程：
...
目前的實作已經支援 Mermaid、JSON Canvas、Vega-Lite、HTML 的回退機制，可編輯的 HTML/SVG，Draw.io XML 類型的元件，最簡化的 Drawnix JSON 子集，預覽診斷功能/僅源碼模式回退，以及適用於常見來源與 CMOS 開關金標準模板的離線 `CircuitSpec -> circuitikz` 原型。電路圖則較為複雜：雖然 circuitikz 能夠呈現精確的電氣拓撲結構，但缺乏限制的 LLM 輸出往往會產生難以辨識的走線或無法渲染的 LaTeX 內容。未來的發展方向是透過金標準參考模板、節點網格佈局規則、渲染診斷功能以及截圖回饋循環，來持續限制 circuitikz 的行為。
...
多數 Obsidian AI 外掛都是以對話為主（你提問，AI 回答，洞察內容都留在聊天中）。而 Notemd 則是**以撰寫為主**：AI 會處理你的筆記，並將結構化的結果直接寫入你的資料庫中。

| 功能特性 | Notemd | Copilot | Smart Connections | Text Generator |
|-----------|--------|---------|-------------------|-----------------|
| 自動維基連結插入 | 是的 | 沒有 | 沒有 | 沒有 |
| 概念說明書的撰寫 | 是（含反向連結 + 去重） | 沒有 | 沒有 | 沒有 |
| 圖表生成 | 是的（Mermaid、Canvas、Vega-Lite、HTML、可編輯的工件） | 沒有 | 沒有 | 沒有 |
| 網路研究整合 | 是的 (Tavily + DuckDuckGo) | 沒有 | 沒有 | 沒有 |
