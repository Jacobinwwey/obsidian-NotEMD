...
3. 對於 Ollama：確認 `ollama serve` 正在運行（`ollama list` 應該會返回模型）
4. 關於 LMStudio：確認伺服器正在 `localhost:1234` 上運行
5. 試著使用不同的傳輸方式——行動裝置使用者應確保 `requestUrl` 傳輸方式已啟用
...
**修復：**
1. 某些模型需要特殊權限（例如，透過 Azure 使用的 GPT-4 需要一個部署名稱）
2. 有些供應商會根據方案等級限制模型使用——請檢查您的帳戶。
3. 可能會有地區限制（部分中國的服務供應商會封鎖國際 IP，反之亦然）
4. 驗證模型名稱的拼寫是否正確（例如，當您的方案僅允許使用迷你模型時，應為 `gpt-4o` 而非 `gpt-4o-mini`）
...
5. 考慮轉換為具有更高限制的供應商（DeepSeek, Ollama）
...
**修復：**
1. 檢查診斷面板以查看實際的 LLM 回應
2. 試用功能更強大的模型（有些小型模型在處理結構化輸出時會遇到困難）
3. 請確保該備註的內容足夠豐富（超過 50 個字）。
4. 檢視您的自訂提示詞，確認其中沒有衝突的指示。
5. 暫時關閉同義詞抑制功能，以查看它是否過度過濾。
...
- [Custom Prompts](/docs/advanced/custom-prompts) -- 透過調整提示語來修正意外的 LLM 行為
