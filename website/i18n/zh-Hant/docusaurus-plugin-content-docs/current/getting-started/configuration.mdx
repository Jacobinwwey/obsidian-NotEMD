...
此設定頁面旨在供反複使用，而非僅用於一次性的配置：

- 使用具備欄位辨識功能的 **Search settings** 欄位，可透過設定的名稱、描述、別名或顯示類別來搜尋。搜尋結果會依相關度排序，並直接跳轉至對應的設定。
- 使用 **★ Favorites** 可將經常調整的設定保留在目前的 Vault 中。點擊該按鈕後，會開啟一個專用的清單，顯示設定的名稱、描述、類別以及直接跳轉的連結。您可以在不離開設定頁面的情況下，從清單中移除收藏項目。
- 探索工具列預設是展開狀態。當需要更多垂直空間時，可使用箭頭按鈕將其收縮為右上角的浮動控制項；再次展開時，目前的搜尋條件會被保留並恢復。
- 收藏清單與僅顯示收藏項目的過濾功能是相互獨立的。收藏的設定會使用穩定的設定 ID，即使在 Obsidian 重新啟動後仍會保留；而當某個設定不再存在時，舊的 ID 也會被安全地移除。

**Diagram preview export PPI** 設定的預設值為 **300 PPI**，且可接受的範圍為 **72-600**。此設定僅用於控制 PNG 的光柵化處理；SVG 與 PDF 則會保留向量幾何結構。**Also export complete Mermaid visuals** 設定用於控制是否要同時匯出完整的 Mermaid 圖像，但無法控制 Mermaid 在 Drawnix 預覽中的顯示狀態。
---
